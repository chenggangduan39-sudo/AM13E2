...
double two_tone_test( int nfft, int bin1,int bin2)
{
    ...
    qdm_kiss_fft_cpx *kout	= NULL;
    qdm_kiss_fft_scalar *tbuf = NULL;
    ...
    cfg = qdm_kiss_fftr_alloc(nfft , NULL, NULL);
#else
    cfg = qdm_kiss_fftr_alloc(nfft , 0, NULL, NULL);
#endif
    tbuf	= KISS_FFT_MALLOC(nfft * sizeof(qdm_kiss_fft_scalar));
    kout	= KISS_FFT_MALLOC(nfft * sizeof(qdm_kiss_fft_cpx));
...
int main(int argc,char ** argv)
{
    ...
    printf("sizeof(qdm_kiss_fft_scalar) = %d\n",(int)sizeof(qdm_kiss_fft_scalar) );
    return 0;
}
