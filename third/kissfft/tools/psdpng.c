...
{
    short *inbuf;
    qdm_kiss_fftr_cfg cfg=NULL;
    qdm_kiss_fft_scalar *tbuf;
    qdm_kiss_fft_cpx *fbuf;
    float *mag2buf;
    int i;
    int n;
    int avgctr=0;
    ...
    CHECKNULL( cfg=qdm_kiss_fftr_alloc(nfft,0,0) );
#else
    CHECKNULL( cfg=qdm_kiss_fftr_alloc(nfft,0,0,0) );
#endif
    CHECKNULL( inbuf=(short*)malloc(sizeof(short)*2*nfft ) );
    CHECKNULL( tbuf=(qdm_kiss_fft_scalar*)malloc(sizeof(qdm_kiss_fft_scalar)*nfft ) );
    CHECKNULL( fbuf=(qdm_kiss_fft_cpx*)malloc(sizeof(qdm_kiss_fft_cpx)*nfreqs ) );
    ...
    while (1) {
        ...
        if (remove_dc) {
            ...
            avg /= nfft;
            for (i=0;i<nfft;++i)  tbuf[i] -= (qdm_kiss_fft_scalar)avg;
        }

        /* do FFT */
        qdm_kiss_fftr(cfg,tbuf,fbuf);
