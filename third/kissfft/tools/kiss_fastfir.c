...
typedef qdm_kiss_fft_scalar kffsamp_t;
typedef qdm_kiss_fftr_cfg kfcfg_t;
#define FFT_ALLOC qdm_kiss_fftr_alloc
#define FFTFWD qdm_kiss_fftr
#define FFTINV qdm_kiss_fftri
#else
#define MIN_FFT_LEN 1024
typedef qdm_kiss_fft_cpx kffsamp_t;
typedef qdm_kiss_fft_cfg kfcfg_t;
#define FFT_ALLOC qdm_kiss_fft_alloc
#define FFTFWD qdm_kiss_fft
#ifdef KISS_FFT_ALLINONE
#define FFTINV kiss_ifft
#else
#define FFTINV qdm_kiss_fft
#endif
#endif
...
struct kiss_fastfir_state{
    ...
    kfcfg_t fftcfg;
    kfcfg_t ifftcfg;
    qdm_kiss_fft_cpx * fir_freq_resp;
    qdm_kiss_fft_cpx * freqbuf;
    size_t n_freq_bins;
    kffsamp_t * tmpbuf;
};
        ...
        void * mem,size_t*lenmem)
{
    ...
    /*fftcfg*/
#ifdef KISS_FFT_ALLINONE
    FFT_ALLOC (nfft, NULL, &len_fftcfg);
#else
    ...
    /*ifftcfg*/
#ifdef KISS_FFT_ALLINONE
    FFT_ALLOC (nfft, NULL, &len_ifftcfg);
#else
    ...
    /* fir_freq_resp */
    memneeded += sizeof(qdm_kiss_fft_cpx) * n_freq_bins;
    /* freqbuf */
    memneeded += sizeof(qdm_kiss_fft_cpx) * n_freq_bins;
    ...
    st->freqbuf = (qdm_kiss_fft_cpx*)ptr;
    ptr += sizeof(qdm_kiss_fft_cpx) * n_freq_bins;
    
    st->fir_freq_resp = (qdm_kiss_fft_cpx*)ptr;
    ptr += sizeof(qdm_kiss_fft_cpx) * n_freq_bins;

#ifdef KISS_FFT_ALLINONE
    FFT_ALLOC (nfft,st->fftcfg , &len_fftcfg);
    FFT_ALLOC (nfft,st->ifftcfg , &len_ifftcfg);
#else
