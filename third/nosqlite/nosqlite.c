...
//just use to read
struct nosqlite *
nosqlite_open2(const char *path,int start_seek_pos,int file_len,int capacity)
{
    int rv = -1;
    struct nosqlite *db;
    ...
    int file_end_pos=start_seek_pos+file_len;
    ...
    db->start_seek_pos=start_seek_pos;
    db->file_len=file_len;
	...
	char version[12];
	rv=fseek(db->file,db->start_seek_pos,SEEK_SET);
	if(rv==-1){goto end;}
...
nosqlite_open2(const char *path,int start_seek_pos,int file_len,int capacity)
{
	return nosqlite_open(path,capacity);
}
#endif
