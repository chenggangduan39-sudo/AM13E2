...
exclude_dir=sdk/dev/key sdk/dev/led sdk/dev/wifi sdk/dev/hid 

ifeq ($(use_sdk), 1)
exclude_dir += sdk/audio sdk/binding sdk/module
ifneq ($(use_auth),1)
exclude_dir += sdk/session/auth
endif
ifneq ($(use_enc8838), 1)
exclude_dir += sdk/dev/enc
endif
else
exclude_dir= sdk/dev/enc sdk/audio sdk/module sdk/binding
endif
...
tool_src += ./test/tool/sdk/process/enc.c
#tool_src += ./test/tool/asr/svp-enroll.c
#tool_src += ./test/tool/asr/svp-eval.c

ifeq ($(use_enc8838), 1)
tool_src += ./test/tool/sdk/process/ws222_test.c
endif
ifeq ($(use_zhiwei), 1)
tool_src += ./test/tool/sdk/process/i2c_ws222_std.c
endif
...
############### compile section ###############
BIN_PATH=/home/why/tool/sigmastar/gcc-sigmastar-9.1.0-2020.07-x86_64_arm-linux-gnueabihf/tools/toolchain/gcc-sigmastar-9.1.0-2020.07-x86_64_arm-linux-gnueabihf/bin
TOOL_NAME=/home/why/tool/sigmastar/gcc-sigmastar-9.1.0-2020.07-x86_64_arm-linux-gnueabihf/tools/toolchain/gcc-sigmastar-9.1.0-2020.07-x86_64_arm-linux-gnueabihf
...
INC = -I ${TOOL_NAME}/arm-linux-gnueabihf/libc/usr/include -I . -I ./sdk/api_1 -I ./sdk -I ./qtk -I ./third -I ./third/libmad-0.15.1b -I ./third/libogg/include/ -I ./third/libmad-0.15.1b/msvc++ -I ./third/speexdsp/include -I ./third/speex/include -I ./third/speexdsp/include/speex -I third/libuuid -I third/arm/alsa-lib-1.1.5/include
...
DNN_CFLAG= -mfpu=neon #-march=armv7-a -mtune=cortex-a7 #-mfloat-abi=hard

ifeq ($(use_debug), 1)
OF= -g -ffunction-sections -fdata-sections
else
OF= -O3 -ffast-math -Ofast -Wall -funroll-loops -Wno-unused-result -Wno-write-strings -ffunction-sections -fdata-sections -Wl,--gc-sections -fomit-frame-pointer 
#OF= -O0 -ffast-math -Ofast -Wall -funroll-loops -Wno-unused-result -Wno-write-strings -ffunction-sections -fdata-sections -Wl,--gc-sections -fomit-frame-pointer 
endif
WFLAGS=-Wall 
CFLAGS= $(INC) $(OF) ${WFLAGS} ${DNN_CFLAG}

CFLAGS +=-DUSE_CHAR_NET -DUSE_SQL -DUSE_ALIGN -DHAVE_USLEEP -DUSE_NEON -DUSE_KISS_FFT -DEXPORT="" -DFIXED_POINT #-D_GNU_SOURCE -DUSE_NEON
ifeq ($(use_debug), 0)
#CFLAGS +=-DUSE_NEON 
endif

ifeq ($(use_sdk), 1)
CFLAGS +=-D USE_DUMMY_ICONV -DFIXED_POINT -DUSE_KISS_FFT -DEXPORT="" -DHAVE_CONFIG_H -DFPM_DEFAULT -DOPUS_BUILD -DUSE_ALLOCA -fPIC #-shared
endif

ifeq ($(use_debug), 1)
LFLAGS=-ldl -lpthread -lm -lcrypto
else
LFLAGS=-ldl -Wl,-O1 -Wl,--hash-style=gnu -Wl,--as-needed -lpthread -lm ${libs} 
endif
...
ifeq ($(use_enc8838), 1)
LFLAGS+=-L ./third/tcl -lsigmaster8838
endif
#============================================================================================

ifeq ($(use_onnx), 1)
	LFLAGS += -lonnxruntime -L./third/onnxruntime/onnxruntime-android-armeabi-v7a-1.11.1/lib
endif
...
ifeq ($(use_jinruixian), 1)
	CFLAGS += -DUSE_JINRUIXIAN
endif
...
${libcrypto}:
ifeq ($(use_openssl), 1)
	cd ./third/openssl;make -f makefile.tcl CC_HOME=$(TOOL_NAME) clean all;
endif
