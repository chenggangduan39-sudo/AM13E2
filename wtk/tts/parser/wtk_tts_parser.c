...
static int str_str_count_(char *start,int length,char *sub)
{
	int n=0;
	int len=length;
	int tmp;
...
wtk_strbuf_t * wtk_tts_note_process(wtk_tts_parser_t *p,char *data,int bytes){
	char* s = data;
	char* e = s + bytes;
	int cnt,cnt2,flag = 0,i = 0,wrd_cnt = 0;

	p->n_notes = str_str_count_(data,bytes,"$(");
	//wtk_debug("n note:%d\n",p->n_notes);
	if(p->n_notes <= 0){
		return NULL;
	}

	wtk_strbuf_t *buf, *tmp_buf;
	wtk_tts_phn_repalce_t *pr = NULL;
	p->replace_phns = wtk_heap_malloc(p->heap, sizeof(wtk_tts_phn_repalce_t) * p->n_notes);

	buf = wtk_strbuf_new(1024,1);
	tmp_buf = wtk_strbuf_new(1024,1);
		...
		if(flag == 1){
			if(cnt == 1){
				if(*s == ')'){
					...
					//wtk_debug("%d %d %.*s\n",pr->tone,pr->index,pr->phns->len,pr->phns->data);
				}else if(isdigit(*s)){
					pr->phns = wtk_heap_malloc(p->heap, sizeof(wtk_string_t));
					pr->phns->len = tmp_buf->pos;
					pr->phns->data = wtk_heap_malloc(p->heap, tmp_buf->pos);
					memcpy(pr->phns->data, tmp_buf->data, tmp_buf->pos);
					pr->tone = wtk_str_atoi(s, 1);
				}else if(isalpha(*s)){
				...
				pr->phns->data = wtk_heap_malloc(p->heap, tmp_buf->pos);
				memcpy(pr->phns->data, tmp_buf->data, tmp_buf->pos);
				if(wtk_str_equal_s(s,cnt,"一")){
					pr->tone = 1;
				}else if(wtk_str_equal_s(s,cnt,"二")){
					pr->tone = 2;
				}else if(wtk_str_equal_s(s,cnt,"三")){
					pr->tone = 3;
				}else if(wtk_str_equal_s(s,cnt,"四")){
					pr->tone = 4;
				}else{
					return NULL;
				}
...
int wtk_tts_parser_process(wtk_tts_parser_t *p,char *s,int s_bytes)
{
	...
	//wtk_debug("[%.*s]\n",v.len,v.data);
	wtk_strbuf_t *buf = wtk_tts_note_process(p,v.data,v.len);
	if(buf){
		//wtk_debug("[%.*s]\n",buf->pos,buf->data);
		lab=wtk_tts_segsnt_process(&(p->segsnt),&info,buf->data,buf->pos);
	}else{
		lab=wtk_tts_segsnt_process(&(p->segsnt),&info,v.data,v.len);
	}
	...
	if(ret!=0){goto end;}
	if(!buf){
		ret=wtk_polyphn_process(p->polyphn,&info,lab, 0, NULL);
	}else{
		ret=wtk_polyphn_process(p->polyphn,&info,lab, p->n_notes, p->replace_phns);
