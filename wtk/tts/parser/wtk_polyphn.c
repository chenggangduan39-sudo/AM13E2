...
void wtk_polyphn_reset(wtk_polyphn_t *p)
{
	if(p->cnt > 0){
		int i;
		for(i = 0; i < p->cnt; i++){
			wtk_tts_wrd_pron_t *pron = *(p->replace_pron + i);
			wtk_tts_syl_t *syl = pron->syls + p->replace_n[i];
			syl->tone = p->replace_tone[i];
			syl->v = p->replace_v[i];
		}
		p->cnt = 0;
		wtk_free(p->replace_n);
		wtk_free(p->replace_pron);
		wtk_free(p->replace_v);
		wtk_free(p->replace_tone);
	}
...
int wtk_polyphn_process(wtk_polyphn_t *p,wtk_tts_info_t *info,wtk_tts_lab_t *lab, int n, wtk_tts_phn_repalce_t *rep)
{
	wtk_tts_snt_t **snt,*s;
	wtk_tts_wrd_t **wrds,*w;
	wtk_tts_xsyl_t *xsyl;
	...
	int rep_cnt = 0;
	int wrd_index = -1;
	wtk_tts_syl_t *tmp_syl;

	if(n > 0){
		wrd_index = rep->index;
		p->cnt = n;
		p->replace_n = wtk_malloc(sizeof(int) * n);
		p->replace_pron = wtk_malloc(sizeof(wtk_tts_wrd_pron_t*) * n);
		p->replace_tone= wtk_malloc(sizeof(int) * n);
		p->replace_v = wtk_malloc(sizeof(wtk_string_t*) * n);
	}
	...
	for(i=0;i<lab->snts->nslot;++i)
	{
		...
		for(j=0;j<s->wrds->nslot;++j)
		{
			...
			{
				wrd_cnt++;
				if(wrd_cnt == wrd_index){
					tmp_syl = pron->syls + k;

					*(p->replace_n + rep_cnt) = k;
					*(p->replace_pron + rep_cnt) = pron;
					*(p->replace_tone + rep_cnt) = tmp_syl->tone;
					*(p->replace_v + rep_cnt) = tmp_syl->v;

					tmp_syl->tone = rep->tone;
					tmp_syl->v = wtk_heap_malloc(info->heap, sizeof(wtk_string_t));
					tmp_syl->v->len = rep->phns->len;
					tmp_syl->v->data = wtk_heap_malloc(info->heap, rep->phns->len);
					memcpy(tmp_syl->v->data, rep->phns->data, rep->phns->len);
					rep_cnt++;
					if(rep_cnt >= n){
						wrd_index = -1;
					}else{
