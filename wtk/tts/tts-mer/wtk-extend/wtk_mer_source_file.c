...
{
   return  wtk_mer_source_loader_load_float(sl, src, fn, vf->p, vf->len);
};

int wtk_mer_source_loader_load_vecdf(wtk_source_loader_t *sl, wtk_source_t *src, char *fn, wtk_vecdf_t *vf)
{
    return wtk_mer_source_loader_load_double(sl, src, fn, vf->p, vf->len);
};
...
int wtk_mer_source_loader_load_matf(wtk_source_loader_t *sl, wtk_source_t *src, char *fn, wtk_matf_t *mf)
{
    ...
    return wtk_mer_source_loader_load_float(sl, src, fn, mf->p, len);
	// wtk_exit(1);
};
...
*/
	int ret;
	ret=wtk_mer_source_loader_load_matf(sl, src, fn, mf);
	if (ret!=0)goto end;
	#if defined(USE_MAT_FIX) || defined(USE_NEON_ASM)
...
//    });
	...
	int ret;
	ret=wtk_mer_source_loader_init(sl,src,fn);
	if(ret!=0)goto end;
	ret=wtk_mer_source_read_float(src, f, len, 1);
	if(ret!=0)goto end;
	if(sl->hook==NULL)
	{
		ret=wtk_source_clean_file(src);
		if(ret!=0)goto end;
	}
	ret=0;
end:
...
//    });
	...
	ret=wtk_mer_source_loader_init(sl,src,fn);
	if(ret!=0)goto end;
	ret=wtk_mer_source_read_double(src, f, len, 1);
	if(ret!=0)goto end;
	if(sl->hook==NULL)
	{
		ret=wtk_source_clean_file(src);
