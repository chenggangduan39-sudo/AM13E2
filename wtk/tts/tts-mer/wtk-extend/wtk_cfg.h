...
#define wtk_cfg_def_sprintf_init(lc, fn, v, item_name, ...) {v=wtk_local_cfg_find_string_s(lc, item_name);if(!v){wtk_debug("-------->  not find cfg param:   %s\n", STR(item_name));wtk_exit(1);};sprintf(fn, v->data ,##__VA_ARGS__, "");}
...
#define wtk_cfg_def_param_require(func,lc,cfg,item,v) {func(lc,cfg,item,v);if(!v){wtk_debug("-----------> not find require cfg param:  %s \n", STR(item));wtk_exit(1);}};
