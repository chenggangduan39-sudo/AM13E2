...
#define wtk_mer_wav_stream_setlen(wav, f0_len) {wav->len = ((int)((f0_len - 1) * wav->frame_period/1000.0 * wav->fs) + 1)*sizeof(short) + 44;if(wav->len>wav->max_size){wtk_debug("音频超出预设内存大小 wav->len: %d > wav->max_size: %lu \n", wav->len, wav->max_size );wtk_exit(1);}};
