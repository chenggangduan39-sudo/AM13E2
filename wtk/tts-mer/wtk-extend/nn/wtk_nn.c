...
void wtk_nn_conv1d(wtk_matf_t *kernel, int kernel_size, enum_conv padding, wtk_matf_t *input, wtk_matf_t *dst)
{
    ...
    switch (padding)
    {
        ...
        case enum_conv_same:
            ...
            {wtk_exit_debug("卷积dst大小异常: dst_row: %d != input->row: %d || dst_col: %d != filter:%d \n", dst->row, i_row, dst->col, filter);}
            ...
            break;
        default:
            wtk_exit_debug("未实现的卷积填充方式: enum %d\n", padding);
            break;
    }
    in = input->p;
...
int wtk_nn_conv1d_group(wtk_matf_t *kernel, int kernel_size,int groups, wtk_matf_t *input, wtk_matf_t *dst)
{
    ...
    if(input->col/groups != kernel->col/kernel_size){
        wtk_exit_debug("conv1d group dim error\n");
    }
    if(kernel->row%groups != 0){
        wtk_exit_debug("kernel groupe size error\n");
    }
    ...
    for(i = 0; i < groups; ++i){
        ...
        wtk_memcpy(t_dst_p,s_dst->p,sizeof(float)*s_dst->row*s_dst->col);
...
int wtk_nn_constant_pad1d(wtk_matf_t *in,wtk_matf_t *out,int left,int right)
{
    ...
    if(row_new != out->row || in->col != out->col){
        wtk_exit_debug("pad dim is error\n");
    }
    col = in->col;
    irow = in->row;
    inp = in->p;
    op = out->p;
    if(left > 0){
        wtk_memset(op,0,sizeof(float)*col*left);
        op += left*col;
    }
    if(left < 0){
        inp += col*abs(left);
    ...
    wtk_memcpy(op,inp,sizeof(float)*len);
    op += len;
    if(right > 0){
        wtk_memset(op,0,sizeof(float)*col*right);
    }
...
void wtk_nn_max_pool1d(int pool_size, int stride, enum_conv padding, wtk_matf_t *in, wtk_matf_t *dst)
{
    ...
    switch (padding)
    {
        ...
        case enum_conv_valid:
            wtk_exit_debug("暂未实现的填充方式: valid\n");
        default:
            wtk_exit_debug("暂未实现的填充方式\n");
            break;
    }
    inp = in->p;
    dstp = wtk_matf_at(dst, s, 0);
...
wtk_nn_rnngru_t* wtk_nn_rnngru_new2( wtk_nn_enum_type_t type, int is_forward, int incol, int num_units, void activation(float*, int))
{
    ...
    cell->gate_bias = wtk_mer_vecf_heap_new( heap, gate_col);
    ...
    cell->candidate_bias = wtk_mer_vecf_heap_new( heap, num_units);
    ...
    case wtk_nn_enum_type_pytorch:
        ...
        cell->candidate_bias_hh=wtk_mer_vecf_heap_new( cell->heap, cell->num_units);
        ...
        cell->bias_ih=wtk_mer_vecf_heap_new( heap, 3*num_units);
        cell->bias_hh=wtk_mer_vecf_heap_new( heap, 3*num_units);
...
void wtk_nn_rnngru_reset(wtk_nn_rnngru_t *cell)
{
    ...
    { wtk_exit_debug("rnngru pytorch： 缺少 candidate_bias_hh, 详情查看公式差异 \n"); }
    wtk_vecf_zero(cell->prev_h);
}
