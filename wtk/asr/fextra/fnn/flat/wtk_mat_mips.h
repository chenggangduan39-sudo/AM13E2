#ifndef WTK_VITE_PARM_POST_DNN_FLAT_WTK_MAT_MIPS
#define WTK_VITE_PARM_POST_DNN_FLAT_WTK_MAT_MIPS
...
#define S16SDI(xra,rb,s12)									\
  do { 														\
    __asm__ __volatile ("S16SDI xr%1,%z0,%2,0"		        \
		   :"+d" (rb)				\
 	       :"K"(xra),"I"(s12):"memory");			\
  } while (0)


#define S16STD(xra,rb,s12)									\
  do { 														\
    __asm__ __volatile ("S16STD xr%0,%z1,%2,0"		        \
		   : 												\
 	       :"K"(xra),"d" (rb),"I"(s12):"memory");					\
  } while (0)


#define S32ALNI(xra,xrb,xrc,s12)								    \
 do { 															\
    __asm__ __volatile ("S32ALNI xr%0,xr%1,xr%2,%3"			    \
                 :                                              \
                 :"K"(xra),"K"(xrb),"K"(xrc),"I"(s12));			\
  } while (0)
