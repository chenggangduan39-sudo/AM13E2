...
qtk_kwdec_pth_t* qtk_kwdec_new_pth(qtk_kwdec_t* dec)
{
	qtk_kwdec_pth_t *pth=(qtk_kwdec_pth_t*)wtk_heap_malloc(dec->heap,sizeof(qtk_kwdec_pth_t));
...
qtk_kwdec_token_t* qtk_kwdec_new_token(qtk_kwdec_t* dec,wtk_fst_state_t* state)
{
	qtk_kwdec_token_t *token;

	token=(qtk_kwdec_token_t*)wtk_vpool2_pop(dec->tok_pool);
	token->tot_cost=0.0;
	token->pth=NULL;
	token->state=state;
...
void qtk_kwdec_push_token(qtk_kwdec_t* dec,qtk_kwdec_token_t *tok)
{
	wtk_vpool2_push(dec->tok_pool,tok);
}

int qtk_kwdec_active_words_check(qtk_kwdec_cfg_t *cfg,wtk_string_t *s)
{
	int ret =-1;
	int i;
...
void qtk_kwdec_decwords_set(qtk_kwdec_t* kdec)
{
	int i,n;
	wtk_string_t **s;
	s=(wtk_string_t**)(kdec->cfg->words->slot);
	qtk_kwdec_words_set_t *set;
	kdec->words_set = (qtk_kwdec_words_t**)wtk_malloc(kdec->cfg->words->nslot*sizeof(qtk_kwdec_words_t*));
	for(i=0;i<kdec->cfg->words->nslot;++i)
	{
		// wtk_debug("n=%d,\n",kdec->cfg->words->nslot);
		if((n=qtk_kwdec_active_words_check(kdec->cfg,s[i]))>=0)
		{
			//wtk_debug("n=%d\n",n);
			set = kdec->cfg->set+n;
			kdec->words_set[i]=(qtk_kwdec_words_t*)wtk_malloc(sizeof(qtk_kwdec_words_t));
			kdec->words_set[i]->word = set->word;
			kdec->words_set[i]->key_id = set->key_id;
			kdec->words_set[i]->pdf_conf = set->pdf_conf;
			kdec->words_set[i]->min_kws = set->min_kws;
		}
		else
		{
...
{
	qtk_kwdec_t *kdec = (qtk_kwdec_t*) wtk_malloc(sizeof(qtk_kwdec_t));

	kdec->heap = wtk_heap_new(4096);
	kdec->tok_pool = wtk_vpool2_new(sizeof(qtk_kwdec_token_t),1000);
    ...
    {       
        ...
        kdec->wake_flag=0;
        // kdec->feat_pool=NULL;
        // kdec->feat_pool = wtk_vpool_new(sizeof(wtk_vector_t)*3045,kdec->cfg->wdec_post_win_size);
		kdec->feat_rb = wtk_robin_new(60);
		kdec->reset = 0;
		kdec->reset_frame=0;
		kdec->out_col = 0;
		kdec->words_set=NULL;
		kdec->key_id=-1;
		kdec->min_kws=kdec->cfg->min_kws;
		kdec->pdf_conf=kdec->cfg->pdf_conf;
		qtk_kwdec_decwords_set(kdec);
...
int qtk_kwdec_process_none_emitting_state(qtk_kwdec_t* dec, float cut_off);

int qtk_kwdec_start(qtk_kwdec_t* dec)
{
	int stateID = 0;
	qtk_kwdec_token_t *start_tok;
	wtk_fst_state_t* state;

	state = wtk_fst_net_get_load_state(dec->net, stateID);
	state->frame = 0;
	start_tok = qtk_kwdec_new_token(dec, state);
	state->hook = start_tok;
	dec->decoding_finalized = 0;
	
	wtk_queue_push(dec->cur_tokq,&(start_tok->q_n));
	qtk_kwdec_process_none_emitting_state(dec, dec->cfg->beam);

	return 0;
}

void qtk_kwdec_find_or_add_token(qtk_kwdec_t* dec,
		wtk_fst_trans_t *trans, float tot_cost, int *changed,
		qtk_kwdec_token_t *pre_tok, float ac_cost)
{
//	wtk_debug("===============find or add token state:%d\n",state);
	qtk_kwdec_token_t *token, *update_tok;
	...
	{
		update_tok = (qtk_kwdec_token_t*) state->hook;
	} else
	{
		update_tok = NULL;
	}
...
//		wtk_debug("===============find or add token state not found\n");
		token = qtk_kwdec_new_token(dec, state);
		token->pth = qtk_kwdec_new_pth(dec);
...
float qtk_kwdec_get_cutoff2(qtk_kwdec_t* dec,
		float *adaptive_beam, qtk_kwdec_token_t **best_elem)
{
	...
	if (dec->cfg->max_active == UINT_MAX && dec->cfg->min_active == 0)
	{
		...
		{
			e = data_offset2(qn,qtk_kwdec_token_t,q_n);
			if (e->state->id == 0)
				continue;
...
float qtk_kwdec_process_emitting_state(qtk_kwdec_t* dec,wtk_vector_t *obs)
{
	//wtk_debug("=====================process emitting state\n");
	//wtk_debug("=====================process emitting state %d\n",frame);
	qtk_kwdec_token_t* best_elem = NULL;//, *elem, *e_tail;
	int dnn_cnt, i;
	float adaptive_beam = FLT_MAX;
	float cur_cutoff;

	cur_cutoff = qtk_kwdec_get_cutoff2(dec,
	...
	if (best_elem)
	{
		...
		//qtk_kwdec_token_t *tok = best_elem->token;
	...
	while (1)
	{
		...
		token = data_offset(qn,qtk_kwdec_token_t,q_n);//elem->token;
...
//					wtk_debug("search or add state:%d\n",trans->to_state->id)
					qtk_kwdec_find_or_add_token(dec,
							trans, tot_cost, NULL, token,likelihood);
				}
			}
		}
		qtk_kwdec_push_token(dec,token);
	}
	return next_cutoff;
}

int qtk_kwdec_process_none_emitting_state(qtk_kwdec_t* dec, float cutoff) {
	//wtk_debug("============process none emitting state %f\n",cutoff);
	//qtk_hash_elem_t *elem;
	//st_q_t *st, *st1;
	qtk_kwdec_token_t *token;
	//qtk_kwdec_token_t *new_token;
	wtk_queue_node_t *qn;
...
//		}
		...
		//token = (qtk_kwdec_token_t*) state->hook;
...
int isFiniteNumber2(double d)
{
	return (d < FLT_MAX && d > -FLT_MAX);
}

void qtk_kwdec_compute_final_cost(qtk_kwdec_t* dec)
{
...
//		qtk_kwfstdec_prune_active_tokens(dec,dec->cfg->beam*dec->cfg->prune_scale);
	cost_cutoff = qtk_kwdec_process_emitting_state(dec, f->rv);
	qtk_kwdec_process_none_emitting_state(dec, cost_cutoff);
//	wtk_feat_push_back(f);
	return 0;
}

void qtk_kwdec_log(float *p, int n)
{
	float *pe;
...
int qtk_kwdec_feed2(qtk_kwdec_t* dec, wtk_vector_t *v, int index)
{
	...
	dec->cur_frame = index;
	//print_float(v,10);
	cost_cutoff = qtk_kwdec_process_emitting_state(dec, v2);
	qtk_kwdec_process_none_emitting_state(dec, cost_cutoff);
	if(dec->cfg->use_wdec)
	{
		ret = qtk_kwdec_post_feed(dec,v2);
		if(ret==0)
		{
			dec->found=1;
		}
