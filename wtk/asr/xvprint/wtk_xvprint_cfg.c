...
int wtk_xvprint_cfg_read_enroll_data(wtk_xvprint_cfg_t *cfg,wtk_source_t *src);

int wtk_xvprint_cfg_init(wtk_xvprint_cfg_t *cfg)
{
...
int wtk_xvprint_cfg_clean(wtk_xvprint_cfg_t *cfg)
{
...
int wtk_xvprint_cfg_update_local(wtk_xvprint_cfg_t *cfg, wtk_local_cfg_t *main)
{
...
//     printf("%s\n",cfg->enroll_fn);
    ...
    lc2=wtk_local_cfg_find_lc_s(lc,"xv_vad");
    if(lc2)
    {
        wtk_vad_cfg_init(&cfg->vad);
...
int wtk_xvprint_cfg_update(wtk_xvprint_cfg_t *cfg)
{
    int ret;
    ...
    ret=wtk_xvprint_cfg_update2(cfg,&sl);
    if(ret!=0)
    {
        goto end;
    }
...
int wtk_xvprint_cfg_update2(wtk_xvprint_cfg_t *cfg,wtk_source_loader_t *sl)
{
    int ret;
    wtk_strbuf_t *buf;
...
int wtk_xvprint_cfg_set_vpint_fn(wtk_xvprint_cfg_t *cfg,char *fn)
{
	int ret=0;
    wtk_strbuf_t *buf;
...
int wtk_xvprint_cfg_set_enroll_stats_fn(wtk_xvprint_cfg_t *cfg,char *fn)
{
	int ret=0;
...
int wtk_xvprint_cfg_set_eval_stats_fn(wtk_xvprint_cfg_t *cfg,char *fn)
{
	int ret=0;
	wtk_source_loader_t sl2;
...
wtk_vecf_t* wtk_xvprint_cfg_read_vector(wtk_xvprint_cfg_t *cfg,wtk_source_t *src)
{
    wtk_strbuf_t *buf;
    wtk_vecf_t *vec;
...
wtk_xvprint_cfg_feat_node_t* wtk_xvprint_cfg_node_new3(int name_len)
{
    wtk_xvprint_cfg_feat_node_t *node;
...
wtk_xvprint_cfg_feat_node_t* wtk_xvprint_cfg_node_new2(wtk_string_t *name,wtk_vecf_t *feat,int cnt)
{
    wtk_xvprint_cfg_feat_node_t *node;
...
wtk_xvprint_cfg_feat_node_t* wtk_xvprint_cfg_node_new(char *name,int len)
{
    wtk_xvprint_cfg_feat_node_t *node;
...
void wtk_xvprint_cfg_node_delete(wtk_xvprint_cfg_feat_node_t *node)
{
    if(node->name)
    {
        wtk_free(node->name);
...
int wtk_xvprint_cfg_read_enroll_data(wtk_xvprint_cfg_t *cfg,wtk_source_t *src)
{
    ...
    while(1)
    {
		...
		if(nl){break;}
        node=wtk_xvprint_cfg_node_new(buf->data,buf->pos);
        nl=0;
		wtk_source_skip_sp(src,&nl);
		if(nl){break;}
        ret=wtk_source_read_int(src,&node->num,1,0);
        if(ret!=0){goto end;}
        node->v=wtk_xvprint_cfg_read_vector(cfg,src);
        if(!node->v)
        {
            ret=-1;
            goto end;
