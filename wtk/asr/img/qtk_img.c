...
qtk_img_rec_t *qtk_img_rec_new(qtk_img_cfg_t *cfg)
{
    ...
    if (cfg->use_nnrt) {
        ...
        if (ir->nnrt->cfg->use_rknpu) {
            ir->cache_mem = wtk_malloc(sizeof(void *) * ir->nnrt->num_in - 1);
            for (i = 1; i < ir->nnrt->num_in; i++) {
                int nelem = 1;
                for (j = 0; j < ir->shape[i][0]; j++) {
                    nelem *= ir->shape[i][j + 1];
                }
                ir->cache_mem[i - 1] = wtk_calloc(sizeof(uint16_t), nelem);
                qtk_nnrt_disable_output_dequant(ir->nnrt, i);
            }
        }
    } else {
...
void qtk_img_rec_delete(qtk_img_rec_t *ir)
{
    ...
    if (ir->nnrt) {
        ...
        if (ir->cache_mem) {
            for (i = 0; i < ir->nnrt->num_in - 1; i++) {
                wtk_free(ir->cache_mem[i]);
            }
            wtk_free(ir->cache_mem);
        }
        }
        wtk_free(ir);
}
...
void qtk_img_rec_nnet3_process(qtk_img_rec_t *ir, float *f1) {
    ...
    if(ir->cfg->prob_type != 0){
        float v0 = log(exp(f[0]) + 1e-12);
        float v1 = log(1 - exp(f[0]) + 1e-12);
        speech_prob = v1 - v0;
    }else{
        speech_prob = f[1] - log(exp(f[0]) + 1e-12);
    }
...
static void img_rec_rknpu_forward_(qtk_img_rec_t *ir, uint16_t *feat) {
    int i;
    qtk_nnrt_value_t out;
    qtk_nnrt_feed(ir->nnrt, feat, 0);
    for (i = 1; i < ir->nnrt->num_in; i++) {
        qtk_nnrt_feed(ir->nnrt, ir->cache_mem[i - 1], i);
    }
    qtk_nnrt_run(ir->nnrt);
    qtk_nnrt_get_output(ir->nnrt, &out, 0);
    int64_t shape[8];
    for (i = 1; i < ir->nnrt->num_in; i++) {
        qtk_nnrt_value_t cache;
        qtk_nnrt_get_output(ir->nnrt, &cache, i);
        if (ir->shape[i][0] == 4 && ir->shape[i][4] != 1) {
            int m = ir->shape[i][2] * ir->shape[i][3];
            int n = ir->shape[i][4];
            uint16_t *src = cache;
            uint16_t *dst = ir->cache_mem[i - 1];
            int mm, nn;
            for (mm = 0; mm < m; mm++) {
                for (nn = 0; nn < n; nn++) {
                    dst[mm * n + nn] = src[nn * m + mm];
                }
            }
        } else {
            int nbytes = qtk_nnrt_get_output_nbytes(ir->nnrt, i);
            memcpy(ir->cache_mem[i - 1], cache, nbytes);
        }
    }
    float *prob = (float *)out;
    qtk_nnrt_get_output_shape(ir->nnrt, 0, shape,
                              sizeof(shape) / sizeof(shape[0]));
    for (i = 0; i < shape[1]; i++) {
        qtk_img_rec_nnet3_process(ir, prob + i * shape[2]);
    }
    qtk_nnrt_reset(ir->nnrt);
}
...
static void qtk_img_rec_feed_nnrt(qtk_img_rec_t *ir, float *input) {
    if (ir->nnrt->cfg->use_rknpu) {
        img_rec_rknpu_forward_(ir, cast(uint16_t *, input));
        return;
    }
        ...
        for (i = 0; i < nnrt->num_in - 1; i++) {
            ...
            if (i == nnrt->num_in - 3) {
                cache[i] = qtk_nnrt_value_create_external(
                    ir->nnrt, QTK_NNRT_VALUE_ELEM_BOOL, cache_shape, shape_len,
                    data2);
            } else {
                cache[i] = qtk_nnrt_value_create_external(
...
void qtk_img_rec_on_kxparm(qtk_img_rec_t *ir, wtk_kfeat_t *feat) {
    if (ir->nnrt->cfg->use_rknpu) {
        int k = ir->shape[0][3];
        wtk_strbuf_expand(ir->feat, sizeof(uint16_t) * k);
        qtk_numeric_data_t f32, f16;
        f32.f32 = feat->v;
        f16.u16 = cast(unsigned short *, ir->feat->data + ir->feat->pos);
        qtk_numeric_f32_to_f16(f32, f16, k);
        ir->feat->pos += sizeof(uint16_t) * k;
    } else {
    ...
    if ((ir->num_feats >= ir->shape[0][2]) &&
        ...
        wtk_strbuf_pop(
            ir->feat, NULL,
            ir->shape[0][3] * ir->ret_len *
                (ir->nnrt->cfg->use_rknpu ? sizeof(uint16_t) : sizeof(float)));
...
void qtk_img_rec_on_kxparm_end(qtk_img_rec_t *ir) {
    int i;
    if (ir->num_feats - ir->frames > ir->chunk_len / 3) {
        if (ir->cfg->nnrt.use_rknpu) {
            int k = ir->shape[0][3] - (ir->num_feats - ir->frames);
            uint16_t fp16_eps;
            wtk_strbuf_expand(ir->feat, sizeof(uint16_t) * k);
            qtk_numeric_data_t f32, f16;
            f32.f32 = &eps_feat[0];
            f16.u16 = &fp16_eps;
            qtk_numeric_f32_to_f16(f32, f16, 1);
            for (i = 0; i < k; i++) {
                cast(uint16_t *, ir->feat->data + ir->feat->pos)[i] = fp16_eps;
            }
            ir->feat->pos += sizeof(uint16_t) * k;
        } else {
            for (i = 0; i <= ir->shape[0][3] - (ir->num_feats - ir->frames);
                 i++) {
