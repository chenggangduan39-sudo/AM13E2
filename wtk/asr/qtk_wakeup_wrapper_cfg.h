#ifndef QTK_WAKEUP_WRAPPER_CFG_H_
#define QTK_WAKEUP_WRAPPER_CFG_H_
#include "wtk/core/cfg/wtk_local_cfg.h"
#include "wtk/core/rbin/wtk_rbin2.h"
#include "wtk/core/cfg/wtk_mbin_cfg.h"
#include "wtk/core/cfg/wtk_version_cfg.h"
#include "wtk/asr/vad/wtk_vad.h"
#include "wtk/asr/wfst/k2dec/qtk_k2_dec.h"
#include "wtk/asr/wfst/kwdec2/wtk_kwdec2.h"
#include "wtk/asr/img/qtk_img.h"
#ifdef __cplusplus
extern "C" {
#endif
typedef struct qtk_wakeup_wrapper_cfg qtk_wakeup_wrapper_cfg_t;

typedef enum
{
	QTK_K2=0,
	QTK_KWDEC,
	QTK_IMG
}qtk_wakeup_type;

struct qtk_wakeup_wrapper_cfg
{
	wtk_vad_cfg_t vad;
	wtk_version_cfg_t version;
	wtk_cfg_file_t *cfile;
	wtk_rbin2_t *rbin;
	qtk_k2_wrapper_cfg_t kwake;//k2 wakeup
	wtk_kwdec2_cfg_t kwdec2;
	qtk_img_cfg_t img;
	char *wakeup_type;
	char *wakeup_fn;

	qtk_wakeup_type type;
	wtk_string_t res;
	wtk_strbuf_t *wakeup_buf;
	void *hook;

	unsigned int use_vad:1;
	unsigned int use_vad2:1;
};

int qtk_wakeup_wrapper_cfg_init(qtk_wakeup_wrapper_cfg_t *cfg);
int qtk_wakeup_wrapper_cfg_clean(qtk_wakeup_wrapper_cfg_t *cfg);
int qtk_wakeup_wrapper_cfg_update_local(qtk_wakeup_wrapper_cfg_t *cfg,wtk_local_cfg_t *lc);
int qtk_wakeup_wrapper_cfg_update(qtk_wakeup_wrapper_cfg_t *cfg);
int qtk_wakeup_wrapper_cfg_update2(qtk_wakeup_wrapper_cfg_t *cfg,wtk_source_loader_t *sl);
int qtk_wakeup_wrapper_cfg_update3(qtk_wakeup_wrapper_cfg_t *cfg,wtk_source_loader_t *sl,wtk_rbin2_t *rb);
int qtk_wakeup_wrapper_cfg_bytes(qtk_wakeup_wrapper_cfg_t *cfg);

wtk_main_cfg_t* qtk_wakeup_wrapper_cfg_new(char *cfg_fn);
void qtk_wakeup_wrapper_cfg_delete(wtk_main_cfg_t *main_cfg);
qtk_wakeup_wrapper_cfg_t* qtk_wakeup_wrapper_cfg_new_bin(char *bin_fn);
qtk_wakeup_wrapper_cfg_t* qtk_wakeup_wrapper_cfg_new_bin2(char *bin_fn,unsigned int seek_pos);
void qtk_wakeup_wrapper_cfg_delete_bin(qtk_wakeup_wrapper_cfg_t *cfg);
#ifdef __cplusplus
};
#endif
#endif
