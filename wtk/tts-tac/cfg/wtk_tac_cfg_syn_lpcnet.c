...
{
    wtk_zero(lpcnet,sizeof(wtk_tac_cfg_syn_lpcnet_t));
    lpcnet->nfft = 1024;
    // lpcnet->hop_len = 256;
    lpcnet->hop_len = 200;
    lpcnet->win_len = 800;
...
int wtk_tac_cfg_syn_lpcnet_clean(wtk_tac_cfg_syn_lpcnet_t *lpcnet)
{
    ...
    if(lpcnet->stft) wtk_rfft_delete(lpcnet->stft->rf);
    wtk_heap_delete(lpcnet->heap);
    if(lpcnet->stream_heap) wtk_heap_delete(lpcnet->stream_heap);
    if(lpcnet->stream_wav_buf) wtk_strbufs_delete(lpcnet->stream_wav_buf,lpcnet->num_subband);
    return 0;
}
