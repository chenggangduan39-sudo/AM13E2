...
wtk_mic_check_t *wtk_mic_check_new(wtk_mic_check_cfg_t *cfg) {
    ...
    mic_check->mic_sil_frame =
        (int *)wtk_malloc(sizeof(int) * cfg->nmicchannel);
    mic_check->mic_out_range_frame =
        (int *)wtk_malloc(sizeof(int) * cfg->nmicchannel);
...
void wtk_mic_check_delete(wtk_mic_check_t *mic_check) {
    ...
    wtk_free(mic_check->mic_sil_frame);
    wtk_free(mic_check->mic_out_range_frame);
...
void wtk_mic_check_reset(wtk_mic_check_t *mic_check) {
    ...
    memset(mic_check->mic_sil_frame, 0, sizeof(int) * nmicchannel);
    memset(mic_check->mic_out_range_frame, 0, sizeof(int) * nmicchannel);
    ...
    if (is_end) {
        for (i = 0; i < nmicchannel; ++i) {
            if (mic_check->mic_state[i] == 0) {
                ...
                if (mic_check->mic_state[i] * (1.0 / mic_check->frame) <
                    play_thresh) {
                    if (mic_check->sp_state[i] * (1.0 / mic_check->frame) >
                        play_thresh) {
...
void wtk_mic_check_rcd_check2(wtk_mic_check_t *mic_check, int is_end) {
    wtk_strbuf_t **mic = mic_check->mic;
    wtk_complex_t **fft = mic_check->fft;
    int nbin = mic_check->nbin;
    int wins = mic_check->cfg->wins;
    ...
    int micenr_cnt = mic_check->cfg->micenr_cnt;
    float micenr_thresh = mic_check->cfg->micenr_thresh;
    float micenr_thresh2 = mic_check->cfg->micenr_thresh2;
    float eng_thresh = mic_check->cfg->eng_thresh;
    float var_thresh = mic_check->cfg->var_thresh;
    float sc_thresh = mic_check->cfg->sc_thresh;
    float eng_thresh2 = mic_check->cfg->eng_thresh2;
    ...
    float sc_thresh2 = mic_check->cfg->sc_thresh2;
    float rcd_thresh = mic_check->cfg->rcd_thresh;
    float eng_db_thresh = mic_check->cfg->eng_db_thresh;
    float out_range_thresh = mic_check->cfg->out_range_thresh;
    float micenr = 0;
    float max_micenr = 0;
    float max_data = 0;
    float db_diff = 0;
    ...
    if (is_end) {
        for (i = 0; i < nmicchannel; ++i) {
            if (mic_check->mic_state[i] == 0) {
                ...
                if (mic_check->mic_state[i] * (1.0 / mic_check->frame) >
                    rcd_thresh) {
                    mic_check->type[i] = WTK_MIC_CHECK_RCD_ERROR_OUT_CORR;
                } else {
                    mic_check->type[i] = WTK_MIC_CHECK_NORMAL;
                }
                // printf("%f\n", mic_check->mic_state[i] * (1.0 /
            ...
            if (mic_check->mic_sil_frame[i] > 0) {
                if (mic_check->mic_sil_frame[i] *
                        (1.0 / mic_check->data_frame) >
                    rcd_thresh) {
                    mic_check->type[i] = WTK_MIC_CHECK_RCD_ERROR_SIL;
                }
            }
            if (mic_check->mic_out_range_frame[i] > 0) {
                if (mic_check->mic_out_range_frame[i] *
                        (1.0 / mic_check->data_frame) >
                    out_range_thresh) {
                    mic_check->type[i] = WTK_MIC_CHECK_RCD_ERROR_OUT_RANGE;
                }
            }
        }
        if (mic_check->notify) {
    ...
    mic_check->data_frame++;

    for (i = 0; i < nmicchannel; ++i) {
        fv = (float *)(mic[i]->data);
        micenr = wtk_mic_check_energy(fv, fsize);
        if (micenr < micenr_thresh2) {
            mic_check->mic_sil_frame[i]++;
        }
        max_data = wtk_float_abs_max(fv, fsize);
        if (max_data >= 32767.0) {
            mic_check->mic_out_range_frame[i]++;
        }
        if (micenr > max_micenr) {
            max_micenr = micenr;
        }
    }
    if (max_micenr > micenr_thresh) {
        mic_check->mic_sil = 0;
        mic_check->mic_silcnt = micenr_cnt;
    } else if (mic_check->mic_sil == 0) {
        mic_check->mic_silcnt -= 1;
    ...
    if (mic_check->mic_sil == 0) {
        ...
        if (wtk_float_sum(mic_check->tmp_state, nmicchannel) > 0) {
            for (i = 1; i < nmicchannel; ++i) {
                db_diff = 20 * log10(powf(mic_check->eng[i]/mic_check->eng[0], 0.5));
                if (fabs(db_diff) > eng_db_thresh) {
                    mic_check->mic_state[i]++;
                }
            }
        }
...
void wtk_mic_check_feed(wtk_mic_check_t *mic_check, short *data, int len,
    ...
    while (length >= fsize) {
        ...
        case WTK_MIC_CHECK_PLAY_CHECK:
            ...
            break;
        case WTK_MIC_CHECK_RCD_CHECK2:
            wtk_mic_check_rcd_check2(mic_check, is_end);
            break;
        default:
            break;
        }
