...
void wtk_mask_bf_net_aec_on_gainnet2_2(wtk_mask_bf_net_edr_t *vdr, float *gain,
                                       int len, int is_end);
void wtk_mask_bf_net_on_ssl2(wtk_mask_bf_net_t *mask_bf_net,
                             wtk_ssl2_extp_t *nbest_extp, int nbest, int ts,
                             int te);
...
wtk_mask_bf_net_t *wtk_mask_bf_net_new(wtk_mask_bf_net_cfg_t *cfg) {
    ...
    mask_bf_net->ths2 = NULL;
    mask_bf_net->notify2 = NULL;
    mask_bf_net->ssl_ths = NULL;
    mask_bf_net->notify_ssl = NULL;
    ...
    mask_bf_net->synthesis_mem2 = NULL;
    ...
    mask_bf_net->ekalman = NULL;
    if (cfg->use_kalman) {
        mask_bf_net->ekalman = wtk_kalman_new(&(cfg->echo_kalman));
    }
    ...
    mask_bf_net->mask_bf = NULL;
    if (cfg->use_mask_bf) {
        mask_bf_net->mask_bf = wtk_mask_bf_new(&(cfg->mask_bf));
    }
    ...
    mask_bf_net->fftx2 = NULL;
    mask_bf_net->fft_raw = (wtk_complex_t *)wtk_malloc(
        sizeof(wtk_complex_t) * mask_bf_net->nbin * cfg->num_frame);
    ...
    if (cfg->use_bf2) {
        ...
        if (mask_bf_net->cfg->n_mask_mu >= 0) {
            mask_bf_net->mask_mu =
                (float *)wtk_malloc(sizeof(float) * mask_bf_net->nbin *
    ...
    mask_bf_net->qmmse3 = NULL;
    if (cfg->use_qmmse3) {
        mask_bf_net->qmmse3 = wtk_qmmse_new(&(cfg->qmmse3));
    }
    ...
    mask_bf_net->maskssl2 = NULL;
    mask_bf_net->ssl_mask = NULL;
    if (cfg->use_maskssl2) {
        mask_bf_net->maskssl2 = wtk_maskssl2_new(&(cfg->maskssl2));
        wtk_maskssl2_set_notify(mask_bf_net->maskssl2, mask_bf_net,
                                (wtk_maskssl2_notify_f)wtk_mask_bf_net_on_ssl2);
        mask_bf_net->ssl_mask =
            (float *)wtk_malloc(sizeof(float) * mask_bf_net->nbin);
    }

    mask_bf_net->out = wtk_malloc(sizeof(float) * (mask_bf_net->nbin - 1));
    mask_bf_net->out2 = NULL;
    if (cfg->use_asr_3A_out) {
        mask_bf_net->fftx2 = (wtk_complex_t *)wtk_malloc(
            sizeof(wtk_complex_t) * mask_bf_net->nbin * cfg->num_frame);
        mask_bf_net->synthesis_mem2 =
            (float *)malloc(sizeof(float) * (mask_bf_net->nbin - 1));
        mask_bf_net->out2 =
            (float *)malloc(sizeof(float) * (mask_bf_net->nbin - 1));
    }
    ...
    if (cfg->use_qnn) {
        ...
        if (cfg->qnn2_buf) {
            qtk_mem_t mem;
            qtk_mem_init(&mem, (unsigned char *)cfg->qnn2_buf->data,
                         cfg->qnn2_buf->pos);
    ...
    mask_bf_net->bff = NULL;
    if (cfg->use_two_pass) {
        mask_bf_net->bff = (float *)wtk_malloc(
            sizeof(float) * mask_bf_net->nbin * cfg->num_frame);
    }
    mask_bf_net->post_mask = NULL;
    if (cfg->use_post_mask) {
        mask_bf_net->post_mask = (float *)wtk_malloc(
            sizeof(float) * mask_bf_net->nbin * cfg->num_frame);
    }
...
void wtk_mask_bf_net_delete(wtk_mask_bf_net_t *mask_bf_net) {
    ...
    if (mask_bf_net->synthesis_mem2) {
        wtk_free(mask_bf_net->synthesis_mem2);
    }
    ...
    if (mask_bf_net->ekalman) {
        wtk_kalman_delete(mask_bf_net->ekalman);
    }
    if (mask_bf_net->covm) {
    ...
    if (mask_bf_net->mask_bf) {
        wtk_mask_bf_delete(mask_bf_net->mask_bf);
    }
    ...
    if (mask_bf_net->fftx2) {
        wtk_free(mask_bf_net->fftx2);
    }
    wtk_free(mask_bf_net->fft_raw);
    ...
    if (mask_bf_net->qmmse3) {
        wtk_qmmse_delete(mask_bf_net->qmmse3);
    }
    ...
    if (mask_bf_net->maskssl2) {
        wtk_maskssl2_delete(mask_bf_net->maskssl2);
    }
    if (mask_bf_net->ssl_mask) {
        wtk_free(mask_bf_net->ssl_mask);
    }

    wtk_free(mask_bf_net->out);
    if (mask_bf_net->out2) {
        wtk_free(mask_bf_net->out2);
    }
    ...
    if (mask_bf_net->bff) {
        wtk_free(mask_bf_net->bff);
    }
    if (mask_bf_net->post_mask) {
        wtk_free(mask_bf_net->post_mask);
    }
    ...
    if (mask_bf_net->cfg->use_qnn) {
        ...
        int outer_in_num = 1;
        int outer_out_num = 1;
        if (mask_bf_net->cfg->use_two_pass) {
            outer_in_num += 1;
        }
        if (mask_bf_net->cfg->use_edr_stage1_rt) {
            outer_in_num += 1;
        }
        if (mask_bf_net->cfg->use_post_mask) {
            outer_out_num += 1;
        }
...
void wtk_mask_bf_net_start(wtk_mask_bf_net_t *mask_bf_net) {
    ...
    if (mask_bf_net->mic_delay_samples > 0) {
        int i;
        for (i = 0; i < mask_bf_net->cfg->nmicchannel; ++i) {
            wtk_strbuf_push(mask_bf_net->mic[i], NULL,
                            mask_bf_net->mic_delay_samples * sizeof(float));
        }
    }
    if (mask_bf_net->sp_delay_samples > 0) {
        int i;
        for (i = 0; i < mask_bf_net->cfg->nspchannel; ++i) {
            wtk_strbuf_push(mask_bf_net->sp[i], NULL,
                            mask_bf_net->sp_delay_samples * sizeof(float));
        }
    }
}
void wtk_mask_bf_net_reset(wtk_mask_bf_net_t *mask_bf_net) {
    ...
    if (mask_bf_net->synthesis_mem2) {
        memset(mask_bf_net->synthesis_mem2, 0, sizeof(float) * (nbin - 1));
    }
    ...
    if (mask_bf_net->ekalman) {
        wtk_kalman_reset(mask_bf_net->ekalman);
    }
    if (mask_bf_net->covm) {
    ...
    if (mask_bf_net->mask_bf) {
        wtk_mask_bf_reset(mask_bf_net->mask_bf);
    }
    ...
    if (mask_bf_net->fftx2) {
        memset(mask_bf_net->fftx2, 0, sizeof(wtk_complex_t) * nbin * num_frame);
    }
    memset(mask_bf_net->fft_raw, 0, sizeof(wtk_complex_t) * nbin * num_frame);
    ...
    if (mask_bf_net->qmmse3) {
        wtk_qmmse_reset(mask_bf_net->qmmse3);
    }
    ...
    if (mask_bf_net->out2) {
        memset(mask_bf_net->out2, 0, sizeof(float) * (nbin - 1));
    }
    if (mask_bf_net->maskssl2) {
        wtk_maskssl2_reset(mask_bf_net->maskssl2);
    }
    if (mask_bf_net->ssl_mask) {
        memset(mask_bf_net->ssl_mask, 0, sizeof(float) * nbin);
    }
    ...
    if (mask_bf_net->bff) {
        memset(mask_bf_net->bff, 0, sizeof(float) * nbin * num_frame);
    }
    if (mask_bf_net->post_mask) {
        memset(mask_bf_net->post_mask, 0, sizeof(float) * nbin * num_frame);
    }

    for (i = 0; i < mask_bf_net->cfg->num_frame; i++) {
        mask_bf_net->sp_state[i] = 1;
    }
    ...
    mask_bf_net->raw_add_cnt = 5;
    mask_bf_net->trick_cnt = 0;
    mask_bf_net->trick1_cnt = 0;
    mask_bf_net->reset_model_cnt = 0;

    mask_bf_net->mic_delay_samples = 0;
    mask_bf_net->sp_delay_samples = 0;
}
...
void wtk_mask_bf_net_set_notify2(wtk_mask_bf_net_t *mask_bf_net, void *ths,
                                 wtk_mask_bf_net_notify_f2 notify) {
    mask_bf_net->notify2 = notify;
    mask_bf_net->ths2 = ths;
    if (!mask_bf_net->fftx2) {
        mask_bf_net->fftx2 =
            (wtk_complex_t *)malloc(sizeof(wtk_complex_t) * mask_bf_net->nbin);
        memset(mask_bf_net->fftx2, 0,
               sizeof(wtk_complex_t) * mask_bf_net->nbin);
    }
    if (!mask_bf_net->synthesis_mem2) {
        mask_bf_net->synthesis_mem2 =
            (float *)malloc(sizeof(float) * (mask_bf_net->nbin - 1));
        memset(mask_bf_net->synthesis_mem2, 0,
               sizeof(float) * (mask_bf_net->nbin - 1));
    }
    if (!mask_bf_net->out2) {
        mask_bf_net->out2 =
            (float *)malloc(sizeof(float) * (mask_bf_net->nbin - 1));
        memset(mask_bf_net->out2, 0, sizeof(float) * (mask_bf_net->nbin - 1));
    }
}

void wtk_mask_bf_net_set_ssl_notify(wtk_mask_bf_net_t *mask_bf_net, void *ths,
                                    wtk_mask_bf_net_notify_ssl_f notify) {
    mask_bf_net->notify_ssl = notify;
    mask_bf_net->ssl_ths = ths;
}
...
void wtk_mask_bf_net_on_ssl2(wtk_mask_bf_net_t *mask_bf_net,
    ...
    if (mask_bf_net->notify_ssl) {
        mask_bf_net->notify_ssl(mask_bf_net->ssl_ths, ts, te, nbest_extp,
                                nbest);
    }
}
...
static float wtk_mask_bf_net_sp_energy(float *p, int n) {
    float32x4_t sum_vec = vdupq_n_f32(0.0f);       // 初始化累加向量（4个float）
    float32x4_t scale_vec = vdupq_n_f32(32768.0f); // 缩放因子向量（4个32768.0）

    // ===== 第一部分：计算均值 f = sum(p[i] * 32768.0) / n =====
    for (int i = 0; i < n; i += 4) {
        // 1. 加载 4 个 p[i] 到 NEON 寄存器
        float32x4_t p_vec = vld1q_f32(&p[i]);

        // 2. 计算 p[i] * 32768.0
        float32x4_t scaled_vec = vmulq_f32(p_vec, scale_vec);

        // 3. 累加到 sum_vec
        sum_vec = vaddq_f32(sum_vec, scaled_vec);
    }

    // 4. 计算 f = sum(p[i] * 32768.0) / n
    // float f = vaddvq_f32(sum_vec) / n; // ARMv8 可直接用 vaddvq_f32 横向求和
    // 如果是 ARMv7，需手动求和：
    float f = (sum_vec[0] + sum_vec[1] + sum_vec[2] + sum_vec[3]) / n;

    // ===== 第二部分：计算方差 f2 = sum((p[i]*32768.0 - f)^2) / n =====
    float32x4_t f_vec = vdupq_n_f32(f);         // 广播 f 到 4 个 float
    float32x4_t sum_sq_vec = vdupq_n_f32(0.0f); // 初始化平方和累加器
    ...
    for (int i = 0; i < n; i += 4) {
        ...
        // 2. 计算 p[i] * 32768.0 - f
        float32x4_t scaled_vec = vmulq_f32(p_vec, scale_vec);
        float32x4_t diff_vec = vsubq_f32(scaled_vec, f_vec);
    ...
    // 4. 计算 f2 = sum((p[i]*32768.0 - f)^2) / n
    // float f2 = vaddvq_f32(sum_sq_vec) / n; // ARMv8 可直接用 vaddvq_f32
    // 如果是 ARMv7，需手动求和：
    float f2 =
        (sum_sq_vec[0] + sum_sq_vec[1] + sum_sq_vec[2] + sum_sq_vec[3]) / n;
...
static void _limiter(wtk_mask_bf_net_t *mask_bf_net, float *out, int fsize) {
    int max_out = mask_bf_net->cfg->max_out;
    float fv = max_out / 32768.0;
    float thresh = mask_bf_net->cfg->limiter_thresh;
...
void wtk_mask_bf_net_feed_qnn(wtk_mask_bf_net_t *mask_bf_net) {
    ...
    float *bff = mask_bf_net->bff;
    float *mask = mask_bf_net->mask;
    float *post_mask = mask_bf_net->post_mask;
    ...
    if (mask_bf_net->nv1_idx != 0) {
        ...
        for (i = outer_in_num; i < mask_bf_net->nv1_num_in; i++) {
            memcpy(
                mask_bf_net->nv1_input[i],
                mask_bf_net->nv1_cache[i - outer_in_num + outer_out_num],
                mask_bf_net->nv1_out_sizes[i - outer_in_num + outer_out_num]);
        }
    }
    memcpy(mask_bf_net->nv1_input[idx], err, mask_bf_net->nv1_in_sizes[idx]);

    idx++;
    if (mask_bf_net->cfg->use_two_pass) {
        memcpy(mask_bf_net->nv1_input[idx], bff,
               mask_bf_net->nv1_in_sizes[idx]);
        idx++;
    }

    if (mask_bf_net->cfg->use_edr_stage1_rt) {
        memcpy(mask_bf_net->nv1_input[idx], ee, mask_bf_net->nv1_in_sizes[idx]);
    }
    qtk_nn_vm_run(nv);
    ...
    idx = 0;
    memcpy(mask, mask_bf_net->nv1_output[idx], mask_bf_net->nv1_out_sizes[idx]);
    idx++;
    if (mask_bf_net->cfg->use_post_mask) {
        memcpy(post_mask, mask_bf_net->nv1_output[idx],
               mask_bf_net->nv1_out_sizes[idx]);
        idx++;
    }

    if (mask_bf_net->cfg->reset_model_cnt > 0) {
        if (mask_bf_net->trick1_cnt < 0) {
            mask_bf_net->reset_model_cnt--;
        } else {
            mask_bf_net->reset_model_cnt = mask_bf_net->cfg->reset_model_cnt;
        }
        if (mask_bf_net->reset_model_cnt < 0) {
            for (i = idx; i < nout; i++) {
                memset(mask_bf_net->nv1_output[i], 0, mask_bf_net->nv1_out_sizes[i]);
            }
            mask_bf_net->reset_model_cnt = mask_bf_net->cfg->reset_model_cnt;
        }
    }
...
void wtk_mask_bf_net_feed_onnx(wtk_mask_bf_net_t *mask_bf_net) {
    ...
    if (mask_bf_net->cfg->use_two_pass) {
        outer_in_num += 1;
        float *bf_data = qtk_nnrt_value_get_data(
            mask_bf_net->stage1_rt, mask_bf_net->stage1_inputs[idx]);
        memcpy(bf_data, bff, sizeof(float) * nbin * num_frame);
        idx++;
    }

    if (mask_bf_net->cfg->use_edr_stage1_rt) {
        outer_in_num += 1;
        float *ee_data = qtk_nnrt_value_get_data(
            mask_bf_net->stage1_rt, mask_bf_net->stage1_inputs[idx]);
...

    {
        ...
        qtk_nnrt_get_output(mask_bf_net->stage1_rt, &output_mask, idx);
        memcpy(mask,
        ...
        if (mask_bf_net->cfg->use_post_mask) {
            ...
            qtk_nnrt_get_output(mask_bf_net->stage1_rt, &output_post_mask, idx);
            memcpy(post_mask,
                   qtk_nnrt_value_get_data(mask_bf_net->stage1_rt,
                                           output_post_mask),
                   sizeof(float) * nbin * num_frame);
            qtk_nnrt_value_release(mask_bf_net->stage1_rt, output_post_mask);
        }
        qtk_nnrt_reset(mask_bf_net->stage1_rt);
    }
    // for (int k = 0; k < nbin; ++k) {
        ...
        if (mask_bf_net->cfg->use_echo_bf) {
            ...
            for (n = 0; n < num_frame; ++n, ffty += nbin) {
                ...
                if (mask_bf_net->sp_state[n] == 0) {
                    if (mask_bf_net->cfg->use_echo_filter_model) {
                        for (k = 0; k < nbin; ++k) {
                            for (i = 0; i < nbfchannel; ++i) {
                                fft[i][n * nbin + k].a =
...
#ifdef USE_NEON
void wtk_mask_bf_net_preemph(wtk_mask_bf_net_t *mask_bf_net,
                             wtk_complex_t **fft) {
    const int num_frame = mask_bf_net->cfg->num_frame;
    const int nbin = mask_bf_net->nbin;
    const int nmicchannel = mask_bf_net->cfg->nmicchannel;
    const int pre_clip_s = mask_bf_net->cfg->pre_clip_s;
    const int pre_clip_e = mask_bf_net->cfg->pre_clip_e;
    const float *pre_alpha = mask_bf_net->pre_alpha;
    int n, i;
    wtk_complex_t *a;
    ...
    for (n = 0; n < num_frame; ++n) {
        for (i = 0; i < nmicchannel; ++i) {
            a = fft[i];
            // 对频点维度（k）进行向量化，每次处理4个频点
            int k = pre_clip_s;
            for (; k + 3 < pre_clip_e; k += 4) {
                const int idx0 = k + n * nbin;
                const int idx1 = k + 1 + n * nbin;
                const int idx2 = k + 2 + n * nbin;
                const int idx3 = k + 3 + n * nbin;

                // 加载4个频点的 alpha
                float32x4_t alpha_vec = vld1q_f32(&pre_alpha[k]);

                // 加载4个频点的复数实部（.a）和虚部（.b）
                float32x4_t a_vec = {a[idx0].a, a[idx1].a, a[idx2].a,
                                     a[idx3].a};
                float32x4_t b_vec = {a[idx0].b, a[idx1].b, a[idx2].b,
                                     a[idx3].b};

                // 计算乘法
                a_vec = vmulq_f32(a_vec, alpha_vec);
                b_vec = vmulq_f32(b_vec, alpha_vec);

                // 存回结果
                a[idx0].a = vgetq_lane_f32(a_vec, 0);
                a[idx1].a = vgetq_lane_f32(a_vec, 1);
                a[idx2].a = vgetq_lane_f32(a_vec, 2);
                a[idx3].a = vgetq_lane_f32(a_vec, 3);

                a[idx0].b = vgetq_lane_f32(b_vec, 0);
                a[idx1].b = vgetq_lane_f32(b_vec, 1);
                a[idx2].b = vgetq_lane_f32(b_vec, 2);
                a[idx3].b = vgetq_lane_f32(b_vec, 3);
            }

            // 处理剩余频点（pre_clip_e % 4）
            for (; k < pre_clip_e; ++k) {
                const int idx = k + n * nbin;
                a[idx].a *= pre_alpha[k];
                a[idx].b *= pre_alpha[k];
            }
        }
...
void _phase_correction2(wtk_mask_bf_net_t *mask_bf_net) {
    ...
    int nbin = mask_bf_net->nbin;
    int nmicchannel = mask_bf_net->cfg->nmicchannel;
    int nmicchannel_4 = nmicchannel & ~3; // 向下对齐到4的倍数
    ...
    for (int k = 0; k < nbin; ++k) {
        if (vec[k][0].b == 0) {
            ...
            // 2. 广播标量到NEON寄存器
            float32x4_t real_vec = vdupq_n_f32(scaled_real);
            float32x4_t imag_vec = vdupq_n_f32(scaled_imag);

            // 3. NEON向量化处理（每次4个复数）
            int i = 0;
            for (; i < nmicchannel_4; i += 4) {
                // 加载4个复数（8个float）
                float32x4x2_t vec_data = vld2q_f32(&vec[k][i].a);

                // 复数乘法：(a+ib) * (c-id) = (ac+bd) + i(bc-ad)
                float32x4_t ac = vmulq_f32(vec_data.val[0], real_vec); // a*c
                float32x4_t bd = vmulq_f32(vec_data.val[1], imag_vec); // b*d
                float32x4_t real_part = vaddq_f32(ac, bd);             // ac+bd

                float32x4_t bc = vmulq_f32(vec_data.val[1], real_vec); // b*c
                float32x4_t ad = vmulq_f32(vec_data.val[0], imag_vec); // a*d
                float32x4_t imag_part = vsubq_f32(bc, ad);             // bc-ad

                // 存储结果
                float32x4x2_t result = {real_part, imag_part};
                vst2q_f32(&vec2[k][i].a, result);
            }

            // 4. 处理剩余元素（nmicchannel % 4）
            for (; i < nmicchannel; ++i) {
...
void wtk_mask_bf_net_feed_bf(wtk_mask_bf_net_t *mask_bf_net) {
    ...
    for (n = 0; n < num_frame; ++n, fftx += nbin, mask += nbin) {
        ...
        entropy = 1e5;
        if (mask_bf_net->cfg->use_freq_preemph && mask_bf_net->denoise_enable) {
            wtk_mask_bf_net_preemph(mask_bf_net, fft);
        }
        if (debug) {
            for (i = 0; i < nmicchannel; ++i) {
                for (k = 0; k < nbin; ++k) {
            ...
            for (k = 0; k + 3 < nbin; k += 4) {
                // 每次处理 4 个频点（k, k+1, k+2, k+3）
                for (i = 0; i < nmicchannel; ++i) {
                    // 计算输入数据的索引
                    int idx0 = k + n * nbin;
                    int idx1 = k + 1 + n * nbin;
                    ...
                    // 加载 4 个复数（8 个 float）到 NEON 寄存器
                    float32x4x2_t data = {
                        .val[0] = {fft[i][idx0].a, fft[i][idx1].a,
                                   fft[i][idx2].a, fft[i][idx3].a},
                        .val[1] = {fft[i][idx0].b, fft[i][idx1].b,
                                   fft[i][idx2].b, fft[i][idx3].b}};

                    // 转置：将数据从 [i][k] 布局转为 [k][i] 布局
                    for (j = 0; j < 4; ++j) {
                        fft_chn[k + j][i].a = data.val[0][j];
                        fft_chn[k + j][i].b = data.val[1][j];
                    }
                }
            }

            // 处理剩余的频点（nbin % 4）
            for (k = (nbin & ~3); k < nbin; ++k) {
                for (i = 0; i < nmicchannel; ++i) {
                    int idx = k + n * nbin;
                    fft_chn[k][i].a = fft[i][idx].a;
                    fft_chn[k][i].b = fft[i][idx].b;
        ...
        if (debug) {
            eta_mean =
                wtk_float_mean(eta + eta_clip_s, eta_clip_e - eta_clip_s);
        } else {
        ...
        if (mask_bf_net->bf_init_frame < mask_bf_net->cfg->bf_init_frame) {
            ...
            wtk_bf_update_ncov2(mask_bf_net->bf, noise_covar, nbin);
            for (k = 0; k < nbin; ++k) {
...
void wtk_mask_bf_net_feed_bf_v2(wtk_mask_bf_net_t *mask_bf_net) {
    ...
    for (n = 0; n < num_frame;
        ...
        if (mask_bf_net->bf_init_frame < mask_bf_net->cfg->bf_init_frame) {
            ...
            wtk_bf_update_ncov2(mask_bf_net->bf, covm_tmp->ncov, nbin);
            for (k = 0; k < nbin; ++k) {
                memcpy(mask_bf_net->bf->ovec[k], vec2[k],
                       nmicchannel * sizeof(wtk_complex_t));
                if (mask_bf_net->update_w == update_w_freq[k]) {
...
void wtk_mask_bf_net_feed_mask_bf(wtk_mask_bf_net_t *mask_bf_net) {
    int nbin = mask_bf_net->nbin;
    float *mask = mask_bf_net->mask;
    float *post_mask = mask_bf_net->post_mask;
    wtk_mask_bf_t *mask_bf = mask_bf_net->mask_bf;
    ...
    for (n = 0; n < num_frame; ++n) {
        if (mask_bf_net->cfg->use_freq_preemph && mask_bf_net->denoise_enable) {
            wtk_mask_bf_net_preemph(mask_bf_net, mask_bf_net->fft);
        }
        for (i = 0; i < nbfchannel; ++i) {
            fft[i] = mask_bf_net->fft[i] + n * nbin;
        }
        wtk_mask_bf_feed(mask_bf, fft, mask + n * nbin, NULL,
                         post_mask + n * nbin, mask_bf_net->sp_state[n]);
        memcpy(fftx + n * nbin, mask_bf->fftx, sizeof(wtk_complex_t) * nbin);
    }
}
...
void wtk_mask_bf_net_debug(wtk_mask_bf_net_t *mask_bf_net) {
    ...
    for (n = 0; n < num_frame; ++n, fftx += nbin) {
        ...
        if (mask_bf_net->cfg->use_limiter) {
            _limiter(mask_bf_net, out, fsize);
        } else {
            wtk_mask_bf_net_control_bs(mask_bf_net, out, fsize);
        }
        for (i = 0; i < fsize; ++i) {
...
void wtk_mask_bf_net_feed_model1(wtk_mask_bf_net_t *mask_bf_net) {
    ...
    if (mask_bf_net->cfg->use_bf_v2) {
        ...
        if (mask_bf_net->bf_init_frame > mask_bf_net->cfg->bf_init_frame) {
            ...
            a = fft[refchannel];
            qtk_vector_cpx_mag(a, err, nbin * num_frame);
        }
    ...
    if (mask_bf_net->cfg->use_two_pass) {
        ...
        int nbfchannel = mask_bf_net->cfg->nbfchannel;
        float ta, tb;
        if (mask_bf_net->cfg->use_mask_bf) {
            ww = mask_bf_net->mask_bf->w;
        } else {
            ww = mask_bf_net->bf->w;
        }
        for (n = 0; n < num_frame; ++n) {
        ...
        qtk_vector_cpx_mag(fft_tmp, bff, nbin * num_frame);
        wtk_float_scale(bff, model1_scale, bff, nbin * num_frame);
    }

    if (mask_bf_net->cfg->use_edr_stage1_rt) {
        a = fft_sp[0];
        qtk_vector_cpx_mag(a, ee, nbin * num_frame);
        wtk_float_scale(ee, model1_sp_scale, ee, nbin * num_frame);
    }
    wtk_float_scale(err, model1_scale, err, nbin * num_frame);
...
void wtk_mask_bf_net_dr_feed_onnx(wtk_mask_bf_net_t *mask_bf_net,
    ...
    for (i = outer_in_num, j = outer_out_num;
        ...
        if (mask_bf_net->stage2_rt->cfg->use_ss_ipu) {
            ...
            dst = qtk_nnrt_value_get_data(mask_bf_net->stage2_rt,
                                          mask_bf_net->stage2_inputs[i]);
            memcpy(dst, src,
                   qtk_nnrt_get_input_nbytes(mask_bf_net->stage2_rt, i));
            qtk_nnrt_value_release(mask_bf_net->stage2_rt, out_val);
...
void wtk_mask_bf_net_dr_feed_qnn(wtk_mask_bf_net_t *mask_bf_net,
    ...
    qtk_vector_cpx_mag(fftx, err, nbin * num_frame);
    wtk_float_scale(err, model2_scale, err, nbin * num_frame);
...
void wtk_mask_bf_net_edr_feed_qnn(wtk_mask_bf_net_t *mask_bf_net,
    ...
    qtk_vector_cpx_mag(fftx, err, nbin * num_frame);
    qtk_vector_cpx_mag(ffty, ee, nbin * num_frame);
    wtk_float_scale(err, model2_scale, err, nbin * num_frame);
    wtk_float_scale(ee, model2_sp_scale, ee, nbin * num_frame);
...
void wtk_mask_bf_net_feed_model2(wtk_mask_bf_net_t *mask_bf_net) {
    ...
    fftx = mask_bf_net->fftx;
    ffty = mask_bf_net->ffty;
    if (mask_bf_net->cfg->use_s1_mask_to_s2) {
        memcpy(model_mask, mask_bf_net->mask, sizeof(float) * nbin * num_frame);
        // printf("%f\n", wtk_float_abs_mean(mask_bf_net->mask, nbin));
        if (qmmse) {
            qmmse_gain = mask_bf_net->qmmse_gain;
        ...
        for (n = 0; n < num_frame;
            ...
            if (mask_mu) {
                ...
                memcpy(mask_mu + nbin * (n_mask_mu + num_frame - 2), mask_tmp,
                       sizeof(float) * nbin);
            }
        }
    } else if (mask_bf_net->cfg->use_gainnet2) {
        for (n = 0; n < num_frame; ++n, fftx += nbin, ffty += nbin) {
            for (k = 0; k < nbin; ++k) {
                fftx[k].a *= scale;
                fftx[k].b *= scale;
        ...
        if (mask_bf_net->cfg->use_mask_model) {
            ...
            // printf("%f\n", wtk_float_abs_mean(mask_bf_net->mask, nbin *
            // num_frame)); printf("%f\n", wtk_float_abs_mean(model_mask, nbin *
            // num_frame));
            if (qmmse) {
                qmmse_gain = mask_bf_net->qmmse_gain;
                for (k = 0; k < num_frame * nbin; ++k) {
    ...
    //                     mask_mu2[idx] =
    //                         mask_mu2[idx2] * alpha2 + mask_mu[idx] *
    //                         alpha2_1;
    //                     idx3 = idx2;
    //                 }
    //             }
...
void wtk_mask_bf_net_trick(wtk_mask_bf_net_t *mask_bf_net, wtk_complex_t *fftx,
                           int nbin, int sp_sil) {
    int trick_clip_s = mask_bf_net->cfg->trick_clip_s;
    int trick_clip_e = mask_bf_net->cfg->trick_clip_e;
    int trick_cnt = mask_bf_net->cfg->trick_cnt;
    float trick_thresh;
    float trick_scale = mask_bf_net->cfg->trick_scale;
    int k;
    float low_energy = 0;

    if (sp_sil) {
        trick_thresh = mask_bf_net->cfg->trick_thresh;
    } else {
        trick_thresh = mask_bf_net->cfg->trick_thresh_e;
    }

    low_energy = qtk_vector_cpx_mag_squared_sum(fftx + trick_clip_s,
                                                trick_clip_e - trick_clip_s);
    if (low_energy > trick_thresh) {
        mask_bf_net->trick_cnt = trick_cnt;
    } else {
        --mask_bf_net->trick_cnt;
    }
    if (mask_bf_net->trick_cnt < 0) {
        for (k = 0; k < nbin; ++k) {
            fftx[k].a *= trick_scale;
            fftx[k].b *= trick_scale;
        }
    }
}

void wtk_mask_bf_net_trick1(wtk_mask_bf_net_t *mask_bf_net, wtk_complex_t *fftx,
                            int nbin, int sp_sil) {
    int trick1_clip_s;
    int trick1_clip_e;
    int trick1_cnt;
    ...
    if (sp_sil) {
        trick1_clip_s = mask_bf_net->cfg->trick1_clip_s;
        trick1_clip_e = mask_bf_net->cfg->trick1_clip_e;
        trick1_cnt = mask_bf_net->cfg->trick1_cnt;
        trick1_thresh = mask_bf_net->cfg->trick1_thresh;
        trick1_scale = mask_bf_net->cfg->trick1_scale;
    } else {
        trick1_clip_s = mask_bf_net->cfg->trick1_clip_s_e;
        trick1_clip_e = mask_bf_net->cfg->trick1_clip_e_e;
        trick1_cnt = mask_bf_net->cfg->trick1_cnt_e;
        trick1_thresh = mask_bf_net->cfg->trick1_thresh_e;
        trick1_scale = mask_bf_net->cfg->trick1_scale_e;
    }

    low_energy = qtk_vector_cpx_mag_squared_sum(fftx + trick1_clip_s,
                                                trick1_clip_e - trick1_clip_s);
    
    if (low_energy > trick1_thresh) {
        mask_bf_net->trick1_cnt = trick1_cnt;
    } else {
        --mask_bf_net->trick1_cnt;
    }
    if (mask_bf_net->trick1_cnt < 0) {
        for (k = 0; k < nbin; ++k) {
            fftx[k].a *= trick1_scale;
            fftx[k].b *= trick1_scale;
        }
    } else if (mask_bf_net->trick1_cnt < trick1_cnt) {
        trick1_scale = (float)mask_bf_net->trick1_cnt / trick1_cnt;
        trick1_scale = powf(trick1_scale, 2.0);
        for (k = 0; k < nbin; ++k) {
            fftx[k].a *= trick1_scale;
            fftx[k].b *= trick1_scale;
        }
...
void wtk_mask_bf_net_feed_post_process(wtk_mask_bf_net_t *mask_bf_net) {
    ...
    wtk_complex_t *fftx2 = mask_bf_net->fftx2;
    float *synthesis_mem2 = mask_bf_net->synthesis_mem2;

    fftx = mask_bf_net->fftx;
    ffty = mask_bf_net->ffty;
    fft_raw = mask_bf_net->fft_raw;
    for (n = 0; n < num_frame;
         ++n, model_mask += nbin, fftx += nbin, ffty += nbin, fft_raw += nbin) {
        ...
        if (mask_bf_net->cfg->use_trick1) {
            wtk_mask_bf_net_trick1(mask_bf_net, fftx, nbin, mask_bf_net->sp_state[n]);
        }

        if (mask_bf_net->gc_cnt >= 0 && (mask_bf_net->denoise_enable ||
                                         mask_bf_net->cfg->use_denoise_agc)) {
            if (mask_bf_net->denoise_enable == 0 && mask_bf_net->sp_sil) {
                memcpy(fftx, fft_raw, sizeof(wtk_complex_t) * nbin);
            }
            if (mask_bf_net->qmmse2 && (mask_bf_net->denoise_enable != 0 ||
                                        mask_bf_net->agc_enable != 0)) {
            ...
            if (fftx2 && mask_bf_net->cfg->use_asr_3A_out == 0) {
                if (mask_bf_net->qmmse2) {
                    for (i = 0; i < nbin; ++i) {
                        fftx2[i].a =
                            fftx[i].a * 1.0 / mask_bf_net->qmmse2->out_agc_gain;
                        fftx2[i].b =
                            fftx[i].b * 1.0 / mask_bf_net->qmmse2->out_agc_gain;
                    }
                } else {
                    for (i = 0; i < nbin; ++i) {
                        fftx2[i].a = fftx[i].a;
        ...
        if (mask_bf_net->sp_sil) {
            if (mask_bf_net->cfg->use_raw_add && mask_bf_net->denoise_enable) {
                float raw_alpha = 0;
                float raw_alpha_1 = 1.0;
                // printf("%f\n", mask_bf_net->qmmse2->out_agc_gain);
                if (mask_bf_net->qmmse2) {
                    raw_alpha = 1.0 - mask_bf_net->qmmse2->pframe;
                }
                float sum_eng = 0;
                float sum_raw_eng = 0;
                float raw_scale = mask_bf_net->cfg->raw_scale;
                for (i = 0; i < nbin; ++i) {
                    sum_eng += (fftx[i].a * fftx[i].a + fftx[i].b * fftx[i].b);
                    sum_raw_eng +=
                        (mask_bf_net->fft[0][i].a * mask_bf_net->fft[0][i].a +
                         mask_bf_net->fft[0][i].b * mask_bf_net->fft[0][i].b);
                }
                if (sum_eng > sum_raw_eng * raw_scale) {
                    mask_bf_net->raw_add_cnt = 5;
                } else {
                    --mask_bf_net->raw_add_cnt;
                }
                if (mask_bf_net->raw_add_cnt <= 0) {
                    raw_alpha = 1.0;
                }
                raw_alpha = max(min(raw_alpha, 0.95), 0.05);
                raw_alpha_1 = 1.0 - raw_alpha;
                raw_alpha *= raw_scale;
                for (i = 0; i < nbin; ++i) {
                    fftx[i].a = fftx[i].a * raw_alpha_1 +
                                mask_bf_net->fft[0][i].a * raw_alpha;
                    fftx[i].b = fftx[i].b * raw_alpha_1 +
                                mask_bf_net->fft[0][i].b * raw_alpha;
                }
            }
        }
        if (mask_bf_net->sp_sil) {
            if (mask_bf_net->denoise_enable == 0 &&
                mask_bf_net->cfg->use_denoise_agc == 0) {
                memcpy(fftx, fft_raw, sizeof(wtk_complex_t) * nbin);
            }
            if (mask_bf_net->cfg->use_only_aec) {
                if (fftx2 && mask_bf_net->cfg->use_asr_3A_out == 0) {
                    memcpy(fftx2, fft_raw, sizeof(wtk_complex_t) * nbin);
                }
            }
        }
        if (fftx2 && mask_bf_net->cfg->use_asr_3A_out == 0) {
            if (mask_bf_net->qmmse3 && mask_bf_net->agc2_enable) {
                wtk_qmmse_denoise(mask_bf_net->qmmse3, fftx2);
                if (mask_bf_net->sp_sil) {
                    memcpy(fftx2, fft_raw, sizeof(wtk_complex_t) * nbin);
                    for (i = 0; i < nbin; ++i) {
                        fftx2[i].a *= mask_bf_net->qmmse3->out_agc_gain;
                        fftx2[i].b *= mask_bf_net->qmmse3->out_agc_gain;
                    }
                }
            }
        }

        if (mask_bf_net->cfg->use_asr_3A_out) {
            if (mask_bf_net->qmmse3) {
                wtk_qmmse_denoise(mask_bf_net->qmmse3, fftx2);
            }
        }

        if (mask_bf_net->cfg->use_trick) {
            wtk_mask_bf_net_trick(mask_bf_net, fftx, nbin, mask_bf_net->sp_state[n]);
        }
        ...
        if (mask_bf_net->maskssl2) {
            ...
            int nmicchannel = mask_bf_net->cfg->nmicchannel;
            for (k = 0; k < nbin; ++k) {
                mask_bf_net->ssl_mask[k] =
                    mask_bf_net->model_mask[k + nbin * n];
            }
            if (mask_bf_net->cfg->use_bf) {
                for (k = 0; k < nbin; ++k) {
                    mask_bf_net->ssl_mask[k] *= mask_bf_net->mask[k + nbin * n];
                }
            }
            for (i = 0; i < nmicchannel; ++i) {
                for (k = 0; k < nbin; ++k) {
            ...
            wtk_maskssl2_feed_fft(mask_bf_net->maskssl2, fft_chn,
                                  mask_bf_net->ssl_mask, mask_bf_net->mic_sil);
        }
        ...
        if (mask_bf_net->cfg->use_raw_highpass) {
            for (i = clip_e; i < nbin; ++i) {
                fftx[i].a = fft_raw[i].a;
                fftx[i].b = fft_raw[i].b;
            }
        } else {
            for (i = clip_e; i < nbin; ++i) {
                fftx[i].a = 0;
        ...
        if (fftx2) {
            if (mask_bf_net->cfg->use_pffft) {
                wtk_drft_istft2(rfft, rfft_in, synthesis_mem2, fftx2, out2,
                                wins, synthesis_window);
            } else {
                wtk_drft_istft(rfft, rfft_in, synthesis_mem2, fftx2, out2, wins,
                               synthesis_window);
            }
            for (i = 0; i < fsize; ++i) {
                out2[i] *= 32768.0;
            ...
            if (mask_bf_net->cfg->use_limiter) {
                _limiter(mask_bf_net, out2, fsize);
            }
            for (i = 0; i < fsize; ++i) {
        ...
        if (mask_bf_net->eq && mask_bf_net->denoise_enable) {
            wtk_equalizer_feed_float(mask_bf_net->eq, out, fsize);
        }
        if (mask_bf_net->cfg->use_limiter) {
            _limiter(mask_bf_net, out, fsize);
        ...
        if (fftx2) {
            if (mask_bf_net->notify2) {
                mask_bf_net->notify2(mask_bf_net->ths2, pv, pv2, fsize);
            }
        }
        if (mask_bf_net->cfg->use_asr_3A_out) {
            fftx2 += nbin;
...
void wtk_mask_bf_net_feed(wtk_mask_bf_net_t *mask_bf_net, short *data, int len,
    ...
    if (nspchannel > 0) {
        length = min(length, sp[0]->pos / sizeof(float));
    }
    while (length >= nfsize) {
        ++mask_bf_net->nframe;
        f_offset = (mask_bf_net->nframe - 1) * mask_bf_net->nbin;

        if (mask_bf_net->cfg->use_kalman) {
            float *d[16];
            float *x;
            for (i = 0; i < nmicchannel; ++i) {
                d[i] = (float *)mic[i]->data;
            }
            x = (float *)sp[0]->data;
            wtk_kalman_feed(mask_bf_net->ekalman, d, x);
            for (i = 0; i < nmicchannel; ++i) {
                d[i] = (float *)mic[i]->data;
                memcpy(d[i], mask_bf_net->ekalman->e_n[i],
                       sizeof(float) * fsize);
            }
        }
        ...
        memcpy(mask_bf_net->fft_raw, fft[0],
               sizeof(wtk_complex_t) * nbin * num_frame);
        ...
        if (nspchannel > 0 && mask_bf_net->echo_enable) {
            fv1 = (float *)(sp[0]->data);
            ...
            if (mask_bf_net->nframe == num_frame) {
                if (mask_bf_net->cfg->use_echo_bf) {
                    ...
                    if (mask_bf_net->cfg->use_gainnet2_2 ||
                        ...
                        if (mask_bf_net->cfg->use_mask_bf) {
                            wtk_mask_bf_net_feed_mask_bf(mask_bf_net);
                        } else if (mask_bf_net->cfg->use_bf_v2) {
...
void wtk_mask_bf_net_set_agc2enable(wtk_mask_bf_net_t *mask_bf_net,
                                    int enable) {
    mask_bf_net->agc2_enable = enable;
    if (mask_bf_net->qmmse3) {
        if (enable) {
            mask_bf_net->qmmse3->cfg->use_agc = 1;
        } else {
            mask_bf_net->qmmse3->cfg->use_agc = 0;
        }
    }
}
...
void wtk_mask_bf_net_set_delay(wtk_mask_bf_net_t *mask_bf_net, float mic_delay,
                               float sp_delay) {
    mask_bf_net->mic_delay_samples = floor(mic_delay * mask_bf_net->cfg->rate);
    mask_bf_net->sp_delay_samples = floor(sp_delay * mask_bf_net->cfg->rate);
}
