    ...
    for (n = 0; n < num_frame;
        ...
        if (mask_bf_net->bf_init_frame < mask_bf_net->cfg->bf_init_frame) {
            ...
            for (k = 0; k < nbin; ++k) {
                memcpy(mask_bf_net->bf->ovec[k], vec2[k],
                       nmicchannel * sizeof(wtk_complex_t));
                wtk_bf_update_ncov(mask_bf_net->bf, covm_tmp->ncov, k);
                if (mask_bf_net->update_w == update_w_freq[k]) {
                    wtk_bf_update_mvdr_w_f(mask_bf_net->bf, 0, k);
                }
                wtk_bf_output_fft_k(mask_bf_net->bf, fft_chn[k], fftx + k, k);
...
void wtk_mask_bf_net_feed_mask_bf(wtk_mask_bf_net_t *mask_bf_net) {
    ...
    wtk_mask_bf_feed(mask_bf, fft, mask, NULL, post_mask, mask_bf_net->sp_sil);
...
void wtk_mask_bf_net_feed_model1(wtk_mask_bf_net_t *mask_bf_net) {
    ...
    if (mask_bf_net->cfg->use_two_pass) {
        ...
        for (j = 0; j < nbin * num_frame; ++j) {
            bff[j] = sqrtf(fft_tmp[j].a * fft_tmp[j].a +
                           fft_tmp[j].b * fft_tmp[j].b);
            bff[j] *= model1_scale;
        }
    }
...
void wtk_mask_bf_net_trick(wtk_mask_bf_net_t *mask_bf_net, wtk_complex_t *fftx,
    ...
    float trick_thresh = mask_bf_net->cfg->trick_thresh;
    float trick_scale = mask_bf_net->cfg->trick_scale;
    int k;
    float low_energy = 0;

    for (k = trick_clip_s; k < trick_clip_e; ++k) {
        low_energy += (fftx[k].a * fftx[k].a + fftx[k].b * fftx[k].b);
    }
    ...
    for (n = 0; n < num_frame;
        ...
        if (mask_bf_net->sp_sil) {
            if (mask_bf_net->denoise_enable == 0) {
                memcpy(fftx, mask_bf_net->fft_raw + n * nbin,
                       sizeof(wtk_complex_t) * nbin);
            }
            if (mask_bf_net->cfg->use_only_aec) {
                if (fftx2) {
                    memcpy(fftx2, mask_bf_net->fft_raw + n * nbin,
                           sizeof(wtk_complex_t) * nbin);
                }
            }
        }
        ...
        if (mask_bf_net->cfg->use_trick) {
            wtk_mask_bf_net_trick(mask_bf_net, fftx, nbin);
        }
...
void wtk_mask_bf_net_feed(wtk_mask_bf_net_t *mask_bf_net, short *data, int len,
    ...
    while (length >= nfsize) {
        ...
        if (mask_bf_net->cfg->use_kalman) {
            ...
            for (i = 0; i < nmicchannel; ++i) {
                d[i] = (float *)mic[i]->data;
                memcpy(d[i], mask_bf_net->ekalman->e_n[i], sizeof(float) * fsize);
            }
        }
        ...
        memcpy(mask_bf_net->fft_raw, fft[0], sizeof(wtk_complex_t) * nbin * num_frame);
