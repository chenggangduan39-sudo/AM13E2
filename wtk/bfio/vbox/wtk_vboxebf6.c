...
static void _limiter(wtk_vboxebf6_t *vboxebf6, float *out, int fsize)
{
    int max_out = vboxebf6->cfg->max_out;
    float fv = max_out/32768.0;
    float thresh = vboxebf6->cfg->limiter_thresh;
...
void wtk_vboxebf6_feed_edra(wtk_vboxebf6_t *vboxebf6, wtk_complex_t **fft,
    ...
    if (vboxebf6->sp_sil == 0) {
        ...
        if (vboxebf6->cfg->use_frame_leak) {
            ...
            memmove(leak1, leak1 + idx * nbin, sizeof(float) * nbin);
            memmove(leak2, leak2 + idx * nbin, sizeof(float) * nbin);
        } else {
            for (j = 0; j < leak_frame; ++j) {
                fftytmp = leak_fft + j * nbin;
                fftxtmp = fftx;
...
void wtk_vboxebf6_feed(wtk_vboxebf6_t *vboxebf6, short *data, int len,
    ...
    while (length >= fsize) {
        ...
        if (vboxebf6->cfg->use_limiter) {
            _limiter(vboxebf6, out, fsize);
        } else {
