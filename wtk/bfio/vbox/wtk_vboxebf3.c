...
wtk_vboxebf3_t *wtk_vboxebf3_new(wtk_vboxebf3_cfg_t *cfg) {
    ...
    if (cfg->use_nlms) {
        ...
        vboxebf3->ekalman = wtk_kalman_new(&(cfg->echo_kalman));
    }

    vboxebf3->dd = NULL;
    vboxebf3->xx = NULL;
    if (cfg->use_kalman) {
        vboxebf3->dd = wtk_float_new_p2(cfg->nmicchannel, cfg->wins / 2);
        vboxebf3->xx = (float *)wtk_malloc(sizeof(float) * cfg->wins / 2);
    }
    ...
    vboxebf3->mask_bf = NULL;
    if (cfg->use_mask_bf) {
        vboxebf3->mask_bf = wtk_mask_bf_new(&(cfg->mask_bf));
    }
...
void wtk_vboxebf3_delete(wtk_vboxebf3_t *vboxebf3) {
    ...
    if (vboxebf3->synthesis_mem2) {
        wtk_free(vboxebf3->synthesis_mem2);
    }
    ...
    if (vboxebf3->mask_bf) {
        wtk_mask_bf_delete(vboxebf3->mask_bf);
    }
    ...
    if (vboxebf3->ekalman) {
        wtk_kalman_delete(vboxebf3->ekalman);
    }
    if (vboxebf3->dd) {
        wtk_float_delete_p2(vboxebf3->dd, vboxebf3->cfg->nmicchannel);
    }
    if (vboxebf3->xx) {
        wtk_free(vboxebf3->xx);
    }
...
void wtk_vboxebf3_reset(wtk_vboxebf3_t *vboxebf3) {
    ...
    if (vboxebf3->mask_bf) {
        wtk_mask_bf_reset(vboxebf3->mask_bf);
    }
    ...
    if (vboxebf3->ekalman) {
        wtk_kalman_reset(vboxebf3->ekalman);
    }
    if (vboxebf3->dd) {
        wtk_float_zero_p2(vboxebf3->dd, vboxebf3->cfg->nmicchannel,
                          vboxebf3->cfg->wins / 2);
    }
    if (vboxebf3->xx) {
        memset(vboxebf3->xx, 0, sizeof(float) * vboxebf3->cfg->wins / 2);
    }
...
void wtk_vboxebf3_set_notify2(wtk_vboxebf3_t *vboxebf3, void *ths,
                              wtk_vboxebf3_notify_f2 notify) {
    vboxebf3->notify2 = notify;
    vboxebf3->ths2 = ths;
    if (!vboxebf3->fftx2) {
        vboxebf3->fftx2 =
            (wtk_complex_t *)malloc(sizeof(wtk_complex_t) * vboxebf3->nbin);
        memset(vboxebf3->fftx2, 0, sizeof(wtk_complex_t) * vboxebf3->nbin);
    }
    if (!vboxebf3->synthesis_mem2) {
        vboxebf3->synthesis_mem2 =
            (float *)malloc(sizeof(float) * (vboxebf3->nbin - 1));
        memset(vboxebf3->synthesis_mem2, 0,
               sizeof(float) * (vboxebf3->nbin - 1));
    }
    if (!vboxebf3->out2) {
        vboxebf3->out2 = (float *)malloc(sizeof(float) * (vboxebf3->nbin - 1));
        memset(vboxebf3->out2, 0, sizeof(float) * (vboxebf3->nbin - 1));
    }
}
...
static void _limiter(wtk_vboxebf3_t *vboxebf3, float *out, int fsize)
{
    int max_out = vboxebf3->cfg->max_out;
    float fv = max_out/32768.0;
    float thresh = vboxebf3->cfg->limiter_thresh;
...
static void _qtk_vector_cpx_mag_squared(wtk_complex_t *a, float *d,
                                        int numSamples) {
    int i;
...
void wtk_vboxebf3_edra_feed(wtk_vboxebf3_t *vboxebf3, wtk_complex_t *fftx,
    ...
    //     memcpy(mask_mu, mask_mu + nbin, sizeof(float) * nbin * (n_mask_mu -
    //     1)); memcpy(mask_mu + nbin * (n_mask_mu - 1), vboxebf3->vdr->gf,
    //            sizeof(float) * nbin);
    ...
    if (vboxebf3->mask_mu) {
        ...
        const int end_frame = start_frame + 1 - 1;
        const size_t frame_size = nbin * sizeof(float);

        memmove(mask_mu, mask_mu + nbin, sizeof(float) * nbin * (n_mask_mu - 1));
...
void wtk_vboxebf3_feed_mask_bf(wtk_vboxebf3_t *vboxebf3) {
    int nbin = vboxebf3->nbin;
    float *mask = vboxebf3->vdr->gf;
    wtk_mask_bf_t *mask_bf = vboxebf3->mask_bf;
    wtk_complex_t **fft = vboxebf3->fft;
    wtk_complex_t *fftx = vboxebf3->fftx;
    float scale = vboxebf3->cfg->wins / 32768.0;
    float scale_inv = 32768.0 / vboxebf3->cfg->wins;
    int nbfchannel = vboxebf3->cfg->nbfchannel;
    int i, k;
    ...
    wtk_mask_bf_feed(mask_bf, fft, mask, NULL, NULL, vboxebf3->sp_sil);
...
void wtk_vboxebf3_feed_qmmse2(wtk_vboxebf3_t *vboxebf3, wtk_complex_t *fftx) {
    ...
    wtk_qmmse_set_sp_sil(qmmse2, vboxebf3->sp_sil);
...
void wtk_vboxebf3_feed(wtk_vboxebf3_t *vboxebf3, short *data, int len,
    ...
    wtk_complex_t *fftx2 = vboxebf3->fftx2;
    float *synthesis_mem2 = vboxebf3->synthesis_mem2;
    ...
    while (length >= fsize) {
        if (vboxebf3->cfg->use_kalman) {
            short *d[16];
            short *x;
            float **dd = vboxebf3->dd;
            float *xx = vboxebf3->xx;
            for (i = 0; i < nmicchannel; ++i) {
                d[i] = (short *)(mic[i]->data);
                for (j = 0; j < fsize; ++j) {
                    dd[i][j] = WTK_WAV_SHORT_TO_FLOAT(d[i][j]);
                }
            }
            x = (short *)(sp[0]->data);
            for (j = 0; j < fsize; ++j) {
                xx[j] = WTK_WAV_SHORT_TO_FLOAT(x[j]);
            }
            wtk_kalman_feed(vboxebf3->ekalman, dd, xx);
            for (i = 0; i < nmicchannel; ++i) {
                memcpy(dd[i], vboxebf3->ekalman->e_n[i], sizeof(float) * fsize);
                for (j = 0; j < fsize; ++j) {
                    d[i][j] = WTK_WAV_FLOAT_TO_SHORT(dd[i][j]);
                }
            }
        }
        ...
        if (vboxebf3->cfg->comp_filter) {
            wtk_complex_t tmp_a;
            wtk_complex_t **comp = vboxebf3->cfg->comp_filter;
            int k;
            for (i = 0; i < nmicchannel; ++i) {
                for (k = 0; k < nbin; ++k) {
                    tmp_a.a = fft[i][k].a;
        ...
        if (vboxebf3->cfg->use_mask_bf) {
            wtk_vboxebf3_feed_mask_bf(vboxebf3);
        } else {
        ...
        if (fftx2) {
            memcpy(fftx2, fftx, sizeof(wtk_complex_t) * nbin);
        }
        ...
        if (fftx2) {
            wtk_drft_frame_synthesis(rfft, rfft_in, synthesis_mem2, fftx2, out2,
        ...
        if (vboxebf3->cfg->use_limiter) {
            _limiter(vboxebf3, out, fsize);
        } else {
        ...
        if (fftx2) {
            if (vboxebf3->notify2) {
                vboxebf3->notify2(vboxebf3->ths2, pv, pv2, fsize);
            }
        }
    }
    if (is_end && length > 0) {
...
void wtk_vboxebf3_feed_mul_bf(wtk_vboxebf3_t *vboxebf3, wtk_complex_t **fft) {
    ...
    if (vboxebf3->sp_sil) {
        ...
        } else if (vboxebf3->cfg->use_mul_mask && vboxebf3->denoise_enable) {
            for (k = 1; k < nbin - 1; ++k) {
                gf = vboxebf3->vdr->gf[k];
                for (i = 0; i < nmicchannel; ++i) {
                    fft[i][k].a *= gf;
