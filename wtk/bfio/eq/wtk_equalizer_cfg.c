...
int wtk_equalizer_cfg_update(wtk_equalizer_cfg_t *cfg)
{
    ...
    float *tmp_rate = (float*)wtk_malloc(sizeof(float)*cfg->band_count);
    float *tmp_value = (float*)wtk_malloc(sizeof(float)*cfg->band_count);
    memcpy(tmp_rate,cfg->rate,sizeof(float)*cfg->band_count);
    memcpy(tmp_value,cfg->value,sizeof(float)*cfg->band_count);
    wtk_free(cfg->rate);
    wtk_free(cfg->value);
    ...
    cfg->rate = (float*)wtk_malloc(sizeof(float)*j);
    cfg->value = (float*)wtk_malloc(sizeof(float)*j);
    j = 0;
    for (i = 0; i < cfg->band_count; ++i) {
        if (tmp_value[i] != 0) {
            cfg->rate[j] = tmp_rate[i];
            cfg->value[j] = tmp_value[i];
            j++;
        }
    }
    cfg->band_count = j;
    // for (i = 0; i < cfg->band_count; ++i) {
    //     printf("rate:%f,value:%f\n",cfg->rate[i],cfg->value[i]);
    // }
    wtk_free(tmp_rate);
    wtk_free(tmp_value);
    return ret;
