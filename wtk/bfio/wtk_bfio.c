    ...
    for(j=0;j < wbf_cnt;++j){
        if(idx==j)
        {
        	if(!log[j])
        	{
                ...
                if (bfio->wbf_fn) {
                    char buf[1024];
                    sprintf(buf,"%s_wbf",bfio->wbf_fn);
        		    wtk_wavfile_open2(log[j],buf);
                } else {
...
static void wtk_bfio_on_qform(wtk_bfio_t *bfio, wtk_complex_t *qf_out,
    ...
    if(bfio->qform){
        ...
        if (bfio->qform->cfg->fft_scale != 1.0) {
            scale = bfio->qform->cfg->fft_scale * 0.98;
        }
    }else if(bfio->qform2){
        pad = bfio->qform2->pad;
    }else if(bfio->qform3){
            ...
            if(bfio->cfg->use_all_raw_audio){
                ...
                {   
                    ...
                    if(!raw_log)
                    {
                        ...
                        if (bfio->wbf_fn) {
                            char buf[1024];
                            sprintf(buf,"%s_wbf.0.wav",bfio->wbf_fn);
                            wtk_debug(" ==========> open %s\n", buf);
                            wtk_wavfile_open(raw_log,buf);
                        } else {
...
void wtk_bfio_set_wbf_file(wtk_bfio_t *bfio,char *wbf_fn)
{
    bfio->wbf_fn = wbf_fn;
}
