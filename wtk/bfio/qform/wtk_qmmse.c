...
wtk_qmmse_t* wtk_qmmse_new(wtk_qmmse_cfg_t *cfg)
{
    ...
    if(cfg->entropy_thresh!=-1 || cfg->agc_entropy_thresh!=-1 || cfg->echo_agc_entropy_thresh!=-1){
...
void wtk_qmmse_agc(wtk_qmmse_t *qmmse, float pframe, wtk_complex_t  *out, float *mask)
{
    ...
    if (qmmse->sp_sil) {
        ...
        agc_entropy_thresh = qmmse->cfg->agc_entropy_thresh;
        agc_entropy_cnt = qmmse->cfg->agc_entropy_cnt;
    } else {
        agc_pframe_thresh = qmmse->cfg->echo_agc_pframe_thresh;
        agc_entropy_thresh = qmmse->cfg->echo_agc_entropy_thresh;
        agc_entropy_cnt = qmmse->cfg->echo_agc_entropy_cnt;
    }
    ...
    if(qmmse->cfg->entropy_thresh!=-1 || qmmse->cfg->agc_entropy_thresh!=-1 || qmmse->cfg->echo_agc_entropy_thresh!=-1){
        entropy = wtk_qmmse_entropy(qmmse);
    }

    if (qmmse->cfg->agc_entropy_thresh!=-1 || qmmse->cfg->echo_agc_entropy_thresh!=-1) {
        if (entropy < agc_entropy_thresh) {
            qmmse->agc_entropy_frame = agc_entropy_cnt;
        } else {
            --qmmse->agc_entropy_frame;
        }
        if (qmmse->agc_entropy_frame <= 0) {
            mask_state = 0;
        }
    }
    ...
    if (pframe>.3f)  // pframe为当前帧的语音概率
    {
        ...
        if (qmmse->cfg->use_pre_loudness) {
            qmmse->loudness = (1.f-rate)*qmmse->loudness + (rate)*powf(max(0.001f*loudness, 0.01), -1.0f/qmmse->cfg->loudness_pow);
        } else {
        ...
        if (qmmse->init_max < qmmse->cfg->max_gain) // 计算动态增益上限
        {
    ...
    // 对计算出的增益进行限制
    if ((pframe>0.5f && (entropy < entropy_thresh || entropy_thresh == -1) && qmmse->nframe > qmmse->cfg->init_frame) || qmmse->nframe < qmmse->cfg->init_frame || target_gain < qmmse->agc_gain)
    {
        ...
        if (target_gain > qmmse->init_max && qmmse->nframe > qmmse->cfg->init_frame)  // 增益上限限制2
        {
