...
wtk_rtjoin2_t *wtk_rtjoin2_new(wtk_rtjoin2_cfg_t *cfg) {
    ...
    rtjoin2->qmmse = NULL;
    if (cfg->use_qmmse) {
        rtjoin2->qmmse = wtk_qmmse_new(&(cfg->qmmse));
    }
...
void wtk_rtjoin2_delete(wtk_rtjoin2_t *rtjoin2) {
    ...
    if (rtjoin2->qmmse) {
        wtk_qmmse_delete(rtjoin2->qmmse);
    }
    if (rtjoin2->gc) {
...
static void _limiter(wtk_rtjoin2_t *rtjoin2, float *out, int fsize)
{
    int max_out = rtjoin2->cfg->max_out;
    float fv = max_out/32768.0;
    float thresh = rtjoin2->cfg->limiter_thresh;
...
void wtk_rtjoin2_feed_avg_mix(wtk_rtjoin2_t *rtjoin2, wtk_complex_t **fft,
    ...
    int idx = 0;
    float max_value = 0;
    float change_thresh = rtjoin2->cfg->change_thresh;
    for (i = 0; i < nmicchannel; ++i) {
    ...
    if (weight[idx] > weight[rtjoin2->near_chn] * change_thresh) {
        rtjoin2->near_chn = idx;
    }
...
void wtk_rtjoin2_feed(wtk_rtjoin2_t *rtjoin2, short *data, int len,
    ...
    while (length >= slid_len) {
        ...
        if (rtjoin2->gc_cnt >= 0) {
            if (rtjoin2->qmmse) {
                wtk_qmmse_denoise(rtjoin2->qmmse, fftx);
            }
            if (rtjoin2->gc) {
        ...
        if (rtjoin2->cfg->out_scale != 1.0) {
            for (k = 0; k < nbin; ++k) {
                fftx[k].a *= rtjoin2->cfg->out_scale;
                fftx[k].b *= rtjoin2->cfg->out_scale;
            }
        }
        ...
        if (rtjoin2->cfg->use_mul_out) {
            ...
            if (rtjoin2->cfg->use_limiter) {
                _limiter(rtjoin2, out, fsize);
            } else if (rtjoin2->cfg->use_control_bs) {
