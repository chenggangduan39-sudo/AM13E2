...
int wtk_beamnet3_cfg_clean(wtk_beamnet3_cfg_t *cfg) {
	...
	if(cfg->region_theta){
		wtk_free(cfg->region_theta);
	}
	if(cfg->region_map){
		wtk_free(cfg->region_map);
	}
...
int wtk_beamnet3_cfg_update_local(wtk_beamnet3_cfg_t *cfg, wtk_local_cfg_t *m) {
	...
	wtk_local_cfg_update_cfg_b(lc,cfg,need_theta_calibration,v);
	...
	a=wtk_local_cfg_find_array_s(lc,"region_theta");
	if(a)
	{
		cfg->region_theta=(float*)wtk_malloc(sizeof(float)*a->nslot);
		for(i=0;i<a->nslot;++i)
		{
			v=((wtk_string_t**)a->slot)[i];
			cfg->region_theta[i]=wtk_str_atof(v->data,v->len);
		}
		cfg->map_n = a->nslot;
	}

	a=wtk_local_cfg_find_array_s(lc,"region_map");
	if(a)
	{
		if(a->nslot != cfg-> map_n){
			return - 1;
		}
		cfg->region_map=(float*)wtk_malloc(sizeof(float)*a->nslot);
		for(i=0;i<a->nslot;++i)
		{
			v=((wtk_string_t**)a->slot)[i];
			cfg->region_map[i]=wtk_str_atof(v->data,v->len);
		}
	}
