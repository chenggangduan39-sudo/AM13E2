...
void wtk_beam_bf_net_cfg_update_comp_filter(wtk_beam_bf_net_cfg_t *cfg)
{
	int i, k;
...
int wtk_beam_bf_net_cfg_init(wtk_beam_bf_net_cfg_t *cfg) {
    ...
    cfg->mapping_theta = NULL;
    cfg->real_theta = NULL;
    cfg->n_mapping_theta = 0;
...
int wtk_beam_bf_net_cfg_clean(wtk_beam_bf_net_cfg_t *cfg) {
    ...
    if (cfg->mapping_theta) {
        wtk_free(cfg->mapping_theta);
    }
    if (cfg->real_theta) {
        wtk_free(cfg->real_theta);
    }
	if(cfg->comp_filter)
	{
		for(i=0;i<cfg->nmic;++i)
...
int wtk_beam_bf_net_cfg_update_local(wtk_beam_bf_net_cfg_t *cfg,
	...
	a=wtk_local_cfg_find_array_s(m,"compsamp");
	if(a)
	{
	...
	a=wtk_local_cfg_find_array_s(m,"mapping_theta");
	if(a)
	{
		cfg->mapping_theta=(float*)wtk_malloc(sizeof(float)*a->nslot);
        cfg->n_mapping_theta = a->nslot;
		for(i=0;i<a->nslot;++i)
		{
			v=((wtk_string_t**)a->slot)[i];
			cfg->mapping_theta[i]=wtk_str_atof(v->data,v->len);
        }
    }

	a=wtk_local_cfg_find_array_s(m,"real_theta");
	if(a)
	{
		cfg->real_theta=(float*)wtk_malloc(sizeof(float)*a->nslot);
        if (cfg->n_mapping_theta != a->nslot) {
            wtk_debug("error: n_mapping_theta=%d != n_real_theta=%d\n", cfg->n_mapping_theta, a->nslot);
            ret = -1;
            goto end;
        }
		for(i=0;i<a->nslot;++i)
		{
			v=((wtk_string_t**)a->slot)[i];
			cfg->real_theta[i]=wtk_str_atof(v->data,v->len);
        }
    }
...
void wtk_beam_bf_net_cfg_set(wtk_beam_bf_net_cfg_t *cfg) {
	...
	if(cfg->compsamp){
		wtk_beam_bf_net_cfg_update_comp_filter(cfg);
	}
    if (cfg->mapping_theta && cfg->real_theta) {
        cfg->use_theta_mapping = 1;
    }
    if (cfg->use_line) {
    ...
    // printf("%f %f %d\n", cfg->min_theta, cfg->max_theta, cfg->n_theta);
