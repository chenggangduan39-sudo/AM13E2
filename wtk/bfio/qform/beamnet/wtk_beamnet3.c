...
void wtk_beamnet3_compute_channel_covariance2(wtk_beamnet3_t *beamnet3, wtk_complex_t **fft, wtk_complex_t *covar_mat, int norm)
{
    int nmicchannel = beamnet3->cfg->nmicchannel;
    int nbin = beamnet3->nbin;
    int i, j, k;
    ...
    wtk_complex_t **input_norm = beamnet3->input_norm;
    int idx;
    wtk_complex_t *cpx = covar_mat,cpx1,cpx2;
        ...
        for(k=0;k<nbin;++k){
            ...
            for(i=1;i<nmicchannel;++i){
                for(j=0;j<i;++j,cpx++){
                    cpx1 =  input_norm[i][k];
                    cpx2 =  input_norm[j][k];
                    cpx->a = cpx1.a * cpx2.a + cpx1.b * cpx2.b;
                    cpx->b = cpx1.b * cpx2.a - cpx1.a * cpx2.b;
                    idx++;
                }
            }
        }
...
wtk_beamnet3_t *wtk_beamnet3_new(wtk_beamnet3_cfg_t *cfg) {
    ...
    beamnet3->mic_covar = wtk_complex_new_p2_2(cfg->out_channels, beamnet3->nbin);
    ...
    if(cfg->sep_feature_type == 5){
        ...
        //     beamnet3->ideal_phase_covars[i] = wtk_complex_new_p2(beamnet3->nbin, cfg->out_channels);
        // }
         beamnet3->ideal_phase_covars = wtk_malloc(sizeof(wtk_complex_t*) * cfg->total_regions*cfg->out_channels*beamnet3->nbin);
    ...
    beamnet3->gf=(float *)wtk_calloc(sizeof(float),beamnet3->nbin);
    ...
    if(cfg->sep_feature_type == 5){
        ...
        beamnet3->beamformer_x = (float *)wtk_malloc(sizeof(float)*beamnet3->nbin * 4 * beamnet3->cfg->nmic);
    }
...
void wtk_beamnet3_delete(wtk_beamnet3_t *beamnet3) {
    ...
    wtk_complex_delete_p2_2(beamnet3->mic_covar);
    ...
    if(beamnet3->cfg->sep_feature_type ==5){
        ...
        for(i = 0; i < beamnet3->cfg->total_regions; i++){
            ...
            //wtk_complex_delete_p2(beamnet3->ideal_phase_covars[i],beamnet3->nbin);
...
void wtk_beamnet3_start(wtk_beamnet3_t *beamnet3, float theta, float phi) {
    if(beamnet3->cfg->sep_feature_type == 5){
        ...
        for(i = 0; i < beamnet3->cfg->total_regions; ++i){
            ...
            wtk_beamnet3_compute_channel_covariance2(beamnet3, beamnet3->ideal_phase_shifts[i], beamnet3->ideal_phase_covars + i * beamnet3->nbin * beamnet3->cfg->out_channels, beamnet3->cfg->norm_channel);
            //complex_dump(beamnet3->ideal_phase_shifts[i][0], beamnet3->nbin);
...
void wtk_beamnet3_compute_feature(wtk_beamnet3_t * beamnet3, wtk_complex_t **fft, wtk_complex_t **fft_sp, float **feature, int theta, int feature_type)
{
    ...
    if(feature_type == 5){
        ...
        wtk_complex_t *mic = mic_covar[i];
        for(i=0;i<out_channels;++i){
            for(k=0;k<nbin;++k,mic++){
                tmp = sqrtf(mic->a * mic->a + mic->b * mic->b) + eps;
                mic->a /= tmp;
                mic->b /= tmp;
            }
        }
        ...
        for(i = 0; i < beamnet3->cfg->total_regions; i++) {
            ...
            for(k = 0; k < beamnet3->nbin; k++) {
                cpx.a = cpx.b = 0.0;
                for(j = 0; j < out_channels; j++,ipc++){
                    cpx2 = mic_covar[j][k];
                    cpx.a += cpx2.a * ipc->a - cpx2.b * -ipc->b;
                    cpx.b += cpx2.a * -ipc->b + cpx2.b * ipc->a;
                }
...
void wtk_beamnet3_feed_model(wtk_beamnet3_t *beamnet3, wtk_complex_t **fft, wtk_complex_t **fft_sp, float **feature)
{
    ...
    if(beamnet3->cfg->use_onnx){
        ...
        if(beamnet3->cfg->bf_feature_type > 1){
            ...
            memcpy(beamnet3->beamformer_x+nmicchannel*beamnet3->nbin, beamnet3->speech_est_imag,beamnet3->seperator_out_len[1]*sizeof(float));
            memcpy(beamnet3->beamformer_x+2*nmicchannel*beamnet3->nbin, beamnet3->noise_est_real,beamnet3->seperator_out_len[2]*sizeof(float));
            memcpy(beamnet3->beamformer_x+3*nmicchannel*beamnet3->nbin, beamnet3->noise_est_imag,beamnet3->seperator_out_len[3]*sizeof(float));
#endif
        }else{
...
int wtk_beamnet3_set_query_region(wtk_beamnet3_t *beamnet3,float theta1, float theta2){
    ...
    if(beamnet3->cfg->sep_feature_type == 5 && beamnet3->cfg->query_region){
        int i;
        if(beamnet3->cfg->need_theta_calibration){
            if(beamnet3->cfg->region_map){
                for(i = 0; i < beamnet3->cfg->map_n;i++){
                    if(fabs(theta1 - beamnet3->cfg->region_theta[i]) < 1e-4){
                        theta1 = beamnet3->cfg->region_map[i];
                        ret++;
                    }
                    if(fabs(theta2 -  beamnet3->cfg->region_theta[i]) < 1e-4){
                        theta2 = beamnet3->cfg->region_map[i];
                        ret++;
                    }
                }
            }else{
