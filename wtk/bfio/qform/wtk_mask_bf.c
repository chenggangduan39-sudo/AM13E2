...
wtk_mask_bf_t *wtk_mask_bf_new(wtk_mask_bf_cfg_t *cfg) {
    ...
    mask_bf->fft_tmp = (wtk_complex_t *)wtk_malloc(
        ...
        max(cfg->nbin,
            cfg->channelx2 * 2)); //// max(nbin, channel*channel*2)
    mask_bf->fft_tmp2 = (wtk_dcomplex_t *)wtk_malloc(
...
void wtk_mask_bf_reset(wtk_mask_bf_t *mask_bf) {
    ...
    memset(mask_bf->fft_tmp, 0,
           sizeof(wtk_complex_t) * max(nbin, channelx2 * 2));
    memset(mask_bf->fft_tmp2, 0,
           sizeof(wtk_dcomplex_t) * max(nbin, channelx2 * 2));
...
void wtk_mask_bf_set_notify(wtk_mask_bf_t *mask_bf, void *ths,
...
void wtk_mask_bf_update(wtk_mask_bf_t *mask_bf, wtk_complex_t **fft,
                        int sp_sil) {
    int channel = mask_bf->cfg->channel;
...
void wtk_mask_bf_feed_covm(wtk_mask_bf_t *mask_bf, wtk_complex_t **fft,
                           int sp_sil) {
    wtk_complex_t *covar;
    wtk_complex_t **Rss;
    wtk_complex_t **Rnn;
...
void wtk_mask_bf_update_mvdr_w_f(wtk_mask_bf_t *mask_bf, int k) {
    ...
    float reg = mask_bf->cfg->reg;
    float eps = mask_bf->cfg->eps;
    int i;
    float fa, f;
    ...
    if (mask_bf->cfg->use_tik_reg) {
        float epsilon = 0;
        for (i = 0; i < channel; ++i) {
            epsilon += Rnn_[i * channel + i].a;
        }
        epsilon *= reg;
        for (i = 0; i < channel; ++i) {
            Rnn_[i * channel + i].a += epsilon + eps;
        }
    } else {
        for (i = 0; i < channel; ++i) {
...
void wtk_mask_bf_update_mvdr_w(wtk_mask_bf_t *mask_bf, int k) {
    int channel = mask_bf->cfg->channel;
    int channelx2 = mask_bf->cfg->channelx2;
    float eye = mask_bf->cfg->eye;
    float reg = mask_bf->cfg->reg;
    ...
    float fa, f;
    wtk_complex_t *w = mask_bf->w[k];
    wtk_dcomplex_t *fft_tmp2 = mask_bf->fft_tmp2;
    wtk_complex_t *a;
    wtk_complex_t *Rnn_norm, *vec;
    wtk_complex_t *Rnn_ = mask_bf->Rnn_;
    ...
    //     }
    // }
    wtk_complex_guass_elimination_p1(Rnn_, vec, fft_tmp2, channel, w);
    fa = 0;
    a = w;
    for (i = 0; i < channel; ++i, ++vec, ++a) {
        fa += vec->a * a->a + vec->b * a->b;
...
void wtk_mask_bf_feed_bf(wtk_mask_bf_t *mask_bf, wtk_complex_t **fft) {
    ...
    for (k = 0; k < nbin; ++k) {
        ...
        if (mask_bf->update_w == update_w_freq[k]) {
            wtk_mask_bf_update_mvdr_w(mask_bf, k);
        }
 ...
 */
void wtk_mask_bf_feed(wtk_mask_bf_t *mask_bf, wtk_complex_t **fft,
                      float *speech_mask, float *noise_mask, float *post_mask,
                      int sp_sil) {
    float post_clip;
