...
static void qtk_ahs_equalizer2_on(qtk_ahs_t *a, short *data, int len) {
        wtk_strbuf_push(a->sp,(char*)data,sizeof(short)*len);

        if(a->notify){
            a->notify(a->upval, data, len);
        }
}
...
qtk_ahs_t *qtk_ahs_new(qtk_ahs_cfg_t *cfg) {
    ...
    if(cfg->type == 3){
        ...
        a->km_v2 = qtk_kalman2_new(&cfg->km_v2);
        a->is_loop = 1;
        // if(cfg->km_v2.wb_buf){
        //     a->is_loop = 1;
        // }
    ...
    if(cfg->use_kmv2_mdl){
        a->rnn_state = init_RNNState();
        a->pst = init_pitchstate();
        a->mic_state = init_SignalState();
        a->kalman_out_state = init_SignalState();
        a->echo_est_state = init_SignalState();
    }
    a->wiener = NULL;
    if(cfg->use_mask){
	...
	if(cfg->use_eq2)
	{
		a->eq2 = wtk_equalizer2_new(&(cfg->eq2));
        wtk_equalizer2_set_notify(a->eq2, a,
                              (wtk_equalizer2_notify_f)qtk_ahs_equalizer2_on);
        wtk_equalizer2_start(a->eq2);
	}    
    a->eq_gain = NULL;
    // if(cfg->use_eq2){
...
void qtk_ahs_set_loop(qtk_ahs_t *a, int is_loop){
    a->is_loop = is_loop;
    if(is_loop == 1){
        int length = a->sp->pos - a->delay;
        int L;
        if(a->km_v2){
            L = a->km_v2->L;
        }
        if(length >= L){
            ...
            wtk_strbuf_reset(a->sp);
        }
    }
}

void qtk_ahs_set_sp_buf(qtk_ahs_t *a, char *buf, int len){
    int i;
    short *pv = (short *)buf;
    float fpv;
    ...
    wtk_strbuf_reset(sp);
    wtk_strbuf_push(sp, NULL, a->delay * sizeof(float));
    for(i = 0; i < len/sizeof(short); i++){
        ...
        //wtk_strbuf_push(a->sp, (char *)&pv1, sizeof(short));
        wtk_strbuf_push_float(sp, &fpv, 1);
    }
}
...
void qtk_ahs_delete(qtk_ahs_t *a) {
    ...
    if(a->km_v2){
        qtk_kalman2_delete(a->km_v2);
    }

    if(a->cfg->use_kmv2_mdl){
        wtk_free(a->mic_state);
        wtk_free(a->kalman_out_state);
        wtk_free(a->echo_est_state);
        wtk_free(a->pst);
        delete_RNNState(a->rnn_state);
        wtk_free(a->rnn_state);
        denoise_clean();
        yin_clean();
    }
    ...
    for (i = 0; i < a->cfg->chunk_sz; i++) {
        ...
        if(a->km_v2){
            ...
            for (j = 0; j < a->cfg->hop_sz; ++j) {
                ...
                wtk_strbuf_push_float(a->sp, &pvx, 1);
            }
        }else{
            for (j = 0; j < a->cfg->hop_sz; ++j) {
                //wtk_debug("%d %f\n",j,fv1[j]);
...
void qtk_ahs_feed_nnrt(qtk_ahs_t *a, float *yt, float *rt) {
    int i;
    int nbin = a->cfg->window_sz / 2 + 1;
    float *dst = a->x;
    float *y_re_ptr = dst;
    ...
    wtk_rfft_process_fft(a->fft, a->F, yt);
    ...
    wtk_rfft_process_fft(a->fft, a->F, rt);
    for (i = 0; i < nbin; i++) {
        cpx = wtk_rfft_get_value(a->F, a->cfg->window_sz, i);
        R_ptr[i] = sqrtf(cpx.a * cpx.a + cpx.b * cpx.b);
    }
...
static void _ahs_limiter(float* input_signal, int length) {
    for (int i = 0; i < length; i++) {
        ...
        if (-0.9 <= x && x <= 0.9) {
            ...
            // 正向压缩
            input_signal[i] = 0.9 + 0.1 * (1 - exp(-(x - 0.9)));
        } else {
            // 负向压缩
            input_signal[i] = -0.9 - 0.1 * (1 - exp(-(-x - 0.9)));
        }
...
void qtk_ahs_feed_kalman2(qtk_ahs_t *a) {
    wtk_strbuf_t *mic = a->mic;
    wtk_strbuf_t *sp = a->sp;
    int length =  mic->pos / sizeof(short);
    int fsize = a->km_v2->L;//a->cfg->window_sz / 2;
    int i;
    short *yt,*rt;
    short *pv = a->wav_out;
    float pre_scale = a->cfg->pre_scale;
    ...
    while (length >= fsize) {
        for (i = 0; i < fsize; i++) {
            pv1 = yt[i] / 32768.0f * pre_scale;
            // if (pv1 > 0.9999) {
            //     a->yt_pad_tmp[i] = 0.9999;
            // } else if (pv1 <= -1.0) {
            //     a->yt_pad_tmp[i] = -1.0;
            // }else{
            a->yt_pad_tmp[i] = pv1;
           // }
            a->rt_pad_tmp[i] = rt[i]/32768.0f;
        }

        qtk_kalman2_feed(a->km_v2, a->rt_pad_tmp, a->yt_pad_tmp);
        qtk_kalman2_update(a->km_v2);
        wtk_strbuf_pop(sp, NULL, fsize * sizeof(short));
        wtk_strbuf_pop(mic, NULL, fsize * sizeof(short));
        length = mic->pos / sizeof(short);
        if(a->is_loop){
            float *km_out = a->km_v2->e;
            if(a->nnrt){
                if (!a->start) {
                    for (i = 0; i < fsize; i++) {
                        a->yt_pad_tmp[a->pos + i] = km_out[i];
                    }

                    for (i = 0; i < fsize; i++) {
                        a->frame_rt[fsize - (i + 1)] = a->rt_pad_tmp[i];
                        a->frame_yt[fsize - (i + 1)] = a->yt_pad_tmp[i];
                    }
                    memcpy(a->frame_yt + fsize, km_out, sizeof(float) * fsize);
                    memcpy(a->frame_rt + fsize, a->rt_pad_tmp, sizeof(float) * fsize);
                    a->start = 1;
                }else{
                    memcpy(a->frame_yt + fsize, km_out, sizeof(float) * fsize);
                    memcpy(a->frame_rt + fsize, a->rt_pad_tmp, sizeof(float) * fsize);
                ...
                for (i = 0; i < a->cfg->window_sz; i++) {
                    a->frame_yt_tmp[i] = a->cfg->window[i] * a->frame_yt[i];
                    a->frame_rt_tmp[i] = a->cfg->window[i] * a->frame_rt[i]/a->cfg->pre_scale;
                }
                qtk_ahs_feed_nnrt(a, a->frame_yt_tmp, a->frame_rt_tmp);

                memmove(a->frame_yt, a->frame_yt + fsize, sizeof(float) * fsize);
                memmove(a->frame_rt, a->frame_rt + fsize, sizeof(float) * fsize);

            }else if(a->cfg->use_kmv2_mdl){
                rnnoise_process_frame(a->rnn_state,a->pst, 
                    km_out,a->mic_state,
                    a->rt_pad_tmp,a->echo_est_state, km_out);

                if(a->cfg->use_drc){
                    wtk_drc_feed_float(a->drc,km_out,1,fsize,0);
                for(i = 0; i < fsize; i++){
                    km_out[i] = a->drc->output.samples[i].L;
                }
                }else{
                    _ahs_limiter(km_out,fsize);
                }

                // if(a->eq){
                //     wtk_equalizer_feed_float(a->eq,km_out,fsize);
                // }
                for(i = 0; i < fsize; i++){
                    pv[i] = floorf(km_out[i] * 32768.0f + 0.5);
                }
                //wtk_strbuf_push_float(a->sp, km_out, fsize);

                if(a->eq){
                    wtk_equalizer_feed(a->eq,pv,fsize);
                }else if(a->eq2){
                    wtk_equalizer2_feed(a->eq2,pv,fsize,0);
                }

                if(!a->eq2){
                    wtk_strbuf_push(a->sp,(char*)pv,sizeof(short)*fsize);

                    if(a->notify){
                        a->notify(a->upval, pv, fsize);
                    }
                }
                ...
                for(i = 0; i < fsize; i++){
                    ...
                    // }
                    //wtk_strbuf_push(a->sp, (char *)&pv1, sizeof(short));
                    //wtk_strbuf_push_float(a->sp, km_out + i, 1);
                }
                wtk_strbuf_push(a->sp, (char *)pv, sizeof(short)*fsize);
                if(a->notify){
                    a->notify(a->upval, pv, fsize);
...
int qtk_ahs_kmv2_Wb_write(qtk_ahs_t *a, FILE *fp){
    if(!a->km_v2){
        return -1;
    }
    if(!fp){
        fp = fopen(a->km_v2->cfg->wb_fn,"wb");
    }

    if(fp){
        wtk_debug("qtk_ahs_kmv2_Wb_write\n");
        fwrite(a->km_v2->W_b, sizeof(wtk_complex_t), a->km_v2->nbin * a->km_v2->cfg->B,fp);
        fwrite(a->km_v2->K_b, sizeof(wtk_complex_t), a->km_v2->nbin * a->km_v2->cfg->B,fp);
        fwrite(a->km_v2->Phi_ss, sizeof(float), a->km_v2->nbin,fp);
        fwrite(a->km_v2->P_b, sizeof(float), a->km_v2->nbin * a->km_v2->cfg->B,fp);
        return 0;
    }else{
        return -1;
    }
...
int qtk_ahs_kmv2_Wb_read(qtk_ahs_t *a, FILE *fp){
    if(!a->km_v2){
        return -1;
    }
    if(!fp){
        fp = fopen(a->km_v2->cfg->wb_fn,"rb");
    }
    if(fp){
        wtk_debug("qtk_ahs_kmv2_Wb_read\n");
        a->is_loop = 1;
        fread(a->km_v2->W_b, sizeof(wtk_complex_t), a->km_v2->nbin * a->km_v2->cfg->B,fp);
        fread(a->km_v2->K_b, sizeof(wtk_complex_t), a->km_v2->nbin * a->km_v2->cfg->B,fp);
        fread(a->km_v2->Phi_ss, sizeof(float), a->km_v2->nbin,fp);
        fread(a->km_v2->P_b, sizeof(float), a->km_v2->nbin * a->km_v2->cfg->B,fp);
        return 0;
    }else{
        return -1;
    }
