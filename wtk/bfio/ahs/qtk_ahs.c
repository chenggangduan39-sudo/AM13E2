...
void qtk_ahs_feed_kalman2(qtk_ahs_t *a) {
    ...
    int i;
    short *yt = (short *)mic->data;
    float *rt = (float *)sp->data;
    short *pv = a->wav_out;
    float pre_scale = a->cfg->pre_scale;
    float pv1;
    //short pv2;
    ...
    while (length >= fsize) {
        for (i = 0; i < fsize; i++) {
            a->yt_pad_tmp[i] = yt[i] / 32768.0f;
            a->rt_pad_tmp[i] = rt[i];
        }

        qtk_kalman2_feed(a->km_v2, (float*)sp->data, a->yt_pad_tmp);
        ...
        if(a->is_loop){
            ...
            if(a->nnrt){
                ...
                for(i = 0; i < fsize; i++){
                    ...
                    //wtk_strbuf_push(a->sp, (char *)&pv1, sizeof(short));
                    wtk_strbuf_push_float(a->sp, &pv1, 1);
                }
                if(a->notify){
