...
void qtk_kalman_reset(qtk_ahs_kalman_t *km){
    ...
    if(!km->cfg->use_symmetric_ph){
        if(km->cfg->use_block_dg){
			
            for(i = 0; i < km->Ph_shape[0]; i++){
                for(m = 0; m < km->Ph_shape[1]; m+=km->cfg->dg_row){
                    for(n=0;n<km->cfg->dg_row;n++){
                        cp->a=km->pworg;
                        cp+=km->cfg->dg_col+1;  
                    }
                    cp-=km->cfg->dg_col;
            ...
            #endif
        }else if(km->cfg->use_dg){
            for(i = 0; i < km->Ph_shape[0]; i++){
                for(m = 0; m < km->Ph_shape[1]; m++,++cp){
                    cp->a=km->pworg;
                }
            }
        }else{
...
#ifdef USE_NEON
void qtk_kalman_update2_3_neon(qtk_ahs_kalman_t *km, wtk_complex_t *y,
                        int nbin, int batchsize,
                        wtk_complex_t *x_fft)
{
    wtk_complex_t tc = {1.0, 0.0};
...
void qtk_kalman_update(qtk_ahs_kalman_t *km, wtk_complex_t *y, int nbin, int batchsize, wtk_complex_t *x_fft){
    if(km->type == 0){
        if(km->cfg->use_dg){
#ifdef USE_NEON
            qtk_kalman_update2_3_neon(km,y,nbin,batchsize,x_fft);
#else
            qtk_kalman_update2_3(km,y,nbin,batchsize,x_fft);
#endif
        }else if(km->cfg->use_block_dg){
#ifdef USE_NEON
