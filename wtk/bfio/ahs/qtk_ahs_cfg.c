...
int qtk_ahs_cfg_init(qtk_ahs_cfg_t *cfg) {
    ...
    wtk_equalizer2_cfg_init(&(cfg->eq2));    
    cfg->use_eq=0;
    cfg->use_eq2=0;
    cfg->eq_gain_fn = NULL;
    ...
    qtk_ahs_kalman2_cfg_init(&(cfg->km_v2));
    cfg->use_kmv2 = 0;
    cfg->use_kmv2_mdl = 0;
    cfg->use_sp_check = 0;
    cfg->use_mt = 0;
...
int qtk_ahs_cfg_update_local(qtk_ahs_cfg_t *cfg, wtk_local_cfg_t *lc) {
    ...
    wtk_local_cfg_update_cfg_b(lc, cfg, use_kmv2, v);
    wtk_local_cfg_update_cfg_b(lc, cfg, use_kmv2_mdl, v);
    wtk_local_cfg_update_cfg_b(lc, cfg, use_drc, v);
    ...
    sub=wtk_local_cfg_find_lc_s(lc,"eq2");
    if(sub)
    {
        wtk_equalizer2_cfg_update_local(&(cfg->eq2),sub);
    }    
    ...
    sub=wtk_local_cfg_find_lc_s(lc,"km_v2");
	if(sub)
	{
		qtk_ahs_kalman2_cfg_update_local(&(cfg->km_v2),sub);
	}
    ...
    sub=wtk_local_cfg_find_lc_s(lc,"drc");
    if(sub)
    {
        wtk_drc_cfg_update_local(&(cfg->drc),sub);
    }
...
int qtk_ahs_cfg_update2(qtk_ahs_cfg_t *cfg, wtk_source_loader_t *sl) {
    ...
    wtk_bf_cfg_set_channel(&(cfg->bf), 1);
    wtk_equalizer_cfg_update(&(cfg->eq));
    wtk_equalizer2_cfg_update2(&(cfg->eq),sl);
    ...
    qtk_ahs_kalman2_cfg_update2(&cfg->km_v2, sl);
    if(cfg->use_gainnet){
    ...
    wtk_drc_cfg_update2(&(cfg->drc),sl);
