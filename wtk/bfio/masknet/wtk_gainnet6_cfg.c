...
int wtk_gainnet6_cfg_clean(wtk_gainnet6_cfg_t *cfg)
{
	if(cfg->de_cnn)
	{
		wtk_cnnnet_layer_delete(cfg->de_cnn);
	}
	if(cfg->de_dnn)
	{
...
int wtk_gainnet6_cfg_update_mdl(wtk_gainnet6_cfg_t *cfg, wtk_source_t *src)
{
	int ret;
	wtk_strbuf_t *buf;
	...
	{
		cfg->de_cnn=wtk_cnnnet_layer_new(src, buf);
		if(!cfg->de_cnn)
		{
			ret=-1;goto end;
		}
...
int wtk_gainnet6_cfg_update(wtk_gainnet6_cfg_t *cfg)
{
    ...
    ret = wtk_source_loader_load(&sl, cfg, (wtk_source_load_handler_t) wtk_gainnet6_cfg_update_mdl, cfg->model_fn);
	if(ret!=0){goto end;}
...
wtk_gainnet6_cfg_t* wtk_gainnet6_cfg_new_bin(char *bin_fn,char *cfg_fn)
{
	wtk_mbin_cfg_t *cfg;
	wtk_gainnet6_cfg_t *vc;

	//wtk_debug("%s/%s\n",bin_fn,cfg_fn);
	cfg=wtk_mbin_cfg_new_type(wtk_gainnet6_cfg,bin_fn,cfg_fn);
	vc=(wtk_gainnet6_cfg_t*)(cfg->cfg);
	vc->hook=cfg;
	return vc;
}
...
{
	return wtk_gainnet6_cfg_new_bin(bin_fn,"./gainnet.cfg");
}
...
{
	wtk_gainnet6_cfg_t *cfg;
	wtk_rbin2_item_t *item;
	wtk_source_loader_t sl;
	char *cfg_fn="./gainnet.cfg";
	int ret;

	cfg=(wtk_gainnet6_cfg_t*)wtk_malloc(sizeof(wtk_gainnet6_cfg_t));
	wtk_gainnet6_cfg_init(cfg);
	...
	if(ret!=0){goto end;}
	ret=wtk_gainnet6_cfg_update_local(cfg,cfg->cfile->main);
	if(ret!=0){goto end;}
	sl.hook=cfg->rbin;
	sl.vf=(wtk_source_loader_v_t)wtk_rbin2_load_file;
	ret=wtk_gainnet6_cfg_update2(cfg,&sl);
	if(ret!=0){goto end;}
end:
	//wtk_debug("ret=%d\n",ret);
	if(ret!=0)
	{
		wtk_gainnet6_cfg_delete_bin2(cfg);
		cfg=NULL;
	}
	return cfg;
}
