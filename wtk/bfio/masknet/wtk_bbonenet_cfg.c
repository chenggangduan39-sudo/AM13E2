...
int wtk_bbonenet_cfg_clean(wtk_bbonenet_cfg_t *cfg)
{
	if(cfg->cnn1)
	{
		wtk_cnn1dnet_layer_delete(cfg->cnn1);
	}
	if(cfg->cnn2)
	{
		wtk_cnn1dnet_layer_delete(cfg->cnn2);
	}	
	if(cfg->idnn)
	{
		wtk_dnnnet_layer_delete(cfg->idnn);
	}
	if(cfg->gru1)
	{
...
int wtk_bbonenet_cfg_update_mdl(wtk_bbonenet_cfg_t *cfg, wtk_source_t *src)
{
	int ret;
	wtk_strbuf_t *buf;
	...
	{
		cfg->idnn=wtk_dnnnet_layer_new(src, buf);
		if(!cfg->idnn)
		{
			ret=-1;goto end;
		}
	...
	if(wtk_str_equal_s(buf->data, buf->pos,"cnn1d"))
	{
		cfg->cnn1=wtk_cnn1dnet_layer_new(src, buf);
		if(!cfg->cnn1)
		{
			ret=-1;goto end;
		}
	...
	if(wtk_str_equal_s(buf->data, buf->pos,"cnn1d"))
	{
		cfg->cnn2=wtk_cnn1dnet_layer_new(src, buf);
		if(!cfg->cnn2)
		{
			ret=-1;goto end;
		}
...
int wtk_bbonenet_cfg_update(wtk_bbonenet_cfg_t *cfg)
{
    ...
    ret = wtk_source_loader_load(&sl, cfg, (wtk_source_load_handler_t) wtk_bbonenet_cfg_update_mdl, cfg->model_fn);
	if(ret!=0){goto end;}
...
wtk_bbonenet_cfg_t* wtk_bbonenet_cfg_new_bin(char *bin_fn,char *cfg_fn)
{
	wtk_mbin_cfg_t *cfg;
	wtk_bbonenet_cfg_t *vc;

	//wtk_debug("%s/%s\n",bin_fn,cfg_fn);
	cfg=wtk_mbin_cfg_new_type(wtk_bbonenet_cfg,bin_fn,cfg_fn);
	vc=(wtk_bbonenet_cfg_t*)(cfg->cfg);
	vc->hook=cfg;
	return vc;
}
...
{
	return wtk_bbonenet_cfg_new_bin(bin_fn,"./gainnet.cfg");
}
...
wtk_bbonenet_cfg_t* wtk_bbonenet_cfg_new_bin3(char *bin_fn,unsigned int seek_pos)
{
	...
	cfg=(wtk_bbonenet_cfg_t*)wtk_malloc(sizeof(wtk_bbonenet_cfg_t));
	wtk_bbonenet_cfg_init(cfg);
	...
	if(ret!=0){goto end;}
	ret=wtk_bbonenet_cfg_update_local(cfg,cfg->cfile->main);
	if(ret!=0){goto end;}
	sl.hook=cfg->rbin;
	sl.vf=(wtk_source_loader_v_t)wtk_rbin2_load_file;
	ret=wtk_bbonenet_cfg_update2(cfg,&sl);
	if(ret!=0){goto end;}
end:
	//wtk_debug("ret=%d\n",ret);
	if(ret!=0)
	{
		wtk_bbonenet_cfg_delete_bin2(cfg);
		cfg=NULL;
	}
	return cfg;
}
