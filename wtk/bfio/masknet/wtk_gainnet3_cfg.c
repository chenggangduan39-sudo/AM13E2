...
int wtk_gainnet3_cfg_init(wtk_gainnet3_cfg_t *cfg)
{
	cfg->model_fn=NULL;
...
int wtk_gainnet3_cfg_clean(wtk_gainnet3_cfg_t *cfg)
{
	if(cfg->ns_dnn)
	{
...
int wtk_gainnet3_cfg_update_local(wtk_gainnet3_cfg_t *cfg,wtk_local_cfg_t *lc)
{
    wtk_string_t *v;
...
int wtk_gainnet3_cfg_update_mdl(wtk_gainnet3_cfg_t *cfg, wtk_source_t *src)
{
	int ret;
	wtk_strbuf_t *buf;
...
int wtk_gainnet3_cfg_update(wtk_gainnet3_cfg_t *cfg)
{
	int ret;
    ...
    ret = wtk_source_loader_load(&sl, cfg, (wtk_source_load_handler_t) wtk_gainnet3_cfg_update_mdl, cfg->model_fn);
	if(ret!=0){goto end;}
...
int wtk_gainnet3_cfg_update2(wtk_gainnet3_cfg_t *cfg,wtk_source_loader_t *sl)
{
	int ret;
...
wtk_gainnet3_cfg_t* wtk_gainnet3_cfg_new_bin(char *bin_fn,char *cfg_fn)
{
	wtk_mbin_cfg_t *cfg;
	wtk_gainnet3_cfg_t *vc;

	//wtk_debug("%s/%s\n",bin_fn,cfg_fn);
	cfg=wtk_mbin_cfg_new_type(wtk_gainnet3_cfg,bin_fn,cfg_fn);
	vc=(wtk_gainnet3_cfg_t*)(cfg->cfg);
	vc->hook=cfg;
	return vc;
}

wtk_gainnet3_cfg_t* wtk_gainnet3_cfg_new_bin2(char *bin_fn)
{
	return wtk_gainnet3_cfg_new_bin(bin_fn,"./gainnet.cfg");
}

int wtk_gainnet3_cfg_delete_bin2(wtk_gainnet3_cfg_t *cfg)
{
	wtk_mbin_cfg_delete((wtk_mbin_cfg_t*)(cfg->hook));
	return 0;
}

wtk_gainnet3_cfg_t* wtk_gainnet3_cfg_new_bin3(char *bin_fn,unsigned int seek_pos)
{
	...
	cfg=(wtk_gainnet3_cfg_t*)wtk_malloc(sizeof(wtk_gainnet3_cfg_t));
	wtk_gainnet3_cfg_init(cfg);
	...
	if(ret!=0){goto end;}
	ret=wtk_gainnet3_cfg_update_local(cfg,cfg->cfile->main);
	if(ret!=0){goto end;}
	sl.hook=cfg->rbin;
	sl.vf=(wtk_source_loader_v_t)wtk_rbin2_load_file;
	ret=wtk_gainnet3_cfg_update2(cfg,&sl);
	if(ret!=0){goto end;}
end:
	//wtk_debug("ret=%d\n",ret);
	if(ret!=0)
	{
		wtk_gainnet3_cfg_delete_bin2(cfg);
		cfg=NULL;
	}
	return cfg;
}

int wtk_gainnet3_cfg_delete_bin3(wtk_gainnet3_cfg_t *cfg)
{
    wtk_rbin2_delete(cfg->rbin);
    wtk_cfg_file_delete(cfg->cfile);
	wtk_gainnet3_cfg_clean(cfg);
	wtk_free(cfg);
	return 0;
}
