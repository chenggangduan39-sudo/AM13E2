...
void test_file(qtk_gainnetbf_t *vb, char *data_fn)
{
	char *data;
	int datalen;
	int ret;
	...
	while(1)
	{
		...
		qtk_gainnetbf_feed(vb, data, ret, 0);

		datalen+=ret;
	}
	qtk_gainnetbf_feed(vb, NULL, 0, 1);
	qtk_gainnetbf_reset(vb);
	wtk_riff_delete(riff);
	wtk_free(data);
}

int gainnetbf_run(int argc, char **argv)
{
	wtk_arg_t *arg = NULL;
	qtk_gainnetbf_cfg_t *cfg = NULL;
	qtk_gainnetbf_t *vb = NULL;
	char *cfg_fn=NULL;
	char *data_fn=NULL;
	char *out_fn=NULL;
	int ret;
	...
	cfg = qtk_gainnetbf_cfg_new(cfg_fn);
	vb = qtk_gainnetbf_new(cfg);
	if(!vb){
		wtk_debug("gainnetbf new failed.\n")
		goto end;
	}
	qtk_gainnetbf_set_notify(vb, wav, (qtk_gainnetbf_notify_f)test_on_data);
	qtk_gainnetbf_start(vb);
