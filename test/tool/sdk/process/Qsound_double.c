...
int qformssl_run(char *cfg_fn, char *infn, int channel)
{
	...
	qtk_mod_double_cfg_t *cfg = NULL;
	qtk_mod_double_t *mod = NULL;

	int starttime=(int)time_get_ms();
	int overtime=3600*1000*2;

	main_cfg = wtk_main_cfg_new_type(qtk_mod_double_cfg, cfg_fn);
	if(!main_cfg){
		wtk_debug("main_cfg new failed.\n");
		goto end;
	}

	cfg = (qtk_mod_double_cfg_t *)main_cfg->cfg;
	mod = qtk_mod_double_new(cfg);
	if(!mod){
	...
	while(pos < len){
		...
		qtk_mod_double_feed(mod, data+pos, flen);
		// wtk_msleep(20);
		usleep(32* 1000);
		pos += flen;
	}
	free(data);
#endif
	qtk_mod_double_stop(mod);
end:

	if(mod){
		qtk_mod_double_delete(mod);
	}
	if(main_cfg){
