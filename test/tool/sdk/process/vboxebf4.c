...
static void test_vboxebf4_on(wtk_wavfile_t *wav,short *data,int len)
{
	int i;
...
static void test_vboxebf4_on_ssl(void *ths,wtk_ssl2_extp_t *nbest_extp,int nbest)
{
    int i;
...
static void vboxebf4_test_file(wtk_vboxebf4_t *vboxebf4,char *ifn,char *ofn)
{
    wtk_riff_t *riff;
    wtk_wavfile_t *wav;
    short *pv;
    int channel=vboxebf4->cfg->nmicchannel+vboxebf4->cfg->nspchannel;
    int len,bytes;
    int ret;
    double t;
    int cnt;
    ...
    wav=wtk_wavfile_new(vboxebf4->cfg->bf.rate);
    wav->max_pend=0;
	...
	wtk_vboxebf4_start(vboxebf4);
    wtk_vboxebf4_set_notify(vboxebf4,wav,(wtk_vboxebf4_notify_f)test_vboxebf4_on);
    wtk_vboxebf4_set_ssl_notify(vboxebf4,wav,(wtk_vboxebf4_notify_ssl_f)test_vboxebf4_on_ssl);
    ...
    while(1)
    {
        ...
        cnt+=len;
        wtk_vboxebf4_feed(vboxebf4,pv,len,0);
		// wtk_debug("deley %f\n",(cnt-out_len)/16.0);
    }
    wtk_vboxebf4_feed(vboxebf4,NULL,0,1);
    // char c[32];
    ...
    //     wtk_vboxebf4_set_notify(vboxebf4,wav,(wtk_vboxebf4_notify_f)test_vboxebf4_on);
    //     wtk_vboxebf4_feed(vboxebf4,pv,len,1);
...
{
	wtk_vboxebf4_t *vboxebf4=NULL;
	wtk_vboxebf4_cfg_t *cfg=NULL;
	wtk_arg_t *arg;
	char *cfg_fn=NULL;
	char *bin_fn=NULL;
	char *scp=NULL;
	...
	if(cfg_fn)
	{
		cfg=wtk_vboxebf4_cfg_new(cfg_fn);
		if(cfg==NULL)
		{
			print_usage();
			goto end;
	...
	{
		cfg=wtk_vboxebf4_cfg_new_bin(bin_fn);
		if(cfg==NULL)
		{
			print_usage();
			goto end;
	...
	if(wav)
	{
		...
		for(i=0;i<1;++i)
		{
			vboxebf4_test_file(vboxebf4,wav,output);
		}
//		vboxebf4_test_file(vboxebf4,wav,"2.wav");
//		vboxebf4_test_file(vboxebf4,wav,"3.wav");
	}
	if(scp)
	{
		wtk_flist_it_t *it;
		...
		while(1)
		{
			...
			{
				vboxebf4_test_file(vboxebf4,ifn,buf->data);
			}
			wtk_string_delete(v);
		}
	...
	if(vboxebf4)
	{
		wtk_vboxebf4_delete(vboxebf4);
	}
	if(cfg)
	{
		if(cfg_fn)
		{
			wtk_vboxebf4_cfg_delete(cfg);
		}else
		{
			wtk_vboxebf4_cfg_delete_bin(cfg);
		}
	}
	if(arg)
	{
