...
void test_etest(qtk_session_t *s, char *infn, int is_enroll)
{
	...
	qtk_engine_set_notify(eb->b,eb,(qtk_engine_notify_f)etest_on_notify);
	wav_fn = infn;

	qtk_engine_set(eb->b, "enroll_fn=./mem.bin;");
	if(is_enroll)
	{
		qtk_engine_set(eb->b, "enroll_name=liuyong;enroll_isend=0;");
	}
	...
	if(is_enroll)
	{
		qtk_engine_set(eb->b, "enroll_name=liuyong;enroll_isend=1;");
	}
