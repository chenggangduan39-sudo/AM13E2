#include "test-demo/test-am13e2/qtk_mod_am13e2.h"
...
void qtk_mod_am13e2_test_on_usb(qtk_mod_am13e2_t *mod, qtk_usb_uevent_state_t state, int sample_rate)
{
	switch (state)
	{
	case QTK_USB_STATE_PLAYER_START:
		printf("==================>>>>>>>>>>>>>>start\n");
		if(is_start==0 || sample_rate!=48000){
			qtk_mod_am13e2_start2(mod, sample_rate);
			is_start=1;
		}
		break;
...
int qformssl_run(char *cfg_fn, char *infn, char *vpfn, int channel)
{
	...
	qtk_mod_am13e2_cfg_t *cfg = NULL;
	qtk_mod_am13e2_t *mod = NULL;
// #ifdef USE_KTC3308
// 	qtk_usb_uevent_t *qu;
// #endif
	...
	main_cfg = wtk_main_cfg_new_type(qtk_mod_am13e2_cfg, cfg_fn);
	if(!main_cfg){
	...
	cfg = (qtk_mod_am13e2_cfg_t *)main_cfg->cfg;

	mod = qtk_mod_am13e2_new(cfg);
	if(!mod){
...
// 	qtk_usb_uevent_set_notify(qu, mod, (qtk_usb_uevent_notify_f)qtk_mod_am13e2_test_on_usb);
// #endif
	...
	wtk_debug("===================>>>>>>>>>>rate=%d\n",rate);
	qtk_mod_am13e2_start2(mod, rate);
	is_start=1;
	qtk_mod_am13e2_start(mod, 0);
	wtk_debug("----------------------------------\n");
#ifndef OFFLINE_TEST
#ifndef DEBUG
	...
	while(pos < len){
		...
		qtk_mod_am13e2_feed(mod, data+pos, flen);
		// wtk_msleep(20);
		usleep(32* 1000);
		pos += flen;
	}
	free(data);
#endif
	qtk_mod_am13e2_stop(mod);
end:

	if(mod){
		qtk_mod_am13e2_delete(mod);
	}
	if(main_cfg){
