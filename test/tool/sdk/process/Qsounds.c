...
int qformssl_run(char *cfg_fn, char *infn, int channel)
{
	...
	qtk_mod2_cfg_t *cfg = NULL;
	qtk_mod2_t *mod = NULL;

	int starttime=(int)time_get_ms();
	int overtime=3600*1000*2;

	main_cfg = wtk_main_cfg_new_type(qtk_mod2_cfg, cfg_fn);
	if(!main_cfg){
		wtk_debug("main_cfg new failed.\n");
		goto end;
	}

	cfg = (qtk_mod2_cfg_t *)main_cfg->cfg;
	mod = qtk_mod2_new(cfg);
	if(!mod){
	...
	while(pos < len){
		...
		qtk_mod2_feed(mod, data+pos, flen);
		// wtk_msleep(20);
		usleep(32* 1000);
		pos += flen;
	}
