...
void test_ecsrsc_on_notify(void *ths,qtk_var_t *var)
{
	switch(var->type) {
...
void test_ecsrsc(qtk_session_t *session, char *infn)
{
	qtk_engine_t *e;
	char *params;
	char *fn,*data;
	...
	params = "role=csrsc;cfg=./res-csrsc/cfg;use_bin=0;use_thread=0;use_lex=0;use_hint=1;";//left_margin=10;right_margin=10;
	...
	ret = qtk_engine_set_notify(e,NULL,(qtk_engine_notify_f)test_ecsrsc_on_notify);
	if(ret != 0) {
		wtk_debug("set notify failed.\n");
		exit(1);
	}

	qtk_engine_set(e, "enroll_fn=./mem.bin;");
