...
static void test_mask_bf_net_on2(wtk_wavfile_t *wav,short *data,short *data2,int len)
{
	int i;
...
static void mask_bf_net_test_file(wtk_mask_bf_net_t *mask_bf_net, char *ifn,
                                  char *ofn, char *ofn2) {
    wtk_riff_t *riff;
    wtk_wavfile_t *wav;
    ...
    wtk_mask_bf_net_set_notify(mask_bf_net, wav,
                               (wtk_mask_bf_net_notify_f)test_mask_bf_net_on);
    if(wav2){
        wtk_mask_bf_net_set_notify2(mask_bf_net,wav2,(wtk_mask_bf_net_notify_f2)test_mask_bf_net_on2);
    }
    // wtk_riff_read(riff,(char *)pv,56*channel);

    // wtk_mask_bf_net_set_micscale(mask_bf_net, 1.0);
    // wtk_mask_bf_net_set_agcenable(mask_bf_net, 1);
    // wtk_mask_bf_net_set_agclevel(mask_bf_net, 4);  // [1-6]
    // wtk_mask_bf_net_set_echoenable(mask_bf_net, 1);
    // wtk_mask_bf_net_set_denoiseenable(mask_bf_net, 1);
    // wtk_mask_bf_net_set_denoiselevel(mask_bf_net, 4);  // [1-6]
    ...
    t = time_get_ms() - t;
    wtk_debug("rate=%f t=%f\n", t / (cnt / (mask_bf_net->cfg->rate / 1000.0)),
              t);
