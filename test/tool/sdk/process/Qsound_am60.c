...
void qtk_mod_am60_test_on_usb(qtk_mod_am60_t *mod, qtk_usb_uevent_state_t state, int sample_rate)
{
	switch (state)
	{
	case QTK_USB_STATE_PLAYER_START:
		...
		{
			qtk_mod_am60_start2(mod, sample_rate);
			is_start=1;
		}
		break;
...
int qformssl_run(char *cfg_fn, char *infn, char *vpfn, int channel)
{
	...
	qtk_mod_am60_cfg_t *cfg = NULL;
	qtk_mod_am60_t *mod = NULL;
// #ifdef USE_KTC3308
// 	qtk_usb_uevent_t *qu;
// #endif
	...
	main_cfg = wtk_main_cfg_new_type(qtk_mod_am60_cfg, cfg_fn);
	if(!main_cfg){
	...
	cfg = (qtk_mod_am60_cfg_t *)main_cfg->cfg;
	if(vpfn)
	{
		wtk_string_set(&cfg->enroll_fn, vpfn, strlen(vpfn));
	}

	mod = qtk_mod_am60_new(cfg);
	if(!mod){
...
// 	qtk_usb_uevent_set_notify(qu, mod, (qtk_usb_uevent_notify_f)qtk_mod_am60_test_on_usb);
// #endif
	...
	wtk_debug("===================>>>>>>>>>>rate=%d\n",rate);
	qtk_mod_am60_start2(mod, rate);
	is_start=1;
#endif

	qtk_mod_am60_start(mod);
	...
	while(pos < len){
		...
		qtk_mod_am60_feed(mod, data+pos, flen);
		// wtk_msleep(20);
		usleep(32* 1000);
		pos += flen;
	}
	free(data);
#endif
	qtk_mod_am60_stop(mod);
end:

	if(mod){
		qtk_mod_am60_delete(mod);
	}
	if(main_cfg){
