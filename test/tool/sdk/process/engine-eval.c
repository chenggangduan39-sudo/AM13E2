...
//		s=data+offset;e=s+len-offset;
		...
		while(s<e)
		{
			...
			//if(nt/(3000*16) > 1 && is_enroll)
			// // if(0)
			//{
			//	qtk_engine_set(dec, "enroll_fn=./mem2.bin;");
			//	is_enroll=0;
			// 	qtk_var_t var;
...
//	}
	...
	int ochannel=8;
	char tcfg[256]={0};
	snprintf(tcfg, 256, "role=kws;cfg=%s;syn=1;use_thread=0;out_channel=%d;", cfg_fn, ochannel);
	...
	if(o_fn)
	{
		...
		owav->max_pend=0;
		wtk_wavfile_set_channel(owav, ochannel);
