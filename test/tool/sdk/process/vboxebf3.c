...
static void test_vboxebf3_on_ssl(void *ths,float ts, float te,wtk_ssl2_extp_t *nbest_extp,int nbest)
{
    ...
    for(i=0;i<nbest;++i)
    {
        printf("i=%d [%f %f] ==> %d %d %f\n", i, ts, te, nbest_extp[i].theta, nbest_extp[i].phi, nbest_extp[i].nspecsum);
    }
	if(nbest==0)
	{
...
static void vboxebf3_test_file(wtk_vboxebf3_t *vboxebf3,char *ifn,char *ofn)
{
    ...
    wtk_vboxebf3_set_sslenable(vboxebf3, 0);
    cnt=0;
    t=time_get_ms();
    while(1)
    {
    ...
    //     wtk_vboxebf3_set_notify(vboxebf3,wav,(wtk_vboxebf3_notify_f)test_vboxebf3_on);
    //     wtk_vboxebf3_feed(vboxebf3,pv,len,1);
    ...
    t=time_get_ms()-t;
    wtk_debug("rate=%f t=%f time=%f\n",t/(cnt/48.0),t,cnt/48.0);
...
int main(int argc,char **argv)
{
	...
	if(cfg_fn)
	{
		cfg=wtk_vboxebf3_cfg_new(cfg_fn);
		if(cfg==NULL)
		{
			print_usage();
			goto end;
	...
	{
		cfg=wtk_vboxebf3_cfg_new_bin(bin_fn);
		if(cfg==NULL)
		{
			print_usage();
			goto end;
...
//		vboxebf3_test_file(vboxebf3,wav,"3.wav");
	...
	if(cfg)
	{
		if(cfg_fn)
		{
			wtk_vboxebf3_cfg_delete(cfg);
		}else
		{
			wtk_vboxebf3_cfg_delete_bin(cfg);
		}
	}
	if(arg)
	{
