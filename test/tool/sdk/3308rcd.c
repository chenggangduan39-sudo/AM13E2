...
int main(int argc, char **argv)
{
	...
	main_cfg = wtk_main_cfg_new_type(qtk_record_cfg, cfg_fn);
	if(!main_cfg){
		printf("cfg new failed.\n");
		goto end;
	}	
	cfg = (qtk_record_cfg_t *)main_cfg->cfg;
	rcd = qtk_record_new(cfg);
	if(!rcd){
		printf("rcd new failed.\n");
		goto end;
	}
	channel = cfg->channel - cfg->nskip;
	mul = wtk_wavfile_new(16000);
	...
	printf("pless enter key to stop recorder.");
	getchar();
	run = 0;
	wtk_thread_join(&t);
