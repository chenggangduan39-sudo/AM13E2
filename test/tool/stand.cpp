...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        for (int i = 0; i < hk->head_res.cnt; i++){
            putText(frame, std::to_string(hk->head->res.box[i].score), Point((int)(hk->head_res.box[i].roi.x1 * w_dio), (int)(hk->head_res.box[i].roi.y1 * h_dio)-10), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 255, 0), 1);
            rectangle(frame,
