...
wtk_queue_t vqueue;
int wakeup_cnt = 0;
void test_vad_file(qtk_asr_wrapper_t *dec,wtk_vad_t *vad,char *fn,wtk_rate_t *rate){
    wtk_strbuf_t **buf;
	wtk_vframe_t *f;
	...
	while(1){
		...
		case wtk_vframe_sil:
			if(f->state != wtk_vframe_sil){
				...
				ret = qtk_asr_wrapper_feed(dec,(char*)f->wav_data,sizeof(short)*vad->cfg->dnnvad.parm.frame_step,0);
				if(ret == 1){
					wtk_debug("wakeup\n");
				}
		...
		case wtk_vframe_speech:
			if(f->state != wtk_vframe_speech){
				...
				wtk_debug("%d %d\n",st,end);
				ret = qtk_asr_wrapper_feed(dec,0,0,1);
				if(ret == 1){
					wtk_debug("wakeup\n");
					wakeup_cnt++;
				}				
...
void asr_cb(void *ths, char *data, int len){
	if(len > 0){
		wtk_json_item_t *item;
		wtk_json_parser_parse(parser,data,len);
		item = wtk_json_obj_get_s(parser->json->main, "rec");
		if(item->v.str->len > 0){
...
void wake_cb(void *ths, float fs, float fe){
	wtk_debug("wakeup: %f %f\n",fs,fe);
}
...
int main(int argc,char **argv)
{
	...
	wtk_arg_get_str_s(arg,"word",&w_fn);
...
#ifndef USE_KSD_EBNF
		qtk_asr_wrapper_set_asr_notify(dec,NULL,asr_cb);
		qtk_asr_wrapper_set_wakeup_notify(dec,NULL,wake_cb);
#endif
    }
	...
	if(h_fn && h2_fn){
		...
		qtk_asr_wrapper_set_context_wakeup(dec,data,len);
		data = file_read_buf(h2_fn,&len);
		qtk_asr_wrapper_set_context_asr(dec,data,len);
	}else if(h_fn){
		data = file_read_buf(h_fn,&len);
	...
	if(mode >= 0 && mode < 3){
		qtk_asr_wrapper_set_mode(dec,mode);
	}
    if(w_fn){
        data = file_read_buf(w_fn,&len);
        qtk_asr_wrapper_set_contacts(dec,data,len);
        wtk_free(data);
    }
#endif
    if(vcfg_fn)
