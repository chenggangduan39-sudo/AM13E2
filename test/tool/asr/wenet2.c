...
#include "wtk/asr/wfst/wenetdec/qtk_wenet_wrapper_cfg.h"
...
void test_wav_file(qtk_wenet_wrapper_t *dec,char *fn,wtk_rate_t *rate)
{
	int n,i;
	int len,offset;
    ...
    //fprintf(fppp,"%s ",fn);
    printf("%s ",fn);
	qtk_wenet_wrapper_start(dec);
	{
        while (is_end == 0) {
            read_cnt = wtk_riff_read(riff, buf, sizeof(buf));
            qtk_wenet_wrapper_get_hint_result(dec,&v);
            printf("%.*s\n",v.len,v.data);
            if (read_cnt < sizeof(buf)) {
                is_end = 1;
            }
            qtk_wenet_wrapper_feed(dec, buf, read_cnt,0);
        }
		qtk_wenet_wrapper_feed(dec,0,0,1);
	}

	qtk_wenet_wrapper_get_result(dec,&v);
	//fprintf(fppp,"%.*s\n",v.len,v.data);
	printf("%.*s\n",v.len,v.data);
//	fflush(fppp);
	qtk_wenet_wrapper_reset(dec);
...
void test_scp(qtk_wenet_wrapper_t *dec,char *fn)
{
	wtk_flist_t *f;
	wtk_queue_node_t *n;
	wtk_fitem_t *item;
...
int main(int argc,char **argv)
{
	...
	wtk_arg_t *arg;
	qtk_wenet_wrapper_t *dec=0;
	qtk_wenet_wrapper_cfg_t *cfg=NULL;
	char *cfg_fn=0;
	char *scp_fn=0;
	char *wav_fn=0;
	char *feat_fn=0;
...
//	}
    ...
    if(cfg_fn)
    {
		if(bin)
		{
			cfg=qtk_wenet_wrapper_cfg_new_bin(cfg_fn);
		}else
		{
		...
		cfg=(qtk_wenet_wrapper_cfg_t*)main_cfg->cfg;

		}
		dec=qtk_wenet_wrapper_new(cfg);
		if(!dec)
		{
			wtk_debug("create wenet failed.\n");
			goto end;
		}
    }
	if(wav_fn)
...
/*	main_cfg=wtk_main_cfg_new_type(qtk_wenet_wrapper_cfg,cfg_fn);
	...
	}else
	{
		if(cfg)
		{
			qtk_wenet_wrapper_cfg_delete_bin(cfg);
		}
	}
	wtk_arg_delete(arg);
