...
{
	printf("Usage:\n"
			" vits -c cfgfn use_bin [0|1] -s text -scp textfn\n"
			"    -c cfgfn cfg file\n"
			"    -b use_bin res format, default 0\n"
			"    -s text, optional\n"
			"    -scp scp text file, optional, don't permit null line(include space, tab) \n"
			"    -o outdir, placing tts-wav \n"
			"    control switches:\n"
			...
			"    pl:\n"
			"    ./vits -b 1 -c res/bin/tts.bin -scp data/text.len.2\n"
			"    ./vits -b 1 -c res/bin/tts.bin -s \"今天天气怎么样\"\n"
			"    ./vits -b 1 -c res/bin/tts.bin -s \"今天天气怎么样\" -v 50 \n\n");
	return 0;
}
