...
int main(int argc, char *argv[]) {
    ...
    wtk_arg_get_str_s(arg,"h",&head_model);
    ...
    cv::Mat frame;
    qtk_cv_detection_head_t *head = qtk_cv_detection_head_new();
    head->iou = 0.45;
    head->conf = 0.3;
    head->img_desc.width = 960;
    head->img_desc.height = 540;

    const char *model = head_model;
    head->invo =
    ...
    while (1) {
        ...
        qtk_cv_detection_head_process(head, data1, qtk_cv_detection_onnxruntime_invoke_single);
        double w_dio = (double)width / (double)960;
        double h_dio = (double)height / (double)540;
        for (int i = 0; i < head->res.cnt; i++){                               
            rectangle(frame,
                      Point((int)(head->res.box[i].roi.x1 * w_dio),
                            (int)(head->res.box[i].roi.y1 * h_dio)),
                      Point((int)(head->res.box[i].roi.x2 * w_dio),
                            (int)(head->res.box[i].roi.y2 * h_dio)),
    ...
    cap.release();
    wir.release();
    qtk_cv_detection_onnxruntime_delete(head->invo);
    qtk_cv_detection_head_delete(head);
