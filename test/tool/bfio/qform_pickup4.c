...
#include "wtk/bfio/qform/wtk_qform_pickup4.h"
...
static void test_qform_pickup4_on(wtk_wavfile_t *wav,short *data,int len)
{
    int i;
...
static void test_qform_pickup4_file(wtk_qform_pickup4_t *qform,char *ifn,char *ofn,char *gravfn)
{
    wtk_riff_t *riff;
    wtk_wavfile_t *wav;
    short *pv;
    ...
    wtk_qform_pickup4_set_notify(qform,wav,(wtk_qform_pickup4_notify_f)test_qform_pickup4_on);
    wtk_qform_pickup4_start(qform, 90);

    cnt=0;
    t=time_get_ms();
    // wtk_qform_pickup4_set_grav(qform,10,10,10);
    ...
    while(1)
    {
        ...
        if(it)
        {
            ...
            wtk_qform_pickup4_set_grav(qform,x,y,z);
        }
        cnt+=len;
        wtk_qform_pickup4_feed(qform,data,len,0);
    }
    wtk_qform_pickup4_feed(qform,NULL,0,1);
...
int main(int argc,char **argv)
{
    ...
    qform=wtk_qform_pickup4_new();

    test_qform_pickup4_file(qform,ifn,ofn,gravfn);

    wtk_qform_pickup4_delete(qform);
end:
    if(arg)
    {
        wtk_arg_delete(arg);
