...
static void test_qform2_on(wtk_wavfile_t *wav,short *data,int len)
{
    int i;
...
static void test_qform2_file2(wtk_qform2_t *qform2,char *scp,char *odir)
{
    wtk_flist_it_t *it;
    wtk_riff_t *riff;
    wtk_wavfile_t *wav;
    ...
    wav=wtk_wavfile_new(qform2->cfg->bf.rate);
    wav->max_pend=0;
    riff=wtk_riff_new();
    ...
    wtk_qform2_set_notify(qform2,wav,(wtk_qform2_notify_f)test_qform2_on);
	...
	while(1)
	{
        ...
        while(1)
        {
            ...
            wtk_qform2_feed(qform2,data,len,0);
        }

        wtk_riff_close(riff);
        wtk_wavfile_close(wav);
	}
    wtk_qform2_feed(qform2,NULL,0,1);
...
static void test_qform2_file(wtk_qform2_t *qform2,char *ifn,char *ofn)
{
    wtk_riff_t *riff;
    wtk_wavfile_t *wav;
    short *pv;
...
int main(int argc,char **argv)
{
    ...
    main_cfg=wtk_main_cfg_new_type(wtk_qform2_cfg,cfg_fn);
    if(!main_cfg){goto end;}

    cfg=(wtk_qform2_cfg_t *)(main_cfg->cfg);

    qform2=wtk_qform2_new(cfg);


    if(scp)
    {
        test_qform2_file2(qform2,scp,ofn);
    }else
    {
        test_qform2_file(qform2,ifn,ofn);
    }

    wtk_qform2_delete(qform2);
end:
    if(arg)
    {
        wtk_arg_delete(arg);
