...
static void test_cmask_bfse_on(wtk_wavfile_t *wav,short *data,int len)
{
	int i;
...
static void test_cmask_bfse_feat_on(FILE *fp,float *data,int len)
{
    if(len>0)
    {
...
static void cmask_bfse_test_file(wtk_cmask_bfse_t *cmask_bfse,char *ifn,char *ofn)
{
    wtk_riff_t *riff;
    wtk_wavfile_t *wav;
    wtk_strbuf_t **buf;
    ...
    wav=wtk_wavfile_new(cmask_bfse->cfg->rate);
	wtk_wavfile_set_channel(wav,cmask_bfse->cfg->ntheta);
    wav->max_pend=0;
	...
	wtk_cmask_bfse_start(cmask_bfse);
    wtk_cmask_bfse_set_notify(cmask_bfse,wav,(wtk_cmask_bfse_notify_f)test_cmask_bfse_on);
    ...
    while(1)
    {
        ...
        cnt+=len;
        wtk_cmask_bfse_feed(cmask_bfse,pv,len,0);
		// wtk_debug("deley %f\n",(cnt-out_len)/16.0);
    }
    wtk_cmask_bfse_feed(cmask_bfse,NULL,0,1);
    t=time_get_ms()-t;
...
{
	wtk_cmask_bfse_t *cmask_bfse=NULL;
	wtk_cmask_bfse_cfg_t *cfg=NULL;
	wtk_arg_t *arg;
	char *cfg_fn=NULL;
	char *bin_fn=NULL;
	char *scp=NULL;
	...
	if(cfg_fn)
	{
		cfg=wtk_cmask_bfse_cfg_new(cfg_fn);
		if(cfg==NULL)
		{
			print_usage();
			goto end;
	...
	{
		cfg=wtk_cmask_bfse_cfg_new_bin(bin_fn);
		if(cfg==NULL)
		{
			print_usage();
			goto end;
	...
	cmask_bfse=wtk_cmask_bfse_new(cfg);
	if(wav)
	{
		cmask_bfse_test_file(cmask_bfse,wav,output);
	}
	if(scp)
	{
		wtk_flist_it_t *it;
		...
		while(1)
		{
			...
			{
				cmask_bfse_test_file(cmask_bfse,ifn,buf->data);
			}
			wtk_string_delete(v);
		}
	...
	if(cmask_bfse)
	{
		wtk_cmask_bfse_delete(cmask_bfse);
	}
	if(cfg)
	{
		if(cfg_fn)
		{
			wtk_cmask_bfse_cfg_delete(cfg);
		}else
		{
			wtk_cmask_bfse_cfg_delete_bin(cfg);
		}
	}
	if(arg)
	{
