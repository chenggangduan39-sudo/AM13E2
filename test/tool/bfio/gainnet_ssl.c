...
static void test_gainnet_ssl_on_ssl(void *ths,float ts, float te,wtk_ssl2_extp_t *nbest_extp,int nbest)
{
    int i;
...
static void gainnet_ssl_test_file(wtk_gainnet_ssl_t *gainnet_ssl,char *ifn,char *ofn)
{
    wtk_riff_t *riff;
    wtk_wavfile_t *wav;
    short *pv;
    ...
    wav=wtk_wavfile_new(gainnet_ssl->cfg->rate);
    wav->max_pend=0;
	...
	wtk_gainnet_ssl_start(gainnet_ssl);
    wtk_gainnet_ssl_set_ssl_notify(gainnet_ssl,wav,(wtk_gainnet_ssl_notify_ssl_f)test_gainnet_ssl_on_ssl);
    ...
    while(1)
    {
        ...
        cnt+=len;
        wtk_gainnet_ssl_feed(gainnet_ssl,pv,len,0);
		// wtk_debug("deley %f\n",(cnt-out_len)/16.0);
    }
    wtk_gainnet_ssl_feed(gainnet_ssl,NULL,0,1);
    // char c[32];
    ...
    //     wtk_gainnet_ssl_set_notify(gainnet_ssl,wav,(wtk_gainnet_ssl_notify_f)test_gainnet_ssl_on);
    //     wtk_gainnet_ssl_feed(gainnet_ssl,pv,len,1);
...
{
	wtk_gainnet_ssl_t *gainnet_ssl=NULL;
	wtk_gainnet_ssl_cfg_t *cfg=NULL;
	wtk_arg_t *arg;
	char *cfg_fn=NULL;
	char *bin_fn=NULL;
	char *scp=NULL;
	...
	if(cfg_fn)
	{
		cfg=wtk_gainnet_ssl_cfg_new(cfg_fn);
		if(cfg==NULL)
		{
			print_usage();
			goto end;
	...
	{
		cfg=wtk_gainnet_ssl_cfg_new_bin(bin_fn);
		if(cfg==NULL)
		{
			print_usage();
			goto end;
	...
	gainnet_ssl=wtk_gainnet_ssl_new(cfg);
	if(wav)
	{
		int i;
		...
		for(i=0;i<1;++i)
		{
			gainnet_ssl_test_file(gainnet_ssl,wav,output);
		}
//		gainnet_ssl_test_file(gainnet_ssl,wav,"2.wav");
//		gainnet_ssl_test_file(gainnet_ssl,wav,"3.wav");
	}
	if(scp)
	{
		wtk_flist_it_t *it;
		...
		while(1)
		{
			...
			{
				gainnet_ssl_test_file(gainnet_ssl,ifn,buf->data);
			}
			wtk_string_delete(v);
		}
	...
	if(gainnet_ssl)
	{
		wtk_gainnet_ssl_delete(gainnet_ssl);
	}
	if(cfg)
	{
		if(cfg_fn)
		{
			wtk_gainnet_ssl_cfg_delete(cfg);
		}else
		{
			wtk_gainnet_ssl_cfg_delete_bin(cfg);
		}
	}
	if(arg)
	{
