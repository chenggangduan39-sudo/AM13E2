...
#include "wtk/bfio/qform/wtk_qform_pickup3.h"
...
static void test_qform_pickup3_on(wtk_wavfile_t *wav,short *data,int len)
{
    int i;
...
static void test_qform_pickup3_file(wtk_qform_pickup3_t *qform,char *ifn,char *ofn)
{
    wtk_riff_t *riff;
    wtk_wavfile_t *wav;
    short *pv;
    ...
    wtk_qform_pickup3_set_notify(qform,wav,(wtk_qform_pickup3_notify_f)test_qform_pickup3_on);
    wtk_qform_pickup3_start(qform);

    cnt=0;
    t=time_get_ms();
    // wtk_qform_pickup3_set_grav(qform,10,10,10);
    ...
    while(1)
    {
        ...
        cnt+=len;
        wtk_qform_pickup3_feed(qform,data,len,0);
    }
    wtk_qform_pickup3_feed(qform,NULL,0,1);
...
int main(int argc,char **argv)
{
    ...
    qform=wtk_qform_pickup3_new();

    test_qform_pickup3_file(qform,ifn,ofn);

    wtk_qform_pickup3_delete(qform);
end:
    if(arg)
    {
        wtk_arg_delete(arg);
