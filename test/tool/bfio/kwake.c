...
static void _test_seg(wtk_kwake_t *dec, char *audio_fn, char *seg_fn,
                      float offset) {
    qtk_audio_segmenter_t seg;
    float fs, fe;
    wtk_string_t key, audio;
    int step;
    int wake_tot = 0;

    qtk_audio_segmenter_init(&seg, audio_fn, seg_fn, offset);

    while (qtk_audio_segmenter_next(&seg, &key, &audio, &fs, &fe)) {
        wtk_kwake_start(dec);
        while (audio.len > 0) {
            step = MIN(audio.len, 128);
            if (wtk_kwake_feed(dec, cast(short *, audio.data), step / 2, 0)) {
                wtk_kwake_reset(dec);
                wake_tot++;
            }
            audio.data += step;
        ...
        if (wtk_kwake_feed(dec, NULL, 0, 1)) {
            wake_tot++;
        }
        wtk_kwake_reset(dec);
    }

    qtk_audio_segmenter_clean(&seg);
    printf("%s: %d\n", audio_fn, wake_tot);
}
...
//wtk_debug("%d ,%d, %s \n",cfg->parm.use_knn,cfg->parm.knn.use_bin,cfg->parm.knn.bin_fn);
	...
	if(ifn)
	{
		...
		int ret;
                char *seg = NULL;
                double offset = 0;
                wtk_arg_get_str_s(arg, "seg", &seg);
                wtk_arg_get_number_s(arg, "offset", &offset);
