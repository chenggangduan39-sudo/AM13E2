...
#include "wtk/bfio/maskform/wtk_gainnet_qform9.h"
...
static void test_gainnet_qform9_on(wtk_wavfile_t *wav,short *data,int len,int is_end)
{
    int i;
...
static void test_gainnet_qform9_on_two_channel(wtk_wavfile_t *wav,short *data[2],int len,int is_end)
{
    int i;
...
static void test_gainnet_qform9_file2(wtk_gainnet_qform9_t *gainnet_qform9,char *scp,char *odir)
{
    wtk_flist_it_t *it;
    wtk_riff_t *riff;
    wtk_wavfile_t *wav;
    short *pv;
    short **data;
    int channel=gainnet_qform9->cfg->stft2.channel;
    int len,bytes;
    int ret;
    int i,j;
    ...
    wav=wtk_wavfile_new(gainnet_qform9->cfg->bf.rate);
    wav->max_pend=0;
    riff=wtk_riff_new();
    ...
    wtk_gainnet_qform9_set_notify(gainnet_qform9,wav,(wtk_gainnet_qform9_notify_f)test_gainnet_qform9_on);
    wtk_gainnet_qform9_start(gainnet_qform9,theta,phi);
	...
	while(1)
	{
        ...
        while(1)
        {
            ...
            wtk_gainnet_qform9_feed(gainnet_qform9,data,len,0);
        }

        wtk_riff_close(riff);
        wtk_wavfile_close(wav);
	}
    wtk_gainnet_qform9_feed(gainnet_qform9,NULL,0,1);
...
static void test_gainnet_qform9_file(wtk_gainnet_qform9_t *gainnet_qform9,char *ifn,char *ofn)
{
    wtk_riff_t *riff;
    wtk_wavfile_t *wav;
    short *pv;
    ...
    if(gainnet_qform9->cfg->use_two_channel){
        if(0){
            wtk_gainnet_qform9_set_notify_two_channel(gainnet_qform9,wav,(wtk_gainnet_qform9_notify_two_channel_f)test_gainnet_qform9_on_two_channel);
        }else{
            wtk_gainnet_qform9_set_notify(gainnet_qform9,wav,(wtk_gainnet_qform9_notify_f)test_gainnet_qform9_on);
        }
    }else{
...
int main(int argc,char **argv)
{
    ...
    gainnet_qform9=wtk_gainnet_qform9_new(cfg);


    if(scp)
    {
        test_gainnet_qform9_file2(gainnet_qform9,scp,ofn);
    }else
    {
        test_gainnet_qform9_file(gainnet_qform9,ifn,ofn);
    }

    wtk_gainnet_qform9_delete(gainnet_qform9);
end:
    if(arg)
    {
        wtk_arg_delete(arg);
