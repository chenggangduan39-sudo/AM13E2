...
static void test_wbf_on(wtk_wavfile_t **wav,short *data,int len,int idx, int is_end)
{
    int i;
...
static void test_wbf_file(wtk_wbf_t *wbf,char *ifn,char *ofn)
{
    wtk_riff_t *riff;
    wtk_wavfile_t *wav[20];
    short *pv;
    short **data;
    int channel=wbf->cfg->stft2.channel;
    int len,bytes;
    int ret;
    int i,j;
    double t;
    ...
    {
        wav[i]=wtk_wavfile_new(wbf->cfg->bf.rate);
        wav[i]->max_pend=0;
    ...
    wtk_wbf_set_notify(wbf,wav,(wtk_wbf_notify_f)test_wbf_on);
    wtk_wbf_start(wbf);
    ...
    while(1)
    {
        ...
        cnt+=len;
        wtk_wbf_feed(wbf,data,len,0);
    }
    wtk_wbf_feed(wbf,NULL,0,1);
...
int main(int argc,char **argv)
{
    ...
    main_cfg=wtk_main_cfg_new_type(wtk_wbf_cfg,cfg_fn);
