...
static void test_beamnet_on(wtk_wavfile_t *wav,short *data,int len)
{
	int i;
...
static void beamnet_test_file(wtk_beamnet_t *beamnet,char *ifn,char *ofn)
{
    wtk_riff_t *riff;
    wtk_wavfile_t *wav;
    short *pv;
    ...
    wav=wtk_wavfile_new(beamnet->cfg->rate);
    wav->max_pend=0;
	...
	wtk_beamnet_start(beamnet, theta, 0.0);
    wtk_beamnet_set_notify(beamnet,wav,(wtk_beamnet_notify_f)test_beamnet_on);
    ...
    while(1)
    {
        ...
        cnt+=len;
        wtk_beamnet_feed(beamnet,pv,len,0);
		// wtk_debug("deley %f\n",(cnt-out_len)/16.0);
    }
    wtk_beamnet_feed(beamnet,NULL,0,1);
    t=time_get_ms()-t;
...
{
	wtk_beamnet_t *beamnet=NULL;
	wtk_beamnet_cfg_t *cfg=NULL;
	wtk_arg_t *arg;
	char *cfg_fn=NULL;
	char *bin_fn=NULL;
	char *scp=NULL;
	...
	if(cfg_fn)
	{
		cfg=wtk_beamnet_cfg_new(cfg_fn);
		if(cfg==NULL)
		{
			print_usage();
			goto end;
	...
	{
		cfg=wtk_beamnet_cfg_new_bin(bin_fn);
		if(cfg==NULL)
		{
			print_usage();
			goto end;
	...
	if(wav)
	{
		...
		for(i=0;i<1;++i)
		{
			beamnet_test_file(beamnet,wav,output);
		}
//		beamnet_test_file(beamnet,wav,"2.wav");
//		beamnet_test_file(beamnet,wav,"3.wav");
	}
	if(scp)
	{
		wtk_flist_it_t *it;
		...
		while(1)
		{
			...
			{
				beamnet_test_file(beamnet,ifn,buf->data);
			}
			wtk_string_delete(v);
		}
	...
	if(beamnet)
	{
		wtk_beamnet_delete(beamnet);
	}
	if(cfg)
	{
		if(cfg_fn)
		{
			wtk_beamnet_cfg_delete(cfg);
		}else
		{
			wtk_beamnet_cfg_delete_bin(cfg);
		}
	}
	if(arg)
	{
