...
static void test_cmask_aec2_on(wtk_wavfile_t *wav,short *data,int len)
{
	int i;
...
static void test_cmask_aec2_on_ssl(void *ths,float ts, float te,wtk_ssl2_extp_t *nbest_extp,int nbest)
{
    int i;
...
static void cmask_aec2_test_file(wtk_cmask_aec2_t *cmask_aec2,char *ifn,char *ofn)
{
    wtk_riff_t *riff;
    wtk_wavfile_t *wav;
    short *pv;
    ...
    wav=wtk_wavfile_new(cmask_aec2->cfg->rate);
    wav->max_pend=0;
	...
	wtk_cmask_aec2_start(cmask_aec2);
    wtk_cmask_aec2_set_notify(cmask_aec2,wav,(wtk_cmask_aec2_notify_f)test_cmask_aec2_on);
    wtk_cmask_aec2_set_ssl_notify(cmask_aec2,wav,(wtk_cmask_aec2_notify_ssl_f)test_cmask_aec2_on_ssl);
    ...
    while(1)
    {
        ...
        cnt+=len;
        wtk_cmask_aec2_feed(cmask_aec2,pv,len,0);
		// wtk_debug("deley %f\n",(cnt-out_len)/16.0);
    }
    wtk_cmask_aec2_feed(cmask_aec2,NULL,0,1);
...
{
	wtk_cmask_aec2_t *cmask_aec2=NULL;
	wtk_cmask_aec2_cfg_t *cfg=NULL;
	wtk_arg_t *arg;
	char *cfg_fn=NULL;
	char *bin_fn=NULL;
	char *scp=NULL;
	...
	if(cfg_fn)
	{
		cfg=wtk_cmask_aec2_cfg_new(cfg_fn);
		if(cfg==NULL)
		{
			print_usage();
			goto end;
	...
	{
		cfg=wtk_cmask_aec2_cfg_new_bin(bin_fn);
		if(cfg==NULL)
		{
			print_usage();
			goto end;
	...
	cmask_aec2=wtk_cmask_aec2_new(cfg);
	if(wav)
	{
		int i;
		...
		for(i=0;i<1;++i)
		{
			cmask_aec2_test_file(cmask_aec2,wav,output);
		}
//		cmask_aec2_test_file(cmask_aec2,wav,"2.wav");
//		cmask_aec2_test_file(cmask_aec2,wav,"3.wav");
	}
	if(scp)
	{
		wtk_flist_it_t *it;
		...
		while(1)
		{
			...
			{
				cmask_aec2_test_file(cmask_aec2,ifn,buf->data);
			}
			wtk_string_delete(v);
		}
	...
	if(cmask_aec2)
	{
		wtk_cmask_aec2_delete(cmask_aec2);
	}
	if(cfg)
	{
		if(cfg_fn)
		{
			wtk_cmask_aec2_cfg_delete(cfg);
		}else
		{
			wtk_cmask_aec2_cfg_delete_bin(cfg);
		}
	}
	if(arg)
	{
