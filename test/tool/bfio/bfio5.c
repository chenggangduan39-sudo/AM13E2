...
static void test_bfio5_on_ssl2(wtk_bfio5_t *bfio5, wtk_ssl2_extp_t *nbest_extp, int nbest)
{
    int i;
...
static void test_bfio5_on(wtk_bfio5_t *bfio5, wtk_bfio5_cmd_t cmd, short *data, int len)
{
    static int ki = 0;
    static int ki_w = 0;
    ...
    case WTK_BFIO5_VAD_START:
        ...
        if(save_asr){
            ++ki;
            wlog = wtk_wavfile_new(bfio5->cfg->rate);
            wlog->max_pend = 0;
    ...
    case WTK_BFIO5_VAD_END:
        ...
        break;
    case WTK_BFIO5_WAKE:
        printf("%f %f %f %d\n", bfio5->wake_fs + bfio5->cfg->wake_ssl_fs, bfio5->wake_fe + bfio5->cfg->wake_ssl_fe, bfio5->out_wake_prob, bfio5->wake_key);
        fflush(stdout);
        fs = (int)floor((bfio5->wake_fs + bfio5->cfg->wake_ssl_fs)*bfio5->cfg->rate);
        f_len = (int)floor(((bfio5->wake_fe + bfio5->cfg->wake_ssl_fe)-(bfio5->wake_fs + bfio5->cfg->wake_ssl_fs))*bfio5->cfg->rate);
        if(wake_buf){
            printf("%d %d %d\n", fs, f_len, wake_buf->pos);
            wake_wav=wtk_wavfile_new(bfio5->cfg->rate);
            wake_wav->max_pend = 0;
            ++ki_w;
    ...
    case WTK_BFIO5_VAD_CANCEL:
        ...
        break;
    case WTK_BFIO5_ASR_CANCEL:
        // wtk_debug("asr cancle len=%d out_len=%d\n", len, out_len);
        break;
    case WTK_BFIO5_ASR_RES:
        ...
        // printf("[asr] %f %f %.*s/%f\n", bfio5->vad_ts, bfio5->vad_te,
        //        res->v.str->len, res->v.str->data, conf->v.number);
        // break;
    case WTK_BFIO5_WAKE_RES:
        break;
...
static void test_bfio5_file2(wtk_bfio5_t *bfio5, char *scp, char *odir)
{
    wtk_flist_it_t *it;
    wtk_riff_t *riff;
    wtk_wavfile_t *wav;
    ...
    wav = wtk_wavfile_new(bfio5->cfg->rate);
    wav->max_pend = 0;
    riff = wtk_riff_new();
    ...
    wtk_bfio5_set_notify(bfio5, bfio5, (wtk_bfio5_notify_f)test_bfio5_on);
    wtk_bfio5_start(bfio5);
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            wtk_bfio5_feed(bfio5, data, len, 0);
        }

        wtk_riff_close(riff);
        wtk_wavfile_close(wav);
    }
    wtk_bfio5_feed(bfio5, NULL, 0, 1);
...
static void test_bfio5_file(wtk_bfio5_t *bfio5, char *ifn, char *ofn)
{
    wtk_riff_t *riff;
    short *pv;
    short **data;
    ...
    out_wav = wtk_wavfile_new(bfio5->cfg->rate);
    out_wav->max_pend = 0;
    ...
    wtk_bfio5_set_notify(bfio5, bfio5, (wtk_bfio5_notify_f)test_bfio5_on);
    wtk_bfio5_set_ssl2_notify(bfio5, bfio5, (wtk_bfio5_notify_ssl2_f)test_bfio5_on_ssl2);
    wtk_bfio5_start(bfio5);
    // wtk_bfio5_set_wake_words(bfio5, "小爱同学|天猫精灵", strlen("小爱同学|天猫精灵"));
    wtk_bfio5_set_wake_words(bfio5, "天猫精灵", strlen("天猫精灵"));
    ...
    while (1)
    {
        ...
        // if(cnt==0){  // 0s
        //     // start kws reg
        //     wtk_bfio5_set_offline_qform(bfio5, 1000, -1);
        // }
        // if(cnt==160000){  // 10s
        //     // end kws reg
        //     wtk_bfio5_feed(bfio5, NULL, 0, 1);
...
int main(int argc, char **argv)
{
    ...
    if (cfg_fn)
    {
        cfg = wtk_bfio5_cfg_new(cfg_fn);
    }
    if (bin_fn)
    {
        cfg = wtk_bfio5_cfg_new_bin(bin_fn);
    }
    // wtk_bfio5_cfg_set_wakeword(cfg, "小爱同学");
    bfio5 = wtk_bfio5_new(cfg);

    if (scp)
    {
        test_bfio5_file2(bfio5, scp, ofn);
    }
    else
    {
        test_bfio5_file(bfio5, ifn, ofn);
    }

    wtk_bfio5_delete(bfio5);
end:
    if (arg)
    {
        wtk_arg_delete(arg);
    ...
    if (bin_fn)
    {
        wtk_bfio5_cfg_delete_bin(cfg);
    }
    if (cfg_fn)
    {
        wtk_bfio5_cfg_delete(cfg);
    }
    return 0;
}
