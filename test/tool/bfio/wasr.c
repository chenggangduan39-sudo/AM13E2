...
static void _test(qtk_wasr_t *w, char *wav) {
    wtk_riff_t *riff;
    int c, nchan;
    wtk_strbuf_t **audio;
    short *d;
    ...
    d = cast(short *, audio[0]->data);
    len = audio[0]->pos / 2;

    while (len > 0) {
        int step = MIN(len, 512);
        qtk_wasr_feed(w, d, step);
...
int main(int argc, char *argv[]) {
    ...
    main_cfg = wtk_main_cfg_new_type(qtk_wasr_cfg, cfn);

    w = qtk_wasr_new(cast(qtk_wasr_cfg_t *, main_cfg->cfg));
    qtk_wasr_set_notify(w, cast(qtk_wasr_notify_t, _on_wasr), jsp);
    _test(w, wav);
    qtk_wasr_delete(w);
