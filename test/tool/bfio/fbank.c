...
static int idx=0;
void test_fbank_on(wtk_fbank_t *fbank, float *data, int len)
{
	int i;
	for(i=0;i<len;++i){
		printf("%f\n",data[i]);
...
static void fbank_test_file(wtk_fbank_t *fbank,char *ifn,char *ofn)
{
    wtk_riff_t *riff;
    wtk_wavfile_t *wav;
    short *pv;
    ...
    wav=wtk_wavfile_new(fbank->cfg->kaldi_fbank.sample_frequency);
    wav->max_pend=0;
	...
	wtk_fbank_set_notify(fbank,fbank,(wtk_fbank_notify_f)test_fbank_on);
    ...
    while(1)
    {
        ...
        cnt+=len;
        wtk_fbank_feed(fbank,pv,len,0);
		// wtk_debug("deley %f\n",(cnt-out_len)/16.0);
    }
    wtk_fbank_feed(fbank,NULL,0,1);
    t=time_get_ms()-t;
...
{
	wtk_fbank_t *fbank=NULL;
	wtk_fbank_cfg_t *cfg=NULL;
	wtk_arg_t *arg;
	char *cfg_fn=NULL;
	char *bin_fn=NULL;
	char *scp=NULL;
	...
	{
		cfg=wtk_fbank_cfg_new_bin(bin_fn);
		if(cfg==NULL)
		{
			print_usage();
			goto end;
	...
	if(wav)
	{
		...
		for(i=0;i<1;++i)
		{
			fbank_test_file(fbank,wav,output);
		}
	}
	if(scp)
	{
		...
		while(1)
		{
			...
			{
				fbank_test_file(fbank,ifn,buf->data);
			}
			wtk_string_delete(v);
		}
	...
	if(fbank)
	{
		wtk_fbank_delete(fbank);
	}
	if(cfg)
	{
		if(cfg_fn)
		{
			wtk_fbank_cfg_delete(cfg);
		}else
		{
			wtk_fbank_cfg_delete_bin(cfg);
		}
	}
	if(arg)
	{
