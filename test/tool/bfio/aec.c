...
static void test_aec_on(wtk_wavfile_t *wav,short **data,int len,int is_end)
{
    if(len>0)
    {
...
static void test_aec_file(wtk_aec_t *aec,char *ifn,char *ofn)
{
    wtk_riff_t *riff;
    wtk_wavfile_t *wav;
    short *pv;
    short **data;
    int channel=aec->cfg->stft.channel;
    int len,bytes;
    int ret;
    int i,j;
    double t;
    ...
    wtk_wavfile_set_channel(wav, aec->channel);
    ...
    wtk_aec_set_notify(aec,wav,(wtk_aec_notify_f)test_aec_on);
    ...
    while(1)
    {
        ...
        cnt+=len;
        wtk_aec_feed(aec,data,len,0);
    }
    wtk_aec_feed(aec,NULL,0,1);
...
int main(int argc,char **argv)
{
    ...
    main_cfg=wtk_main_cfg_new_type(wtk_aec_cfg,cfg_fn);
