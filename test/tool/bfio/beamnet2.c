...
static void test_beamnet2_on(wtk_wavfile_t *wav,short *data,int len)
{
	int i;
...
static void beamnet2_test_file(wtk_beamnet2_t *beamnet2,char *ifn,char *ofn)
{
    wtk_riff_t *riff;
    wtk_wavfile_t *wav;
    short *pv;
    ...
    wav=wtk_wavfile_new(beamnet2->cfg->rate);
    wav->max_pend=0;
	...
	wtk_beamnet2_start(beamnet2, theta, 0.0);
    wtk_beamnet2_set_notify(beamnet2,wav,(wtk_beamnet2_notify_f)test_beamnet2_on);
    ...
    while(1)
    {
        ...
        cnt+=len;
        wtk_beamnet2_feed(beamnet2,pv,len,0);
		// wtk_debug("deley %f\n",(cnt-out_len)/16.0);
    }
    wtk_beamnet2_feed(beamnet2,NULL,0,1);
    t=time_get_ms()-t;
...
{
	wtk_beamnet2_t *beamnet2=NULL;
	wtk_beamnet2_cfg_t *cfg=NULL;
	wtk_arg_t *arg;
	char *cfg_fn=NULL;
	char *bin_fn=NULL;
	char *scp=NULL;
	...
	if(cfg_fn)
	{
		cfg=wtk_beamnet2_cfg_new(cfg_fn);
		if(cfg==NULL)
		{
			print_usage();
			goto end;
	...
	{
		cfg=wtk_beamnet2_cfg_new_bin(bin_fn);
		if(cfg==NULL)
		{
			print_usage();
			goto end;
	...
	if(wav)
	{
		...
		for(i=0;i<1;++i)
		{
			beamnet2_test_file(beamnet2,wav,output);
		}
//		beamnet2_test_file(beamnet2,wav,"2.wav");
//		beamnet2_test_file(beamnet2,wav,"3.wav");
	}
	if(scp)
	{
		wtk_flist_it_t *it;
		...
		while(1)
		{
			...
			{
				beamnet2_test_file(beamnet2,ifn,buf->data);
			}
			wtk_string_delete(v);
		}
	...
	if(beamnet2)
	{
		wtk_beamnet2_delete(beamnet2);
	}
	if(cfg)
	{
		if(cfg_fn)
		{
			wtk_beamnet2_cfg_delete(cfg);
		}else
		{
			wtk_beamnet2_cfg_delete_bin(cfg);
		}
	}
	if(arg)
	{
