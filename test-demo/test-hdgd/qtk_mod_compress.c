...
#ifndef OFFLINE_TEST
int qtk_mod_compress_rcd_entry(qtk_mod_compress_t *m, wtk_thread_t *t);
int qtk_mod_compress_lineout_entry(qtk_mod_compress_t *m, wtk_thread_t *t);
#endif
int qtk_mod_compress_uart_entry(qtk_mod_compress_t *m, wtk_thread_t *t);
int qtk_mod_compress_uart_read_entry(qtk_mod_compress_t *m, wtk_thread_t *t);
int qtk_mod_compress_compress_entry(qtk_mod_compress_t *m, wtk_thread_t *t);
int qtk_mod_compress_decompress_entry(qtk_mod_compress_t *m, wtk_thread_t *t);
int qtk_mod_compress_savepcm_entry(qtk_mod_compress_t *m, wtk_thread_t *t);
static void qtk_log_wav_file_new(qtk_mod_compress_t *m);
static void qtk_log_wav_file_delete(qtk_mod_compress_t *m);
void qtk_mod_compress_is_log_audio(qtk_mod_compress_t *m);
void qtk_mod_compress_set_cpu(qtk_mod_compress_t *m, wtk_thread_t *thread, int cpunum);

int qtk_mod_compress_engine_new(qtk_mod_compress_t *m, int type);
void qtk_mod_compress_engine_delete(qtk_mod_compress_t *m, int type);
void qtk_mod_compress_engine_start(qtk_mod_compress_t *m, int type);
void qtk_mod_compress_engine_stop(qtk_mod_compress_t *m, int type);
int qtk_mod_compress_engine_feed(qtk_mod_compress_t *m, int type, char *data, int len);

int pushlen=5*16*1000;
int mul_cnt=0;

int qtk_mod_compress_msg_bytes(qtk_msg_node_t *node)
{
	int bytes=0;
...
int qtk_mod_compress_bytes(qtk_mod_compress_t *m)
{
	...
	// if(m->cfg->audio_type == qtk_mod_compress_BF_COMPRESS)
	// {
	// 	bytes+=qtk_signal_compress_bytes(m->compress);
	// }
#ifndef OFFLINE_TEST
	if(m->rcd)
	{
...
#endif
	tmp=wtk_hoard_bytes((wtk_hoard_t *)(&(m->msg->msg_hoard)),(wtk_hoard_bytes_f)qtk_mod_compress_msg_bytes);
	bytes+=tmp;
	bytes+=sizeof(qtk_mod_compress_t);
...
qtk_mod_compress_t *qtk_mod_compress_new(qtk_mod_compress_cfg_t *cfg)
{
	...
	m = (qtk_mod_compress_t *)wtk_calloc(1, sizeof(*m));
	m->cfg = cfg;
	...
	if(m->cfg->cache_path.len > 0){
		...
		wtk_thread_init(&m->savepcm_t,(thread_route_handler)qtk_mod_compress_savepcm_entry, m);
	}
	...
	if(m->cfg->use_lineout){
		...
		wtk_thread_init(&m->lineout_t,(thread_route_handler)qtk_mod_compress_lineout_entry, m);
...
//    	goto end;
//	}
	wtk_thread_init(&m->rcd_t, (thread_route_handler)qtk_mod_compress_rcd_entry, m);
#endif
#endif

	m->msg = qtk_msg_new();
	wtk_thread_init(&m->compress_t,(thread_route_handler)qtk_mod_compress_compress_entry, m);
	wtk_blockqueue_init(&m->compress_queue);

	wtk_thread_init(&m->decompress_t,(thread_route_handler)qtk_mod_compress_decompress_entry, m);
	wtk_blockqueue_init(&m->decompress_queue);

	ret = qtk_mod_compress_engine_new(m, 1);
	if(ret!=0)
	{
		wtk_log_err(m->log, "audio engine new fiald! type=%d", m->cfg->audio_type);
		goto end;
	}
	ret = qtk_mod_compress_engine_new(m, 2);
	if(ret!=0)
	{
	...
	m->decompresswav = wtk_wavfile_new(8000);
	wtk_sem_init(&m->rcdstart_sem, 0);

	if(m->cfg->use_uart)
	{
		wtk_blockqueue_init(&m->uart_queue);
		wtk_thread_init(&m->uart_t, (thread_route_handler)qtk_mod_compress_uart_entry, m);
		wtk_thread_init(&m->uart_read_t, (thread_route_handler)qtk_mod_compress_uart_read_entry, m);
...
{
	wtk_blockqueue_clean(&m->compress_queue);
	wtk_blockqueue_clean(&m->decompress_queue);
	...
	qtk_mod_compress_engine_delete(m, 1);
	qtk_mod_compress_engine_delete(m, 2);
	...
	if(m->decompresswav)
	{
		wtk_wavfile_delete(m->decompresswav);
	}
...
#endif
#endif
	qtk_mod_compress_engine_start(m, 1);
	m->compress_run = 1;
	wtk_thread_start(&m->compress_t);

	qtk_mod_compress_engine_start(m, 2);
	m->decompress_run = 1;
	wtk_thread_start(&m->decompress_t);
...
void qtk_mod_compress_stop(qtk_mod_compress_t *m)
{
	...
	m->compress_run = 0;
	wtk_blockqueue_wake(&m->compress_queue);
	wtk_thread_join(&m->compress_t);
	qtk_mod_compress_engine_stop(m, 1);

	m->decompress_run = 0;
	wtk_blockqueue_wake(&m->decompress_queue);
	wtk_thread_join(&m->decompress_t);
	qtk_mod_compress_engine_stop(m, 2);
...
void qtk_mod_compress_clean_queue2(qtk_mod_compress_t *m, wtk_blockqueue_t *queue,int nx)
{
...
void qtk_mod_compress_clean_queue(qtk_mod_compress_t *m, wtk_blockqueue_t *queue)
{
	qtk_mod_compress_clean_queue2(m,queue,0);
}

int qtk_mod_compress_grep_start2(char *data, int len, int *alen)
{
	int pos=0;
...
int qtk_mod_compress_grep_end(char *data, int len, int *alen)
{
	int pos=0;
...
int qtk_mod_compress_uart_read_entry(qtk_mod_compress_t *m, wtk_thread_t *t)
{
	...
	qtk_mode_bf_type type=qtk_mod_compress_UART_RECV_START;
	...
	while(m->uart_read_run){
		...
		if(ret > 0)
		{
			...
			switch (type)
			{
			case qtk_mod_compress_UART_RECV_START:
				pos = qtk_mod_compress_grep_start2(outbuf->data, outbuf->pos, &alen);
				if(pos >= 0)
				{
					datalen = pos;
					if(outbuf->pos >= pos+alen+6+6)
					{
						...
						datalen = 0;
						type=qtk_mod_compress_UART_RECV_END;
					}else{
						...
						type=qtk_mod_compress_UART_RECV_DATA;
					}
				}else{
					wtk_strbuf_pop(outbuf, NULL, alen-4);
				}
				break;
			case qtk_mod_compress_UART_RECV_DATA:
				pos = qtk_mod_compress_grep_end(outbuf->data, outbuf->pos, &alen);
				if(pos >= 0)
				{
					// wtk_debug("end=[");
					// int i;
			...
			if(type == qtk_mod_compress_UART_RECV_END)
			{
				wtk_debug("recv data len=%d\n", databuf->pos);
				//TODO push data to decompress queue
				msg_node = qtk_msg_pop_node(m->msg);
				wtk_strbuf_push(msg_node->buf, databuf->data, databuf->pos);
				wtk_blockqueue_push(&m->decompress_queue, &msg_node->qn);
				wtk_strbuf_reset(databuf);
				type=qtk_mod_compress_UART_RECV_START;
			}
		}
	}
	if(recv_buf)
...
void qtk_mod_compress_feed(qtk_mod_compress_t *m, char *data, int len)
{
	...
	wtk_blockqueue_push(&m->compress_queue, &msg_node->qn);
...
int qtk_mod_compress_rcd_entry(qtk_mod_compress_t *m, wtk_thread_t *t)
{
	...
	qtk_mod_compress_set_cpu(m, t, 1);
...
int qtk_mod_decompress_grep_str(char *str, char *data, int len)
{
	int pos=0;
	char *head=str;
...
int qtk_mod_compress_decompress_entry(qtk_mod_compress_t *m, wtk_thread_t *t)
{
	...
	qtk_mod_compress_set_cpu(m, t, 2);

	while(m->decompress_run){
		qn= wtk_blockqueue_pop(&m->decompress_queue,-1,NULL);
		if(!qn) {
			continue;
		}
		msg_node = data_offset2(qn,qtk_msg_node_t,qn);
		if(m->cfg->debug)
		{
			if(m->decompress_queue.length > 2)
			{
				wtk_debug("compress =============>>%d\n",m->decompress_queue.length);
				wtk_log_log(m->log, "compress =============>>%d",m->decompress_queue.length);
			}
		}
		wtk_strbuf_push(tmpbuf, msg_node->buf->data, msg_node->buf->pos);
		if(is_start)
		{
			ret = qtk_mod_decompress_grep_str(headstr, tmpbuf->data, tmpbuf->pos);
			if(ret >= 0)
			{
				int start_pos=ret;
				ret = qtk_mod_decompress_grep_str(endstr, tmpbuf->data, tmpbuf->pos);
		...
		if(is_end)
		{
			...
			qtk_signal_decompress_feed(m->decompress, (int64_t *)(decompressbuf->data+sizeof(int64_t)*4), codec_len);
			qtk_signal_decompress_get_result(m->decompress);
			qtk_signal_decompress_reset(m->decompress);
			wtk_strbuf_reset(decompressbuf);
			is_end = 0;
		}
...
int qtk_mod_compress_compress_entry(qtk_mod_compress_t *m, wtk_thread_t *t)
{
	...
	wtk_queue_node_t *qn;
	char zdata[1024]={0};
	wtk_strbuf_t *compressbuf=wtk_strbuf_new(1024,0);
	int flen=0,i;
	int compress_len=0;

	qtk_mod_compress_set_cpu(m, t, 3);

	while(m->compress_run){
		qn= wtk_blockqueue_pop(&m->compress_queue,-1,NULL);
		if(!qn) {
			continue;
		}
		msg_node = data_offset2(qn,qtk_msg_node_t,qn);
		if(m->cfg->debug)
		{
			if(m->compress_queue.length > 2)
			{
				wtk_debug("compress =============>>%d\n",m->compress_queue.length);
				wtk_log_log(m->log, "compress =============>>%d",m->compress_queue.length);
			}
		}
		...
		if(m->cfg->use_log_wav || m->log_audio)
		{
			...
			msg_node3->type = qtk_mod_compress_BF_SAVE_SOURCE;
			wtk_strbuf_push(msg_node3->buf, msg_node->buf->data, msg_node->buf->pos);
			wtk_blockqueue_push(&m->savepcm_queue, &msg_node3->qn);
		}
		//TODO push data to compress engine
		qtk_signal_compress_feed(m->compress, msg_node->buf->data, msg_node->buf->pos>>1);
		if(flen >= pushlen)
		{
			flen=0;
			int64_t *data;
			...
			int data_cnt;
			int64_t adatalen=0;
			//TODO get data from compress engine
			printf("============== Compress start ======================\n");
			data = qtk_signal_compress_get_result(m->compress, &data_len, &data_cnt);
			printf("============== Compress end   ======================\n");
			// wtk_debug("=========================>>>>>>>>>>>>>>>>>>>>>>>>>compress end datacnt=%d data_len=%d/%d/%d/%d\n",data_cnt,data_len[0],data_len[1],data_len[2],data_len[3]);
			wtk_strbuf_push(compressbuf, "start", strlen("start"));
			wtk_strbuf_push(compressbuf, (char *)data_len, sizeof(int64_t)*data_cnt);
			// int j;
			...
			wtk_strbuf_push(compressbuf, (char *)data, adatalen*sizeof(int64_t));
			wtk_strbuf_push(compressbuf, "end", strlen("end"));

			file_write_buf("./tmp.txt", compressbuf->data+5, compressbuf->pos-8);

			// wtk_debug("compress len=%d\n",compressbuf->pos);
			i=compressbuf->pos%1024;
			if(i!=0)
			{
				wtk_strbuf_push(compressbuf, zdata, 1024-i);
			}
			// wtk_debug("compress len=%d\n",compressbuf->pos);
			compress_len=compressbuf->pos;
			char *ss=compressbuf->data;
			char *ee=ss+compressbuf->pos;
			int step=1024;
			while(ss<ee)
			{
			...
			wtk_strbuf_reset(compressbuf);
			qtk_signal_compress_reset(m->compress);
			qtk_log_wav_file_delete(m);
...
int qtk_mod_compress_lineout_entry(qtk_mod_compress_t *m, wtk_thread_t *t)
{
	...
	while(m->lineout_run){
		...
		if(msg_node->type == 2)
		{
			snprintf(pcmd, sizeof(pcmd), "aplay -Dhw:%s decompress.wav", m->cfg->lineout.snd_name);
		}else{
...
int qtk_mod_compress_savepcm_entry(qtk_mod_compress_t *m, wtk_thread_t *t)
{
	...
	qtk_mod_compress_set_cpu(m, t, 0);
	...
	while(m->savepcm_run){
		...
		{
		case qtk_mod_compress_BF_SAVE_SOURCE:
			if(m->mul){
...
void qtk_mod_compress_on_decompress(qtk_mod_compress_t *m, short *data, int len)
{
	if(m->decompresswav)
	{
		wtk_wavfile_open(m->decompresswav, "decompress.wav");
		wtk_wavfile_write(m->decompresswav, (char*)data, len<<1);
		wtk_wavfile_close(m->decompresswav);
	}
...
int qtk_mod_compress_engine_new(qtk_mod_compress_t *m, int type)
{
	int ret;
	switch(type){
	case qtk_mod_compress_BF_COMPRESS:
		printf("MOD BF COMPRESS resname=%.*s\n",m->cfg->compress_fn.len,m->cfg->compress_fn.data);
		wtk_log_log(m->log, "MOD BF COMPRESS resname=%.*s",m->cfg->compress_fn.len,m->cfg->compress_fn.data);
		if(m->cfg->use_bin)
		{
			m->compress_cfg = qtk_signal_compress_cfg_new_bin(m->cfg->compress_fn.data);
		}else{
			m->compress_cfg = qtk_signal_compress_cfg_new(m->cfg->compress_fn.data);
		}
		if(!m->compress_cfg){
			ret = -1;
			goto end;
		}
		m->compress = qtk_signal_compress_new(m->compress_cfg);
		if(!m->compress){
			ret = -1;goto end;
		}
		break;
	case qtk_mod_compress_BF_DECOMPRESS:
		printf("MOD BF DECOMPRESS resname=%.*s\n",m->cfg->decompress_fn.len,m->cfg->decompress_fn.data);
		wtk_log_log(m->log, "MOD BF DECOMPRESS resname=%.*s",m->cfg->decompress_fn.len,m->cfg->decompress_fn.data);
		if(m->cfg->use_bin){
			m->decompress_cfg = qtk_signal_decompress_cfg_new_bin(m->cfg->decompress_fn.data);
		}else{
			m->decompress_cfg = qtk_signal_decompress_cfg_new(m->cfg->decompress_fn.data);
		}
		if(!m->decompress_cfg)
		{
			ret=-1;
		...
		m->decompress = qtk_signal_decompress_new(m->decompress_cfg);
		if(!m->decompress)
		{
			ret=-1;
			goto end;
		}
		qtk_signal_decompress_set_notify(m->decompress, m, (qtk_signal_decompress_notify_f)qtk_mod_compress_on_decompress);
		break;
	default:
...
void qtk_mod_compress_engine_delete(qtk_mod_compress_t *m, int type)
{
	...
	case qtk_mod_compress_BF_COMPRESS:
		if(m->compress){
			qtk_signal_compress_delete(m->compress);
		}
		if(m->compress_cfg){
			if(m->cfg->use_bin){
				qtk_signal_compress_cfg_delete_bin(m->compress_cfg);
			}else{
				qtk_signal_compress_cfg_delete(m->compress_cfg);
			}
		}
		break;
	case qtk_mod_compress_BF_DECOMPRESS:
		if(m->decompress)
		{
			qtk_signal_decompress_delete(m->decompress);
		}
		if(m->decompress_cfg)
		{
			if(m->cfg->use_bin){
				qtk_signal_decompress_cfg_delete_bin(m->decompress_cfg);
			}else{
				qtk_signal_decompress_cfg_delete(m->decompress_cfg);
			}
		}
		break;
	default:
...
void qtk_mod_compress_engine_start(qtk_mod_compress_t *m, int type)
{
	...
	case qtk_mod_compress_BF_COMPRESS:
		// if(m->compress){
		// 	qtk_signal_compress_start(m->compress);
		// }
		break;
	case qtk_mod_compress_BF_DECOMPRESS:
		// if(m->decompress)
		// {
		// 	qtk_signal_decompress_start(m->decompress);
		// }
		break;
	default:
		break;
...
int qtk_mod_compress_engine_feed(qtk_mod_compress_t *m, int type, char *data, int len)
{
	...
	case qtk_mod_compress_BF_COMPRESS:
		if(m->compress){
			qtk_signal_compress_feed(m->compress, data, len);
		}
		break;
	case qtk_mod_compress_BF_DECOMPRESS:
		if(m->decompress)
		{
			qtk_signal_decompress_feed(m->decompress, data, len);
		}
		break;
	default:
		break;
