...
int qtk_mod_am13e2_cfg_init(qtk_mod_am13e2_cfg_t *cfg)
{
    ...
    wtk_string_set(&cfg->lineout_check_path, 0, 0);

    wtk_string_set(&cfg->gainnetbf_fn, 0, 0);
    wtk_string_set(&cfg->gainnetbf2_fn, 0, 0);
    wtk_string_set(&cfg->gainnetbf3_fn, 0, 0);
    wtk_string_set(&cfg->gainnetbf4_fn, 0, 0);
    ...
    cfg->gainnetbf2_cfg = NULL;
    cfg->gainnetbf3_cfg = NULL;
    cfg->gainnetbf4_cfg = NULL;
    ...
    cfg->echo_shift = 1.0;
    cfg->uac_shift = 1.0;
    cfg->in_inputtolineout_shift = 1.0;
    cfg->in_inputtospeaker_shift = 1.0;
    cfg->sleep_time = 3600;
    cfg->use_dev = 1;
    ...
    cfg->use_onlylineout = 0;
    cfg->use_onlylineinspk = 0;
    cfg->use_onlylineinout = 0;
    cfg->volum_output_mute = 0;
    cfg->volum_input_mute = 0;
    return 0;
}
...
int qtk_mod_am13e2_cfg_clean(qtk_mod_am13e2_cfg_t *cfg)
{
    ...
    if(cfg->gainnetbf2_cfg){
        qtk_gainnetbf_cfg_delete(cfg->gainnetbf2_cfg);
    }
    if(cfg->gainnetbf3_cfg){
        qtk_gainnetbf_cfg_delete(cfg->gainnetbf3_cfg);
    }
    if(cfg->gainnetbf4_cfg){
        qtk_gainnetbf_cfg_delete(cfg->gainnetbf4_cfg);
    }
    if(cfg->vboxebf_cfg){
    ...
    if(cfg->uart_cfg){
        qtk_uart_client_cfg_delete(cfg->uart_cfg);
    }
...
int qtk_mod_am13e2_cfg_update_local(qtk_mod_am13e2_cfg_t *cfg,wtk_local_cfg_t *main)
{
    ...
    wtk_local_cfg_update_cfg_string_v(main, cfg, lineout_check_path, v);

    wtk_local_cfg_update_cfg_string_v(main, cfg, gainnetbf_fn, v);
    wtk_local_cfg_update_cfg_string_v(main, cfg, gainnetbf2_fn, v);
    wtk_local_cfg_update_cfg_string_v(main, cfg, gainnetbf3_fn, v);
    wtk_local_cfg_update_cfg_string_v(main, cfg, gainnetbf4_fn, v);
    ...
    wtk_local_cfg_update_cfg_b(main, cfg, volum_output_mute, v);

    wtk_local_cfg_update_cfg_b(main, cfg, volum_input_mute, v);

    wtk_local_cfg_update_cfg_b(main, cfg, use_onlylineout, v);
    wtk_local_cfg_update_cfg_b(main, cfg, use_onlylineinspk, v);
    wtk_local_cfg_update_cfg_b(main, cfg, use_onlylineinout, v);
    ...
    wtk_local_cfg_update_cfg_f(main, cfg, uac_shift, v);

    wtk_local_cfg_update_cfg_f(main, cfg, in_inputtolineout_shift, v);
    wtk_local_cfg_update_cfg_f(main, cfg, in_inputtospeaker_shift, v);
...
int qtk_mod_am13e2_cfg_update(qtk_mod_am13e2_cfg_t *cfg)
{
    ...
    if(cfg->gainnetbf2_fn.len > 0){
        ret = -1;
        cfg->gainnetbf2_cfg = qtk_gainnetbf_cfg_new(cfg->gainnetbf2_fn.data);
        if(cfg->gainnetbf2_cfg){ret = 0;}
    }
    if(cfg->gainnetbf3_fn.len > 0){
        ret = -1;
        cfg->gainnetbf3_cfg = qtk_gainnetbf_cfg_new(cfg->gainnetbf3_fn.data);
        if(cfg->gainnetbf3_cfg){ret = 0;}
    }
    if(cfg->gainnetbf4_fn.len > 0){
        ret = -1;
        cfg->gainnetbf4_cfg = qtk_gainnetbf_cfg_new(cfg->gainnetbf4_fn.data);
        if(cfg->gainnetbf4_cfg){ret = 0;}
    }
    ...
    if(cfg->uart_fn.len > 0){
        cfg->uart_cfg = qtk_uart_client_cfg_new(cfg->uart_fn.data);
        if(cfg->uart_cfg){ret=0;}
    }
    return ret;
}
