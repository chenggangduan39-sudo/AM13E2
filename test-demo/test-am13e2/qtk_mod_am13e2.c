...
int qtk_mod_am13e2_gainnet2_entry(qtk_mod_am13e2_t *m, wtk_thread_t *t)
{
	...
	while(m->gainnet2_run){
		...
		// wtk_debug("==========>>>>>pos = %d,lineincourseware.pos=%d\n",pos,lineincourseware->pos);
		if(!m->cfg->use_mainlineout && !m->cfg->use_wooflineout && !m->cfg->use_expandlineout && !m->cfg->use_meetinglineout){
			...
			else if (m->cfg->use_rcd3 == 1 && uacbuf->pos == 0 && ! m->cfg->use_linein_courseware){
				// wtk_debug("-------=========================>>>>>>>>>>>\n");
				if(i2sbuf->pos >= pos){
					wtk_strbuf_reset(tbuf);
					int i=0;
...
int qtk_mod_am13e2_gainnet3_entry(qtk_mod_am13e2_t *m, wtk_thread_t *t)
{
	...
	while(m->gainnet3_run){
		...
		if(m->cfg->use_mainlineout || m->cfg->use_wooflineout || m->cfg->use_meetinglineout || m->cfg->use_expandlineout){
			...
			wtk_debug("-------->>>>>arraymul.pos=%d,i2sbuf.pos=%d,lineincourseware.pos=%d,linein_cource=%d\n",arraybuf->pos,i2sbuf->pos,lineincourseware->pos,m->cfg->use_linein_courseware);
			wtk_debug("==========>>>>>pos = %d\n",pos);
			if(m->cfg->use_meetinglineout){
				...
				// wtk_debug("------------------__>>>>>>>>>>>>>>>>>>>>\n");
					...
					wtk_debug("-------------------->>>>>>>>>>>>>>>>\n");
					if((lineincourseware->pos >= 2*pos && i2sbuf->pos >= 2*pos && arraybuf->pos >= pos)) {
						wtk_strbuf_reset(tbuf);
						wtk_strbuf_reset(ttbuf);
						int i=0,j=0;
						while(i<pos){
...
int qtk_mod_am13e2_proc_read(qtk_mod_am13e2_t *m)
{
	...
	if (once == 1)
	{
		...
		}else if (strncmp(buf, "SET_LINEOUTPATTERN", 18) == 0)//设置 lineout模式
		{
			...
			fp = fopen("/sys/class/gpio/gpio10/direction", "w");
			if (fp) {
				fprintf(fp, "%s", "out");
				fclose(fp);
			}
			fp = fopen("/sys/class/gpio/gpio10/value", "w");
			if (fp) {
				fprintf(fp, "%d", 1);
				fclose(fp);
			}
			...
			fp = fopen("/sys/class/gpio/gpio5/direction", "w");
			if (fp) {
				fprintf(fp, "%s", "out");
				fclose(fp);
			}
