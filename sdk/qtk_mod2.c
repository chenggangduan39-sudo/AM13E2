...
#ifndef OFFLINE_TEST
int qtk_mod2_rcd_entry(qtk_mod2_t *m, wtk_thread_t *t);
#endif
#ifdef USE_FOR_DEV
int qtk_mod2_uart_entry(qtk_mod2_t *m, wtk_thread_t *t);
#endif
#ifndef __ANDROID__
int qtk_mod2_usbaudio_entry(qtk_mod2_t *m, wtk_thread_t *t);
int qtk_mod2_lineout_entry(qtk_mod2_t *m, wtk_thread_t *t);
#endif
int qtk_mod2_qform_entry(qtk_mod2_t *m, wtk_thread_t *t);
int qtk_mod2_qform2_entry(qtk_mod2_t *m, wtk_thread_t *t);
int qtk_mod2_savepcm_entry(qtk_mod2_t *m, wtk_thread_t *t);
void qtk_mod2_on_vboxebf(qtk_mod2_t *mod, char *data, int len);
#ifdef USE_SSL
void qtk_mod2_on_ssl(qtk_mod2_t *mod,qtk_ssl_extp_t *extp);
#endif
void qtk_mod2_on_aec(qtk_mod2_t *m, qtk_var_t *var);
static void qtk_log_wav_file_new(qtk_mod2_t *m);
static void qtk_log_wav_file_delete(qtk_mod2_t *m);
void qtk_mod2_is_log_audio(qtk_mod2_t *m);
void qtk_mod2_set_cpu(qtk_mod2_t *m, wtk_thread_t *thread, int cpunum);
void qtk_mod2_on_resample(qtk_mod2_t *m, char *data, int len);

int qtk_mod2_engine_new(qtk_mod2_t *m, int type);
void qtk_mod2_engine_delete(qtk_mod2_t *m, int type);
void qtk_mod2_engine_start(qtk_mod2_t *m, int type);
void qtk_mod2_engine_stop(qtk_mod2_t *m, int type);
int qtk_mod2_engine_feed(qtk_mod2_t *m, int type, char *data, int len, int is_end);

static int skip_frame=100;

int qtk_mod2_msg_bytes(qtk_msg_node_t *node)
{
	int bytes=0;
...
int qtk_mod2_bytes(qtk_mod2_t *m)
{
	int bytes=0;
	int tmp=0;
	...
	// if(m->cfg->audio_type == qtk_mod2_BF_VBOXEBF)
	// {
...
#endif
	tmp=wtk_hoard_bytes((wtk_hoard_t *)(&(m->msg->msg_hoard)),(wtk_hoard_bytes_f)qtk_mod2_msg_bytes);
	bytes+=tmp;
	bytes+=sizeof(qtk_mod2_t);
	wtk_log_log(m->log, "all=%d bytes=%d hoard_length=%d/%d\n",bytes,tmp,m->msg->msg_hoard.use_length,m->msg->msg_hoard.cur_free);
	return bytes;
}

qtk_mod2_t *qtk_mod2_new(qtk_mod2_cfg_t *cfg)
{
	qtk_mod2_t *m;
	char tmp[64];
	int ret;
	...
	m = (qtk_mod2_t *)wtk_calloc(1, sizeof(*m));
	m->cfg = cfg;
	...
	if(m->cfg->cache_path.len > 0){
		...
		wtk_thread_init(&m->savepcm_t,(thread_route_handler)qtk_mod2_savepcm_entry, m);
	}
	...
	if(m->cfg->use_usbaudio){
		...
		wtk_thread_init(&m->usbaudio_t,(thread_route_handler)qtk_mod2_usbaudio_entry, m);
		m->usbaudio = qtk_play_new(&cfg->usbaudio);
	}
	if(m->cfg->use_lineout){
		wtk_blockqueue_init(&m->lineout_queue);
		wtk_thread_init(&m->lineout_t,(thread_route_handler)qtk_mod2_lineout_entry, m);
	...
	wtk_resample_set_notify(m->resample, m, (wtk_resample_notify_f)qtk_mod2_on_resample);
...
//    	goto end;
//	}
	wtk_thread_init(&m->rcd_t, (thread_route_handler)qtk_mod2_rcd_entry, m);
#endif
#endif
	m->in_resample = NULL;
	...
	m->msg = qtk_msg_new();
	wtk_debug("audio_type=%d audio2_type=%d\n",m->cfg->audio_type,m->cfg->audio2_type);
	if(m->cfg->audio_type > 0)
	{
		wtk_thread_init(&m->qform_t,(thread_route_handler)qtk_mod2_qform_entry, m);
		wtk_blockqueue_init(&m->bfio_queue);
		ret = qtk_mod2_engine_new(m,m->cfg->audio_type);
		if(ret!=0)
		{
	...
	{
		wtk_thread_init(&m->qform2_t,(thread_route_handler)qtk_mod2_qform2_entry, m);
		wtk_blockqueue_init(&m->qform_queue);
		ret = qtk_mod2_engine_new(m, m->cfg->audio2_type);
		if(ret!=0)
		{
			wtk_log_err(m->log, "audio engine new fiald! type=%d", m->cfg->audio2_type);
			goto end;
		}
	}
	...
	if(m->cfg->use_uart)
	{
		...
		wtk_thread_init(&m->uart_t, (thread_route_handler)qtk_mod2_uart_entry, m);
...
void qtk_mod2_delete(qtk_mod2_t *m)
{
#ifndef __ANDROID__
#ifndef OFFLINE_TEST
	...
	if(m->cfg->audio_type > 0)
	{
		...
		qtk_mod2_engine_delete(m, m->cfg->audio_type);
	}

	if(m->cfg->audio2_type > 0)
	{
		wtk_blockqueue_clean(&m->qform_queue);
		qtk_mod2_engine_delete(m, m->cfg->audio2_type);
	}
...
void qtk_mod2_start(qtk_mod2_t *m)
{
	...
	if(m->cfg->audio_type > 0)
	{
		qtk_mod2_engine_start(m, m->cfg->audio_type);
		m->qform_run = 1;
	...
	if(m->cfg->audio2_type > 0)
	{
		qtk_mod2_engine_start(m, m->cfg->audio2_type);
		m->qform2_run = 1;
...
void qtk_mod2_stop(qtk_mod2_t *m)
{
#ifndef OFFLINE_TEST
	m->rcd_run = 0;
	...
	if(m->cfg->audio_type > 0)
	{
		...
		qtk_mod2_engine_stop(m, m->cfg->audio_type);
	}
	...
	if(m->cfg->audio2_type > 0)
	{
		...
		qtk_mod2_engine_stop(m, m->cfg->audio2_type);
	}
...
void qtk_mod2_clean_queue2(qtk_mod2_t *m, wtk_blockqueue_t *queue,int nx)
{
...
int  qtk_mod2_clean_sil_queue(qtk_mod2_t *m, wtk_blockqueue_t *q)
{
	wtk_queue_node_t *qn;
	qtk_msg_node_t *msg_node;
	int nx=0;
...
void qtk_mod2_clean_queue(qtk_mod2_t *m, wtk_blockqueue_t *queue)
{
	qtk_mod2_clean_queue2(m,queue,0);
}

void qtk_mod2_clean_front_queue(qtk_mod2_t *m, wtk_blockqueue_t *queue,int count)
{
	qtk_msg_node_t *msg_node;
	wtk_queue_node_t *qn;
	int i=0;
...
#ifdef OFFLINE_TEST
void qtk_mod2_feed(qtk_mod2_t *m, char *data, int len)
{
	qtk_msg_node_t *msg_node;
...
int qtk_mod2_rcd_entry(qtk_mod2_t *m, wtk_thread_t *t)
{
	...
	while(m->rcd_run){
		...
		{
			qtk_mod2_is_log_audio(m);
			count=0;
		}
		

		if(m->cfg->audio2_type == 6)
		{
			msg_node = qtk_msg_pop_node(m->msg);
...
int qtk_mod2_qform2_entry(qtk_mod2_t *m, wtk_thread_t *t)
{
	...
	while(m->qform2_run){
		...
		qtk_mod2_engine_feed(m, m->cfg->audio2_type, msg_node->buf->data, msg_node->buf->pos, 0);
		qtk_msg_push_node(m->msg, msg_node);
	}
	qtk_mod2_engine_feed(m, m->cfg->audio2_type, NULL, 0, 1);
	return 0;
}
...
int qtk_mod2_qform_entry(qtk_mod2_t *m, wtk_thread_t *t)
{
	...
	while(m->qform_run){
		...
		if(m->cfg->use_log_wav || m->log_audio)
		{
			...
			msg_node2->type = qtk_mod2_BF_SAVE_SOURCE;
			wtk_strbuf_push(msg_node2->buf, msg_node->buf->data, msg_node->buf->pos);
			wtk_blockqueue_push(&m->savepcm_queue, &msg_node2->qn);
		}
		qtk_mod2_engine_feed(m, m->cfg->audio_type, msg_node->buf->data, msg_node->buf->pos, 0);
		qtk_msg_push_node(m->msg, msg_node);
	}
	qtk_mod2_engine_feed(m, m->cfg->audio_type, NULL, 0, 1);
	return 0;
}
...
int qtk_mod2_usbaudio_entry(qtk_mod2_t *m, wtk_thread_t *t)
{
	...
	last_t=0;
  	while(m->usbaudio_run){
		...
		type=msg_node?msg_node->type:qtk_mod2_BF_VBOXEBF;
		switch(type){
			case qtk_mod2_BF_VBOXEBF:
				// if(m->cfg->use_log_wav || m->log_audio)
				// {
				// 	msg_node2 = qtk_msg_pop_node(m->msg);
				// 	msg_node2->type = qtk_mod2_BF_SAVE_SPK;
				// 	wtk_strbuf_push(msg_node2->buf, msg_node->buf->data, msg_node->buf->pos);
				// 	wtk_blockqueue_push(&m->savepcm_queue, &msg_node2->qn);
				// }
...
void qtk_mod2_usbaudio_output(qtk_mod2_t *m, char *data, int len)
{
	qtk_msg_node_t *msg_node;
	char *ss;
	int nx=0;
	...
	if(m->cfg->use_usbaudio){
		msg_node = qtk_msg_pop_node(m->msg);
		msg_node->type = qtk_mod2_BF_VBOXEBF;
		msg_node->sil=0;
		...
		{
			qtk_mod2_clean_queue(m, &(m->usbaudio_queue));
		}
	}
#endif
#endif
...
void qtk_mod2_lineout_output(qtk_mod2_t *m, char *data, int len)
{
	qtk_msg_node_t *msg_node;
	char *ss;
	int nx=0;
	...
	if(m->cfg->use_lineout){
		...
		{
			qtk_mod2_clean_queue(m, &(m->lineout_queue));
		}
	}
#endif
#endif
...
void qtk_mod2_on_resample(qtk_mod2_t *m, char *data, int len)
{
	qtk_mod2_usbaudio_output(m, data, len);
}

void qtk_mod2_on_qform(qtk_mod2_t *m, char *data, int len)
{
	qtk_msg_node_t *msg_node, *msg_node2;
	static int frame=0;
...
void qtk_mod2_on_eqform(qtk_mod2_t *m, char *data, int len)
{
	...
	qtk_mod2_lineout_output(m, data, len);

	// if(m->cfg->use_log_wav || m->log_audio)
	// {
	// 	msg_node3 = qtk_msg_pop_node(m->msg);
	// 	msg_node3->type = qtk_mod2_BF_SAVE_SPK;
...
void qtk_mod2_on_vboxebf(qtk_mod2_t *m, char *data, int len)
{
    qtk_msg_node_t *msg_node, *msg_node3;
	// static double vtm;
	// vtm = time_get_ms() - vtm;
	// wtk_debug("===========================>>>>>>>>>>>>>>>>>>>>>tm=%f len=%d\n", vtm, len);
	// vtm = time_get_ms();
...
void qtk_mod2_on_wdec(qtk_mod2_t *m, qtk_var_t *var)
{
...
void qtk_mod2_on_vboxebf_ssl(qtk_mod2_t *mod, qtk_var_t *var)
{
	switch (var->type)
	{
	case QTK_AEC_DIRECTION:
...
int qtk_mod2_engine_new(qtk_mod2_t *m, int type)
{
	...
	case qtk_mod2_BF_VBOXEBF:
		...
		qtk_vboxebf_set_notify(m->vboxebf, m, (qtk_vboxebf_notify_f)qtk_mod2_on_vboxebf);
		qtk_vboxebf_set_notify2(m->vboxebf, m, (qtk_engine_notify_f)qtk_mod2_on_vboxebf_ssl);
#endif
		break;
	case qtk_mod2_BF_GAINNETBF:
#ifdef USE_GAINNETBF
		...
		qtk_gainnetbf_set_notify(m->gainnetbf, m, (qtk_gainnetbf_notify_f)qtk_mod2_on_vboxebf);
#endif
		break;
	case qtk_mod2_BF_SSL:
#ifdef USE_SSL
		...
		qtk_ssl_set_notify(m->ssl, m, (qtk_ssl_notify_f)qtk_mod2_on_ssl);
#endif
		break;
	case qtk_mod2_BF_EQFORM:
#ifdef USE_EQFORM
		printf("qtk_mod2_BF_EQFORM resname=%.*s\n",m->cfg->vboxebf_fn.len,m->cfg->vboxebf_fn.data);
		wtk_log_log(m->log, "qtk_mod2_BF_EQFORM resname=%.*s",m->cfg->vboxebf_fn.len,m->cfg->vboxebf_fn.data);
		...
		qtk_eqform_set_notify(m->eqform, m, (qtk_eqform_notify_f)qtk_mod2_on_eqform);
#endif
		break;
	case qtk_mod2_BF_AEC:
#ifdef USE_AEC
		printf("qtk_mod2_BF_AEC resname=%.*s\n",m->cfg->vboxebf_fn.len,m->cfg->vboxebf_fn.data);
		wtk_log_log(m->log, "qtk_mod2_BF_AEC resname=%.*s",m->cfg->vboxebf_fn.len,m->cfg->vboxebf_fn.data);
		...
		qtk_aec_set_notify2(m->aec, m, (qtk_engine_notify_f)qtk_mod2_on_aec);
#endif
		break;
	case qtk_mod2_BF_QFORM:
#ifdef USE_QFORM
		printf("qtk_mod2_BF_QFORM resname=%.*s\n",m->cfg->qform_fn.len,m->cfg->qform_fn.data);
		wtk_log_log(m->log, "qtk_mod2_BF_QFORM resname=%.*s",m->cfg->qform_fn.len,m->cfg->qform_fn.data);
		...
		qtk_qform_set_notify(m->qform, m, (qtk_qform_notify_f)qtk_mod2_on_qform);
#endif
		break;
	case qtk_mod2_BF_WDEC:
#ifdef USE_WDEC
		printf("qtk_mod2_BF_WDEC resname=%.*s\n",m->cfg->wdec_fn.len,m->cfg->wdec_fn.data);
		wtk_log_log(m->log, "qtk_mod2_BF_WDEC resname=%.*s",m->cfg->wdec_fn.len,m->cfg->wdec_fn.data);
		m->wdec_cfg = qtk_wdec_cfg_new(m->cfg->wdec_fn.data);
		if(!m->wdec_cfg)
		{
			ret=-1;
			goto end;
		}
		m->wdec = qtk_wdec_new(m->wdec_cfg, NULL);
		if(!m->wdec)
		{
			ret=-1;
			goto end;
		}
		qtk_wdec_set_notify(m->wdec, m, (qtk_engine_notify_f)qtk_mod2_on_wdec);
#endif
		break;
	default:
...
void qtk_mod2_engine_delete(qtk_mod2_t *m, int type)
{
	...
	case qtk_mod2_BF_WDEC:
		...
		if(m->wdec)
		{
			qtk_wdec_delete(m->wdec);
		}
		if(m->wdec_cfg)
		{
			qtk_wdec_cfg_delete(m->wdec_cfg);
		}
#endif
		break;
	default:
...
int qtk_mod2_engine_feed(qtk_mod2_t *m, int type, char *data, int len, int is_end)
{
	...
	case qtk_mod2_BF_WDEC:
		...
		if(m->wdec)
		{
			qtk_wdec_feed(m->wdec, data, len, is_end);
		}
#endif
		break;
	default:
...
int qtk_mod2_get_ddff(qtk_mod2_t *m)
{
	char result[1024];
