...
int qtk_hash_add(qtk_hash_t *hash,int key,void *ths)
{
	...
	node = (qtk_hash_node_t*)wtk_heap_malloc(hash->heap,sizeof(qtk_hash_node_t));
	node->key = key;
	node->ths = ths;
	index = key % hash->nslot;

	if(!hash->slot[index]) {
		hash->slot[index] = (wtk_queue_t*)wtk_heap_malloc(hash->heap,sizeof(wtk_queue_t));
...
int qtk_hash_walk(qtk_hash_t *hash,void *user_data,qtk_hash_walk_func walk_func)
{
	wtk_queue_node_t *qn;
	qtk_hash_node_t *node;
	int i;
