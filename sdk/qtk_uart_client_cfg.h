#ifndef __SDK_UART_CLIENT_CFG_H__
#define __SDK_UART_CLIENT_CFG_H__
#include "sdk/dev/uart/qtk_uart_cfg.h"
#include "wtk/core/cfg/wtk_main_cfg.h"

#ifdef _cplusplus
extern "C"{
#endif

typedef struct qtk_uart_client_cfg qtk_uart_client_cfg_t;

struct qtk_uart_client_cfg{
	qtk_uart_cfg_t uart;
	wtk_main_cfg_t *maincfg;
	int dev_no;
	unsigned use_uart:1;
	unsigned log_debug:1;
	unsigned pingsuan_run:1;
};

int qtk_uart_client_cfg_init(qtk_uart_client_cfg_t *cfg);
int qtk_uart_client_cfg_clean(qtk_uart_client_cfg_t *cfg);
int qtk_uart_client_cfg_update_local(qtk_uart_client_cfg_t *cfg, wtk_local_cfg_t *lc);
int qtk_uart_client_cfg_update(qtk_uart_client_cfg_t *cfg);

qtk_uart_client_cfg_t *qtk_uart_client_cfg_new(char *fn);
void qtk_uart_client_cfg_delete(qtk_uart_client_cfg_t *uart);

#ifdef _cplusplus
}
#endif
#endif
