...
{
	if(cfg->agc_cfg){
		cfg->use_bin ? wtk_agc_cfg_delete_bin(cfg->agc_cfg) : wtk_agc_cfg_delete(cfg->agc_cfg);
	}
	if(cfg->mic_pos)
	{
		int i;
...
float ** qtk_agc_cfg_set_mic(qtk_agc_cfg_t *cfg, float **dmic, int dc)
{
	int i,j;
...
int qtk_agc_cfg_update(qtk_agc_cfg_t *cfg)
{
	...
	int i,j;
	if(cfg->cfg_fn.len > 0){
		cfg->agc_cfg = cfg->use_bin ? wtk_agc_cfg_new_bin(cfg->cfg_fn.data) : wtk_agc_cfg_new(cfg->cfg_fn.data);
		if(cfg->agc_cfg){
			ret = 0;
		}
	}
...
qtk_agc_cfg_t *qtk_agc_cfg_new(char *fn)
{
	...
	main_cfg = wtk_main_cfg_new_type(qtk_agc_cfg, fn);
	if(!main_cfg){
		return NULL;
	}
	cfg = (qtk_agc_cfg_t *)main_cfg->cfg;
...
qtk_agc_cfg_t *qtk_agc_cfg_new_bin(char *bin_fn)
{
	...
	cfg = (qtk_agc_cfg_t *)wtk_malloc(sizeof(qtk_agc_cfg_t));
	qtk_agc_cfg_init(cfg);
	...
	ret = qtk_agc_cfg_update_local(cfg, cfg->cfile->main);
	if(ret != 0){
		goto end;
	}
	sl.hook = cfg->rbin;
	sl.vf = (wtk_source_loader_v_t)wtk_rbin2_load_file;
	ret = qtk_agc_cfg_update2(cfg, &sl);
	if(ret != 0){
		goto end;
	}
	ret = 0;
