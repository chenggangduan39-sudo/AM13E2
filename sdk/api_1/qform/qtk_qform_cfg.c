...
int qtk_qform_cfg_init(qtk_qform_cfg_t *cfg)
{
...
int qtk_qform_cfg_clean(qtk_qform_cfg_t *cfg)
{
	if(cfg->qform_cfg){
		cfg->use_bin ? wtk_qform9_cfg_delete_bin(cfg->qform_cfg) : wtk_qform9_cfg_delete(cfg->qform_cfg);
	}
	return 0;
}
int qtk_qform_cfg_update_local(qtk_qform_cfg_t  *cfg, wtk_local_cfg_t *main)
{
	wtk_string_t *v;
...
float ** qtk_qform_cfg_set_mic(qtk_qform_cfg_t *cfg, float **dmic, int dc)
{
	int i,j;
...
int qtk_qform_cfg_update(qtk_qform_cfg_t *cfg)
{
	int ret = -1;	
	if(cfg->cfg_fn.len > 0){
		cfg->qform_cfg = cfg->use_bin ? wtk_qform9_cfg_new_bin(cfg->cfg_fn.data) : wtk_qform9_cfg_new(cfg->cfg_fn.data);
		if(cfg->qform_cfg){
	#if 0
			if(cfg->nmic > 0)
			{
				cfg->qform_cfg->bf.mic_pos = qtk_qform_cfg_set_mic(cfg, cfg->qform_cfg->bf.mic_pos, cfg->qform_cfg->bf.nmic);
				cfg->qform_cfg->bf.nmic = cfg->nmic;

				cfg->qform_cfg->aspec.mic_pos = qtk_qform_cfg_set_mic(cfg, cfg->qform_cfg->aspec.mic_pos, cfg->qform_cfg->aspec.channel);
				cfg->qform_cfg->aspec.channel = cfg->nmic;
			}
	#endif
			...
			if(cfg->use_manual)
			{
				...
				{
					cfg->qform_cfg->theta_range = cfg->theta_range;
				}
...
int qtk_qform_cfg_update2(qtk_qform_cfg_t *cfg, wtk_source_loader_t *sl)
{
	return qtk_qform_cfg_update(cfg);
}

qtk_qform_cfg_t *qtk_qform_cfg_new(char *fn)
{
	wtk_main_cfg_t *main_cfg;
	qtk_qform_cfg_t *cfg;

	main_cfg = wtk_main_cfg_new_type(qtk_qform_cfg, fn);
	if(!main_cfg){
		return NULL;
	}
	cfg = (qtk_qform_cfg_t *)main_cfg->cfg;
	cfg->main_cfg = main_cfg;

	return cfg;
}

void qtk_qform_cfg_delete(qtk_qform_cfg_t *cfg)
{
	wtk_main_cfg_delete(cfg->main_cfg);
}

qtk_qform_cfg_t *qtk_qform_cfg_new_bin(char *bin_fn)
{
	qtk_qform_cfg_t *cfg;
	...
	cfg = (qtk_qform_cfg_t *)wtk_malloc(sizeof(qtk_qform_cfg_t));
	qtk_qform_cfg_init(cfg);
	...
	ret = qtk_qform_cfg_update_local(cfg, cfg->cfile->main);
	if(ret != 0){
		goto end;
	}
	sl.hook = cfg->rbin;
	sl.vf = (wtk_source_loader_v_t)wtk_rbin2_load_file;
	ret = qtk_qform_cfg_update2(cfg, &sl);
	if(ret != 0){
		goto end;
	}
	ret = 0;
	...
	if(ret != 0){
		qtk_qform_cfg_delete_bin(cfg);
		cfg = NULL;
	}
	return cfg;
}
void qtk_qform_cfg_delete_bin(qtk_qform_cfg_t *cfg)
{
	qtk_qform_cfg_clean(cfg);
	if(cfg->cfile){
