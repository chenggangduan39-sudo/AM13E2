...
int qtk_vboxebf_get_agcstate(qtk_vboxebf_t *vb)
{
	if(vb->cfg->use_vboxebf3){
		return vb->vebf3->agc_enable;
	}else if(vb->cfg->use_vboxebf6){
		return vb->vebf6->agc_enable;
	}else if(vb->cfg->use_mask_bf_net){
		return vb->mask_bf_net->agc_enable;
	}
	return -1;
}

int qtk_vboxebf_get_echostate(qtk_vboxebf_t *vb)
{
	if(vb->cfg->use_vboxebf3){
		return vb->vebf3->echo_enable;
	}else if(vb->cfg->use_vboxebf6){
		return vb->vebf6->echo_enable;
	}else if(vb->cfg->use_mask_bf_net){
		return vb->mask_bf_net->echo_enable;
	}
	return -1;
}

int qtk_vboxebf_get_denoisestate(qtk_vboxebf_t *vb)
{
	if(vb->cfg->use_vboxebf3){
		return vb->vebf3->denoise_enable;
	}else if(vb->cfg->use_vboxebf6){
		return vb->vebf6->denoise_enable;
	}else if(vb->cfg->use_mask_bf_net){
		return vb->mask_bf_net->denoise_enable;
	}
	return -1;
}
