...
{
    wtk_debug("[EQ][0103] === Start EQ output setting ===\n");

    // 1) 基本校验
    wtk_debug("[EQ][0103] Step 1: Checking payload validity...\n");
    ...
    wtk_debug("[EQ][0103] Payload check passed\n");

    // 2) 载荷净化
    wtk_debug("[EQ][0103] Step 2: Sanitizing payload...\n");
    ...
    wtk_debug("[EQ][0103] Payload sanitized. Length = %zu\n", san_len);

    // 3) 落盘 eq.json（保留一份最新设置）
    wtk_debug("[EQ][0103] Step 3: Saving to file %s...\n", QTK_EQ_JSON_PATH);
    if (qtk_write_file(QTK_EQ_JSON_PATH, san_json, (int)san_len) != 0){
        uint8_t fail = 0xFE;
        wtk_debug("[EQ][0103] Write to %s failed\n", QTK_EQ_JSON_PATH);
        send_response(uc, frame, &fail, 1);
        break;
    }
    wtk_debug("[EQ][0103] File saved successfully\n");

    // 4) 当前线程直接应用（不再通知后端 / 不再兜底）
    wtk_debug("[EQ][0103] Step 4: Applying EQ settings in current thread...\n");
    ...
    wtk_debug("[EQ][0103] EQ applied successfully\n");

    // 5) 正常应答
    wtk_debug("[EQ][0103] Step 5: Sending success response...\n");
    uint8_t ok = 0x00; 
    send_response(uc, frame, &ok, 1);

    wtk_debug("[EQ][0103] === End EQ output setting ===\n");
    break;
}
