...
int qtk_mqform_mod_cfg_init(qtk_mqform_mod_cfg_t *cfg)
{
    ...
    qtk_recorder_cfg_init(&cfg->two_rcd);
	qtk_player_cfg_init(&cfg->two_ply);
#endif
...
#ifdef USE_RECORD_PLAYER
    qtk_recorder_cfg_clean(&cfg->two_rcd);
	qtk_player_cfg_clean(&cfg->two_ply);
#endif
...
int qtk_mqform_mod_cfg_update_local(qtk_mqform_mod_cfg_t *cfg,wtk_local_cfg_t *main)
{
    ...
    wtk_local_cfg_update_cfg_string_v(main, cfg, ssl_fn, v);
    ...
    wtk_local_cfg_update_cfg_i(main, cfg, bfkasr_type, v);
    wtk_local_cfg_update_cfg_i(main, cfg, audio_type, v);
    wtk_local_cfg_update_cfg_i(main, cfg, ssl_type, v);
    ...
    wtk_local_cfg_update_cfg_b(main, cfg, use_record_player, v);
    ...
    wtk_local_cfg_update_cfg_b(main, cfg, use_ssl, v);
    ...
    if(lc){
        qtk_recorder_cfg_update_local(&cfg->two_rcd, lc);
    }
    lc = wtk_local_cfg_find_lc_s(main, "play2");
    if(lc){
        qtk_player_cfg_update_local(&cfg->two_ply, lc);
    }
#endif
...
#ifdef USE_RECORD_PLAYER
    qtk_recorder_cfg_update(&cfg->two_rcd);
	qtk_player_cfg_update(&cfg->two_ply);
#endif
