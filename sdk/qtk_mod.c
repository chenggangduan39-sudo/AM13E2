...
#ifndef OFFLINE_TEST
int qtk_mod_rcd_entry(qtk_mod_t *m, wtk_thread_t *t);
#endif
#ifdef USE_FOR_DEV
int qtk_mod_uart_entry(qtk_mod_t *m, wtk_thread_t *t);
#endif
#ifndef __ANDROID__
int qtk_mod_usbaudio_entry(qtk_mod_t *m, wtk_thread_t *t);
int qtk_mod_lineout_entry(qtk_mod_t *m, wtk_thread_t *t);
#endif
int qtk_mod_qform_entry(qtk_mod_t *m, wtk_thread_t *t);
int qtk_mod_qform2_entry(qtk_mod_t *m, wtk_thread_t *t);
int qtk_mod_qform3_entry(qtk_mod_t *m, wtk_thread_t *t);
int qtk_mod_savepcm_entry(qtk_mod_t *m, wtk_thread_t *t);
void qtk_mod_on_vboxebf(qtk_mod_t *mod, char *data, int len);
#ifdef USE_SSL
void qtk_mod_on_ssl(qtk_mod_t *mod,qtk_ssl_extp_t *extp);
#endif
void qtk_mod_on_aec(qtk_mod_t *m, qtk_var_t *var);
static void qtk_log_wav_file_new(qtk_mod_t *m);
static void qtk_log_wav_file_delete(qtk_mod_t *m);
void qtk_is_log_audio(qtk_mod_t *m);
void qtk_mod_set_cpu(qtk_mod_t *m, wtk_thread_t *thread, int cpunum);
void qtk_mod_on_resample(qtk_mod_t *m, char *data, int len);

int qtk_mod_engine_new(qtk_mod_t *m, int type);
void qtk_mod_engine_delete(qtk_mod_t *m, int type);
void qtk_mod_engine_start(qtk_mod_t *m, int type);
void qtk_mod_engine_stop(qtk_mod_t *m, int type);
int qtk_mod_engine_feed(qtk_mod_t *m, int type, char *data, int len, int is_end);

#ifdef USE_3308
int qtk_mod_proc_send_pid(qtk_mod_t *m);
int qtk_mod_proc_read(qtk_mod_t *m);
int qtk_mod_proc_write(qtk_mod_t *m,char *data,int len);

// void myfunc(int sig);
void myfunc(int sig, siginfo_t *info, void *ptr);
void myfunc2(int sig, siginfo_t *info, void *ptr);
#endif	

int qtk_mod_msg_bytes(qtk_msg_node_t *node)
{
	int bytes=0;
...
int qtk_mod_bytes(qtk_mod_t *m)
{
	...
	// if(m->cfg->audio_type == QTK_MOD_BF_VBOXEBF)
	// {
...
#endif
	tmp=wtk_hoard_bytes((wtk_hoard_t *)(&(m->msg->msg_hoard)),(wtk_hoard_bytes_f)qtk_mod_msg_bytes);
	bytes+=tmp;
	bytes+=sizeof(qtk_mod_t);
...
qtk_mod_t *qtk_mod_new(qtk_mod_cfg_t *cfg)
{
	...
	m = (qtk_mod_t *)wtk_calloc(1, sizeof(*m));
	m->cfg = cfg;
	...
	if(m->cfg->cache_path.len > 0){
		...
		wtk_thread_init(&m->savepcm_t,(thread_route_handler)qtk_mod_savepcm_entry, m);
	}
	...
	if(m->cfg->use_usbaudio){
		...
		wtk_thread_init(&m->usbaudio_t,(thread_route_handler)qtk_mod_usbaudio_entry, m);
		m->usbaudio = qtk_play_new(&cfg->usbaudio);
	}
	if(m->cfg->use_lineout){
		wtk_blockqueue_init(&m->lineout_queue);
		wtk_thread_init(&m->lineout_t,(thread_route_handler)qtk_mod_lineout_entry, m);
	...
	wtk_resample_set_notify(m->resample, m, (wtk_resample_notify_f)qtk_mod_on_resample);
...
//    	goto end;
//	}
	wtk_thread_init(&m->rcd_t, (thread_route_handler)qtk_mod_rcd_entry, m);
#endif
#endif
	...
	m->msg = qtk_msg_new();
	wtk_thread_init(&m->qform_t,(thread_route_handler)qtk_mod_qform_entry, m);
	wtk_blockqueue_init(&m->bfio_queue);
	if(m->cfg->audio2_type > 0)
	{
		wtk_thread_init(&m->qform2_t,(thread_route_handler)qtk_mod_qform2_entry, m);
		wtk_blockqueue_init(&m->bfio2_queue);
	}
	if(m->cfg->audio3_type > 0)
	{
		wtk_thread_init(&m->qform3_t,(thread_route_handler)qtk_mod_qform3_entry, m);
	...
	m->out_resample = speex_resampler_init(m->cfg->usbaudio.channel, 16000, 48000, SPEEX_RESAMPLER_QUALITY_DESKTOP, NULL);
	m->tmpresample = (char *)wtk_malloc(m->cfg->usbaudio.buf_time*m->cfg->usbaudio.channel*96);
	ret = qtk_mod_engine_new(m,m->cfg->audio_type);
	if(ret!=0)
	{
	...
	if(m->cfg->audio2_type > 0)
	{
		ret = qtk_mod_engine_new(m,m->cfg->audio2_type);
		if(ret!=0)
		{
	...
	if(m->cfg->audio3_type > 0)
	{
		ret = qtk_mod_engine_new(m,m->cfg->audio3_type);
		if(ret!=0)
		{
	...
	if(m->cfg->use_uart)
	{
		...
		wtk_thread_init(&m->uart_t, (thread_route_handler)qtk_mod_uart_entry, m);
    ...
    // 注册信号
	wtk_thread_init(&mem_t,(thread_route_handler)qtk_mod_proc_read, m);
	wtk_thread_init(&mem2_t,(thread_route_handler)qtk_mod_proc_send_pid, m);
...
void qtk_mod_delete(qtk_mod_t *m)
{
	...
	if(m->out_resample){
		speex_resampler_destroy(m->out_resample);
		m->out_resample = NULL;
	}
	if(m->tmpresample){
		wtk_free(m->tmpresample);
	}
	...
	qtk_mod_engine_delete(m, m->cfg->audio_type);

	if(m->cfg->audio2_type > 0)
	{
		qtk_mod_engine_delete(m, m->cfg->audio2_type);
	}
	if(m->cfg->audio3_type > 0)
	{
		qtk_mod_engine_delete(m, m->cfg->audio3_type);
	}
...
void qtk_mod_start(qtk_mod_t *m)
{
	...
	{
		wtk_resample_start(m->resample, 48000, m->cfg->resample_rate);
	}
#endif

	qtk_mod_engine_start(m, m->cfg->audio_type);
	m->qform_run = 1;
	wtk_thread_start(&m->qform_t);

	if(m->cfg->audio2_type > 0)
	{
		qtk_mod_engine_start(m, m->cfg->audio2_type);
		m->qform2_run = 1;
		wtk_thread_start(&m->qform2_t);
	}
	if(m->cfg->audio3_type > 0)
	{
		qtk_mod_engine_start(m, m->cfg->audio3_type);
		m->qform3_run = 1;
...
void qtk_mod_stop(qtk_mod_t *m)
{
	...
	qtk_mod_engine_stop(m, m->cfg->audio_type);
	...
	if(m->cfg->audio2_type > 0)
	{
		...
		qtk_mod_engine_stop(m, m->cfg->audio2_type);
	}
	if(m->cfg->audio3_type > 0)
	{
		m->qform3_run = 0;
		...
		qtk_mod_engine_stop(m, m->cfg->audio3_type);
	}
...
#if (defined USE_3308)
		wtk_resample_start(m->resample, 48000, sample_rate);
#else
		wtk_resample_start(m->resample, 48000, m->cfg->resample_rate);
#endif
	}
...
void qtk_mod_clean_queue2(qtk_mod_t *m, wtk_blockqueue_t *queue,int nx)
{
...
int wtk_qmode_check_sil_queue(wtk_blockqueue_t *q)
{
	int n=0;
	wtk_queue_node_t *qn;
	qtk_msg_node_t *msg_node;
...
int  qtk_mod_clean_sil_queue(qtk_mod_t *m, wtk_blockqueue_t *q)
{
	wtk_queue_node_t *qn;
	qtk_msg_node_t *msg_node;
	int nx=0;
...
void qtk_mod_clean_queue(qtk_mod_t *m, wtk_blockqueue_t *queue)
{
	qtk_mod_clean_queue2(m,queue,0);
}

void qtk_mod_clean_front_queue(qtk_mod_t *m, wtk_blockqueue_t *queue,int count)
{
	qtk_msg_node_t *msg_node;
	wtk_queue_node_t *qn;
	int i=0;
...
int qtk_mod_qform_entry(qtk_mod_t *m, wtk_thread_t *t)
{
	...
	while(m->qform_run){
...
#endif
			qtk_mod_clean_queue(m, &m->bfio_queue);
		}
		...
		{
			msg_node2 = qtk_msg_pop_node(m->msg);
			msg_node2->type = QTK_MOD_BF_SAVE_SOURCE;
			if(m->in_resample)
			{
		...
		if(m->usbaudio_queue.length >30)
		{
			...
			qtk_mod_clean_queue(m, &(m->usbaudio_queue));
		}
		msg_node2 = qtk_msg_pop_node(m->msg);
		msg_node2->type = QTK_MOD_BF_VBOXEBF;
		int i=0,j=0;
		...
		if(m->in_resample)
		{
			qtk_mod_engine_feed(m, m->cfg->audio_type, outresample, (outlen<<1)*(m->cfg->rcd.channel-m->cfg->rcd.nskip), 0);
		}else{
			qtk_mod_engine_feed(m, m->cfg->audio_type, msg_node->buf->data, msg_node->buf->pos, 0);
		}
#endif
#endif
		qtk_msg_push_node(m->msg, msg_node);
	}
	qtk_mod_engine_feed(m, m->cfg->audio_type, NULL, 0, 1);
	if(outresample)
	{
...
int qtk_mod_qform2_entry(qtk_mod_t *m, wtk_thread_t *t)
{
	...
	while(m->qform2_run){
		...
		qtk_mod_engine_feed(m, m->cfg->audio2_type, msg_node->buf->data, msg_node->buf->pos, 0);

		qtk_msg_push_node(m->msg, msg_node);
	}
	qtk_mod_engine_feed(m, m->cfg->audio2_type, NULL, 0, 1);
...
int qtk_mod_qform3_entry(qtk_mod_t *m, wtk_thread_t *t)
{
	...
	while(m->qform3_run){
		...
		qtk_mod_engine_feed(m, m->cfg->audio3_type, msg_node->buf->data, msg_node->buf->pos, 0);

		qtk_msg_push_node(m->msg, msg_node);
	}
	qtk_mod_engine_feed(m, m->cfg->audio3_type, NULL, 0, 1);
  	...
  	while(m->usbaudio_run){
...
#endif
		{
			if(first)
			{
				...
				if(m->cfg->audio_type == QTK_MOD_BF_BFIO)
				{
			...
			type=msg_node?msg_node->type:QTK_MOD_BF_VBOXEBF;
			switch(type){
			case QTK_MOD_BF_VBOXEBF:
				// if(m->cfg->use_log_wav || m->log_audio)
				// {
				// 	msg_node2 = qtk_msg_pop_node(m->msg);
				// 	msg_node2->type = QTK_MOD_BF_SAVE_SPK;
				// 	wtk_strbuf_push(msg_node2->buf, msg_node->buf->data, msg_node->buf->pos);
				// 	wtk_blockqueue_push(&m->savepcm_queue, &msg_node2->qn);
				// }
				// if(m->cfg->audio_type == QTK_MOD_BF_BFIO)
...
// 保留整数的低8位
unsigned char qtk_mod_keep_low_byte(int value) {
    return (unsigned char)(value & 0xFF);
}

// 保留整数的高8位
unsigned char qtk_mod_keep_high_byte(int value) {
    return (unsigned char)((value >> 8) & 0xFF);
}

void qtk_mod_lineout_output(qtk_mod_t *m, char *data, int len)
{
	qtk_msg_node_t *msg_node2;
	char *ss;
	int nx;
	...
	if(m->cfg->use_lineout){
		...
		{
			qtk_mod_clean_queue(m, &(m->lineout_queue));
		}
	}
#endif
#endif
...
void qtk_mod_lineout_output2(qtk_mod_t *m, char *data, int len)
{
	qtk_msg_node_t *msg_node2;
#ifndef __ANDROID__
#ifndef OFFLINE_TEST
...
void qtk_mod_player_output2(qtk_mod_t *m, char *data, int len)
{
	qtk_msg_node_t *msg_node;
#ifndef __ANDROID__
#ifndef OFFLINE_TEST
	if(m->cfg->use_usbaudio){
		msg_node = qtk_msg_pop_node(m->msg);
		msg_node->type = QTK_MOD_BF_VBOXEBF;
		msg_node->sil=0;
		wtk_strbuf_push(msg_node->buf, data, len);
...
void qtk_mod_player_output(qtk_mod_t *m, char *data, int len)
{
	qtk_msg_node_t *msg_node;
	char *ss;
	int nx;
...
void qtk_mod_on_resample(qtk_mod_t *m, char *data, int len)
{
	qtk_mod_player_output(m, data, len);
}

void qtk_mod_on_soundscreen(qtk_mod_t *m, char *data, int len)
{
	...
	// plen+=len;
	// wtk_debug("====================>>>>>>>>>>>>>>>>>>len=%d rlen=%d plen=%d\n", len, rlen, plen);
	if(m->cfg->use_resample){
		int inlen,outlen;
		memset(m->tmpresample, 0, len*3);
		inlen=(len >> 1)/m->cfg->usbaudio.channel;
		outlen=inlen*3;

		if(m->out_resample){
			speex_resampler_process_interleaved_int(m->out_resample,
										(spx_int16_t *)(data), (spx_uint32_t *)(&inlen), 
										(spx_int16_t *)(m->tmpresample), (spx_uint32_t *)(&outlen));
		}
		// wtk_debug("============>>>>>>>>>>>>>>len=%d outlen=%d\n", len, (outlen<<1)*m->cfg->usbaudio.channel);

		qtk_mod_player_output2(m, m->tmpresample, (outlen<<1)*m->cfg->usbaudio.channel);
		if(m->cfg->use_log_wav || m->log_audio){
			qtk_msg_node_t *msg_node3;
			msg_node3 = qtk_msg_pop_node(m->msg);
			msg_node3->type = QTK_MOD_BF_SAVE_SPK;
			wtk_strbuf_push(msg_node3->buf, m->tmpresample, (outlen<<1)*m->cfg->usbaudio.channel);
			wtk_blockqueue_push(&m->savepcm_queue, &msg_node3->qn);
		}
	}else{
		qtk_mod_player_output2(m, data, len);
		if(m->cfg->use_log_wav || m->log_audio){
			qtk_msg_node_t *msg_node3;
			msg_node3 = qtk_msg_pop_node(m->msg);
			msg_node3->type = QTK_MOD_BF_SAVE_SPK;
...
void qtk_mod_on_eqform(qtk_mod_t *m, char *data, int len)
{
...
void qtk_mod_on_vboxebf(qtk_mod_t *m, char *data, int len)
{
    qtk_msg_node_t *msg_node, *msg_node2, *msg_node3;
	char *ss;
	int nx=0;
	...
	if(m->cfg->audio3_type > 0)
	{
		...
		qtk_mod_lineout_output(m, data, len);
	}
	// if(m->cfg->use_log_wav || m->log_audio)
	// {
	// 	msg_node3 = qtk_msg_pop_node(m->msg);
...
void qtk_mod_on_bfio(qtk_mod_t *m, qtk_var_t *var) {
   	qtk_msg_node_t *msg_node, *msg_node2;
	char *ss;
	char *data=NULL;
	int len;
    ...
    case QTK_AEC_WAKE:
        wtk_debug("=================> wake\n");
#ifdef USE_FOR_DEV
#if (defined USE_AM32) || (defined USE_R328) || (defined USE_AM60)
		if(m->cfg->use_uart)
		{
			qtk_msg_node_t *msg_node;
			...
			data[0]=0xff;
			data[1]=0x01;
			data[2]=qtk_mod_keep_low_byte(qtheta);
			data[3]=qtk_mod_keep_high_byte(qtheta);
			memcpy(data+4, &energy, 4);
			data[8]=0x01;

			msg_node = qtk_msg_pop_node(m->msg);
			msg_node->type = QTK_MOD_BF_DIRECTION;
...
void qtk_mod_on_kws(qtk_mod_t *mod, qtk_var_t *var)
{
	switch (var->type)
	{
	case QTK_EVAL_TEXT:
...
void qtk_mod_on_vboxebf_ssl(qtk_mod_t *mod, qtk_var_t *var)
{
	switch (var->type)
	{
	case QTK_AEC_DIRECTION:
...
int qtk_mod_engine_new(qtk_mod_t *m, int type)
{
	...
	case QTK_MOD_BF_VBOXEBF:
		...
		qtk_vboxebf_set_notify(m->vboxebf, m, (qtk_vboxebf_notify_f)qtk_mod_on_vboxebf);
		qtk_vboxebf_set_notify2(m->vboxebf, m, (qtk_engine_notify_f)qtk_mod_on_vboxebf_ssl);
#endif
		break;
	case QTK_MOD_BF_GAINNETBF:
#ifdef USE_GAINNETBF
		...
		qtk_gainnetbf_set_notify(m->gainnetbf, m, (qtk_gainnetbf_notify_f)qtk_mod_on_vboxebf);
#endif
		break;
	case QTK_MOD_BF_SSL:
#ifdef USE_SSL
		...
		qtk_ssl_set_notify(m->ssl, m, (qtk_ssl_notify_f)qtk_mod_on_ssl);
#endif
		break;
	case QTK_MOD_BF_EQFORM:
#ifdef USE_EQFORM
		printf("QTK_MOD_BF_EQFORM resname=%.*s\n",m->cfg->vboxebf_fn.len,m->cfg->vboxebf_fn.data);
		wtk_log_log(m->log, "QTK_MOD_BF_EQFORM resname=%.*s",m->cfg->vboxebf_fn.len,m->cfg->vboxebf_fn.data);
		...
		qtk_eqform_set_notify(m->eqform, m, (qtk_eqform_notify_f)qtk_mod_on_eqform);
#endif
		break;
	case QTK_MOD_BF_AEC:
#ifdef USE_AEC
		printf("QTK_MOD_BF_AEC resname=%.*s\n",m->cfg->vboxebf_fn.len,m->cfg->vboxebf_fn.data);
		wtk_log_log(m->log, "QTK_MOD_BF_AEC resname=%.*s",m->cfg->vboxebf_fn.len,m->cfg->vboxebf_fn.data);
		...
		qtk_aec_set_notify2(m->aec, m, (qtk_engine_notify_f)qtk_mod_on_aec);
#endif
		break;
	case QTK_MOD_BF_BFIO:
#ifdef USE_BFIO
		printf("QTK_MOD_BF_BFIO resname=%.*s\n",m->cfg->qform_fn.len,m->cfg->qform_fn.data);
		wtk_log_log(m->log, "QTK_MOD_BF_BFIO resname=%.*s",m->cfg->qform_fn.len,m->cfg->qform_fn.data);
		...
		qtk_bfio_set_notify2(m->bfio, m, (qtk_engine_notify_f)qtk_mod_on_bfio);
#endif
		break;
	case QTK_MOD_BF_KWS:
#ifdef USE_QKWS
		printf("QTK_MOD_BF_KWS resname=%.*s\n",m->cfg->kws_fn.len,m->cfg->kws_fn.data);
		wtk_log_log(m->log, "QTK_MOD_BF_KWS resname=%.*s",m->cfg->kws_fn.len,m->cfg->kws_fn.data);
		...
		qtk_qkws_set_notify(m->kws, m, (qtk_engine_notify_f)qtk_mod_on_kws);
...
#endif
		break;
	case QTK_MOD_BF_SOUNDSCREEN:
#ifdef USE_SOUNDSCREEN
		printf("QTK_MOD_BF_SOUNDSCREEN resname=%.*s\n",m->cfg->soundscreen_fn.len,m->cfg->soundscreen_fn.data);
		wtk_log_log(m->log, "QTK_MOD_BF_SOUNDSCREEN resname=%.*s",m->cfg->soundscreen_fn.len,m->cfg->soundscreen_fn.data);
		m->soundscreen_cfg = qtk_soundscreen_cfg_new(m->cfg->soundscreen_fn.data);
		if(!m->soundscreen_cfg)
		{
			ret=-1;
			goto end;
		}
		m->soundscreen = qtk_soundscreen_new(m->soundscreen_cfg);
		if(!m->soundscreen)
		{
			ret=-1;
			goto end;
		}
		qtk_soundscreen_set_notify(m->soundscreen, m, (qtk_soundscreen_notify_f)qtk_mod_on_soundscreen);
#endif
		break;
	default:
...
void qtk_mod_engine_delete(qtk_mod_t *m, int type)
{
	...
	case QTK_MOD_BF_SOUNDSCREEN:
		...
		if(m->soundscreen)
		{
			qtk_soundscreen_delete(m->soundscreen);
		}
		if(m->soundscreen_cfg)
		{
			qtk_soundscreen_cfg_delete(m->soundscreen_cfg);
		}
#endif
		break;
	default:
...
void qtk_mod_engine_start(qtk_mod_t *m, int type)
{
	...
	case QTK_MOD_BF_SOUNDSCREEN:
		...
		if(m->soundscreen)
		{
			qtk_soundscreen_start(m->soundscreen);
		}
#endif
		break;
	default:
...
void qtk_mod_engine_stop(qtk_mod_t *m, int type)
{
	...
	case QTK_MOD_BF_SOUNDSCREEN:
		...
		if(m->soundscreen)
		{
			qtk_soundscreen_reset(m->soundscreen);
		}
#endif
		break;
	default:
...
int qtk_mod_engine_feed(qtk_mod_t *m, int type, char *data, int len, int is_end)
{
	...
	case QTK_MOD_BF_SOUNDSCREEN:
		...
		if(m->soundscreen)
		{
			qtk_soundscreen_feed(m->soundscreen, data, len, is_end);
		}
#endif
		break;
	default:
...
int qtk_mod_get_ddff(qtk_mod_t *m)
{
	char result[1024];
...
static void qtk_log_wav_file_new(qtk_mod_t *m)
{
	...
	// channel = 2;
	channel = m->cfg->usbaudio.channel;
	sample_rate = m->cfg->usbaudio.sample_rate;
	m->single = wtk_wavfile_new(sample_rate);
	m->single->max_pend = 0;
...
void myfunc2(int sig, siginfo_t *info, void *ptr)//绑定pid
{
	once2=1;
