	...
	while(s<e)
	{
		...
		case QTK_ASOUND_CARD_NAME_E:
			if(*s=='\n')
			{
				...
				wtk_log_log(glb_log,"card = %s.cardname = %s.deviceid = %s.cardid = %s.",card,cardid,deviceid,cardname);
#ifdef MTK8516
				if(strncmp(deviceid,"mt-snd-card",11)==0)
#else
				if(strncmp(deviceid,"USB-Audio",9)==0 && strncmp(cardname,"Qdreamer",8)==0)
#endif
				{	b=1;goto end;}
			}else{
				cardname[i++]=*s;
