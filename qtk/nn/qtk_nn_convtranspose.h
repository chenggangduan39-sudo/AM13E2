...
static int ncnn_transpose_forward_(float* input, float* weight, float *bias_ptr, float *output,
    ...
    int trans_size = k * n * elemsize;
    int out_size = out_elemsize * outw * outh * out_channels;//need cut to final output
    int out_gemm_size = m * n * out_elemsize;
    ensure_workspace_(nv, trans_size + out_size + out_gemm_size);
    memset(nv->workspace.f32, 0, out_size + out_gemm_size + trans_size);

    float *in_trans = nv->workspace.f32;
    for (int i = 0; i < k; i++) {
        for (int j = 0; j < n; j++,b++) {
            *(in_trans + j * k + i) = *b;
        }
    ...
    b = in_trans;
    float *c = nv->workspace.f32 + trans_size/out_elemsize;
    //b = input;
    //qtk_sgemm(1, 0, m, n, k, 1.0, a, m, b, n, 0.0, c, n);
    qtk_sgemm(0, 1, m, n, k, 1.0, a, k, b, k, 0.0, c, n);

    float *d = nv->workspace.f32 + (trans_size + out_gemm_size)/out_elemsize;
