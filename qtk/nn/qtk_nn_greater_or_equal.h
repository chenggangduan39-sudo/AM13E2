#ifndef G_8YZE7CIK_EM9I_EB65_ZUQ3_JDJ1J7WI04VP
#define G_8YZE7CIK_EM9I_EB65_ZUQ3_JDJ1J7WI04VP
#pragma once
...
vm_GreaterOrEqual_infer_shape_(qtk_nn_vm_t *nv, uint8_t *instructions) {
...
qtk_maybe_unused static void vm_GreaterOrEqual_(qtk_nn_vm_t *nv,
    ...
    if (nelemA == nelemB) {
        if (elem_type == QBL_NN_VM_TENSOR_ELEM_I64) {
            for (uint32_t i = 0; i < nelemA; i++) {
                Y.boolean[i] = A.i64[i] >= B.i64[i] ? 1 : 0;
            }
        } else {
            qtk_debug("%d\n", elem_type);
            qtk_assert(0);
        ...
        if (nelemB == 1) {
            if (elem_type == QBL_NN_VM_TENSOR_ELEM_I64) {
                for (uint32_t i = 0; i < nelemA; i++) {
                    Y.boolean[i] = A.i64[i] >= B.i64[0] ? 1 : 0;
                }
            } else {
                qtk_debug("%d\n", elem_type);
                qtk_assert(0);
...
#endif
#endif /* G_8YZE7CIK_EM9I_EB65_ZUQ3_JDJ1J7WI04VP */
