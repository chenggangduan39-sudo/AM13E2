#ifndef QBL_NN_QBL_NN_MATMULQ_H
#define QBL_NN_QBL_NN_MATMULQ_H
#pragma once

#include "qtk/core/qtk_binary.h"
#include "qtk/linalg/qtk_gemm.h"
#include "qtk/nn/qtk_nn_debug.h"
#include "qtk/nn/qtk_nn_utils.h"
#include "qtk/nn/vm/qtk_nn_vm.h"
#include "qtk/nn/qtk_nn_im2col_quant.h"

#ifdef __cplusplus
extern "C" {
#endif

qtk_maybe_unused static int vm_MatMulInteger_infer_shape_(qtk_nn_vm_t *nv,
                                                   uint8_t *instructions) {
    uint16_t a = qtk_littleEndian_uint16_from_bin(*instructions),
             b = qtk_littleEndian_uint16_from_bin(*instructions + 2),
             y = qtk_littleEndian_uint16_from_bin(*instructions + 8);
    uint32_t *shapeA, *shapeB, shapeY[16];
    int rankA, rankB, rankY;

    shapeA = qtk_nn_get_shape_from_repr(nv, a);
    shapeB = qtk_nn_get_shape_from_repr(nv, b);

    rankA = qtk_nn_get_rank_from_repr(nv, a);
    rankB = qtk_nn_get_rank_from_repr(nv, b);

    if (rankB == 2) {
        qtk_assert(rankA >= rankB);
        for (int i = 0; i < rankA - rankB; i++) {
            shapeY[i] = shapeA[i];
        }
        shapeY[rankA - rankB] = shapeA[rankA - rankB];
        shapeY[rankA - rankB + 1] = shapeB[1];
        rankY = rankA - rankB + 2;
    } else {
        qtk_assert(rankA == rankB && rankA > 2);
        for (int i = 0; i < rankA - 2; i++) {
            qtk_assert(shapeA[i] == shapeB[i]);
            shapeY[i] = shapeB[i];
        }
        shapeY[rankA - 2] = shapeA[rankA - 2];
        shapeY[rankA - 1] = shapeB[rankA - 1];
        rankY = rankA;
    }
    nn_vm_set_dynamic_shape_for_repr_(nv, shapeY, rankY, y);
    return 0;
}

qtk_maybe_unused static void vm_MatMulInteger_(qtk_nn_vm_t *nv,
                                        uint8_t **instructions) {
    uint16_t a = qtk_littleEndian_uint16_from_bin(*instructions),
             b = qtk_littleEndian_uint16_from_bin(*instructions + 2),
             az = qtk_littleEndian_uint16_from_bin(*instructions + 4),
             bz = qtk_littleEndian_uint16_from_bin(*instructions + 6),
             y = qtk_littleEndian_uint16_from_bin(*instructions + 8);
    uint32_t *shapeA, *shapeB;
    qtk_numeric_data_t A, B, Y, AZ, BZ;
    //qtk_nn_vm_tensor_elem_type_t elem_type;
    int rankA, rankB;

    shapeA = qtk_nn_get_shape_from_repr(nv, a);
    shapeB = qtk_nn_get_shape_from_repr(nv, b);
    rankA = qtk_nn_get_rank_from_repr(nv, a);
    rankB = qtk_nn_get_rank_from_repr(nv, b);

    //elem_type = QBL_NN_TENSOR_GET_ELEM_TYPE(y);

    A.raw = qtk_nn_get_loc_from_repr(nv, a);
    B.raw = qtk_nn_get_loc_from_repr(nv, b);
    Y.raw = qtk_nn_get_loc_from_repr(nv, y);
    AZ.raw = qtk_nn_get_loc_from_repr(nv, az);
    BZ.raw = qtk_nn_get_loc_from_repr(nv, bz);

    uint32_t nelem = nn_vm_tensor_get_nelem_from_repr_(nv, a);
    //uint8_t wz = AZ.u8[0];
    // for(int i; i < nelem; i++) {
    //     A.u8[i] = A.u8[i] - zp;
    // }

    if (rankB == 2) {
        qtk_assert(rankA >= 2);
        uint32_t nblock = 1;
        for (int i = 0; i < rankA - rankB; i++) {
            nblock *= shapeA[i];
        }

        for (int i = 0; i < nblock; i++) {
            int m, n, k;

            m = shapeA[rankA - rankB];
            n = shapeB[1];
            k = shapeB[0];

            //memset(Y.i32 + i * m * n, 0, m * n * sizeof(int32_t));
            conv_qtk_sgemm2(0, 0, m, n, k, 1.0, A.u8 + i * m * k, k, B.u8, n,
                        0.0, Y.i32 + i * m * n, n, AZ.u8[0], BZ.u8[0]);
        }
    } else {
        qtk_assert(rankA == rankB && rankA > 2);
        uint32_t nblock = 1;
        for (int i = 0; i < rankA - 2; i++) {
            nblock *= shapeA[i];
        }
        for (int i = 0; i < nblock; i++) {
            int m, n, k;

            m = shapeA[rankA - 2];
            n = shapeB[rankA - 1];
            k = shapeB[rankA - 2];

            //memset(Y.i32 + i * m * n, 0, m * n * sizeof(int32_t));
            conv_qtk_sgemm2(0, 0, m, n, k, 1.0, A.u8 + i * m * k, k,
                        B.u8 + i * k * n, n, 0.0, Y.i32 + i * m * n, n, AZ.u8[0], BZ.u8[0]);
        }
    }


    *instructions += 10;
}

#ifdef __cplusplus
};
#endif
#endif
