#ifndef QBL_NN_QBL_NN_POW_H
#define QBL_NN_QBL_NN_POW_H
#pragma once
...
qtk_maybe_unused static int vm_Pow_infer_shape_(qtk_nn_vm_t *nv,
...
qtk_maybe_unused static void vm_Pow_(qtk_nn_vm_t *nv, uint8_t **instructions) {
    ...
    qtk_assert(QBL_NN_VM_TENSOR_ELEM_F32 == QBL_NN_TENSOR_GET_ELEM_TYPE(b));
    ...
    if (rankB == 0 || (rankB == 1 && shapeB[0] == 1)) {
        ...
        case QBL_NN_VM_TENSOR_ELEM_F32:
            for (idx = 0; idx < nelem; idx++) {
                Y.f32[idx] = qtk_powf(A.f32[idx], B.f32[0]);
            }
            break;
        default:
        ...
        qtk_ndarray_iter_init(&iter, data, rank, shape, shapeA, rankA, 3,
                              elem_szs);
        do {
            qtk_numeric_data_t a, b, y;
            a = iter.data[0];
