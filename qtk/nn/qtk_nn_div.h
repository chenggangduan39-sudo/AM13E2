#ifndef QBL_NN_QBL_NN_DIV_H
#define QBL_NN_QBL_NN_DIV_H
#pragma once
...
qtk_maybe_unused static int vm_Div_infer_shape_(qtk_nn_vm_t *nv,
...
static void div1_(qtk_nn_vm_t *nv, uint32_t *merge_shape, qtk_numeric_data_t A,
    ...
    case QBL_NN_VM_TENSOR_ELEM_F32:
        for (int i = 0; i < pre_dim; i++) {
            qtk_vector_div_elewise(A.f32 + i * last_dim, B.f32,
...
static void div2_(qtk_nn_vm_t *nv, uint32_t *merge_shape, qtk_numeric_data_t A,
                  qtk_numeric_data_t B, qtk_numeric_data_t Y,
                  qtk_nn_vm_tensor_elem_type_t elem_type) {
    uint32_t pre_dim = merge_shape[0];
    uint32_t last_dim = merge_shape[1];
...
qtk_maybe_unused static void vm_Div_(qtk_nn_vm_t *nv, uint8_t **instructions) {
    ...
    if (rankB < rankA) {
        ...
        if (last_merged == rankB) {
            div1_(nv, merge_shape, A, B, Y, elem_type);
        } else {
        ...
        if (rankMerge == rankA) {
            ...
            if (shapeA[rankA - 1] == shapeB[rankB - 1]) {
                ...
                if (shapeA[rankA - 1] == 1) {
                    ...
                    div2_(nv, merge_shape, B, A, Y, elem_type);
                } else if (shapeB[rankA - 1] == 1) {
                    merge_shape[1] = shapeA[rankA - 1];
                    div2_(nv, merge_shape, A, B, Y, elem_type);
                }
            }
        }
    }
