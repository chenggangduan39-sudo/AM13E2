...
qtk_maybe_unused static void vm_AlignBlockOneFrame_(qtk_nn_vm_t *nv, uint8_t **instructions) {
    ...
    *instructions += 30;
    ...
    float *A = Vu + size7;
    float *Yu = A + size8;
    nn_conv2d_naive2_(X_MIC.f32, conv1_out, MW, MB, pads, ksize, strides, out_chan, groups,
                        in_shape, dilations, conv1_workspace, 1, 0);
    nn_conv2d_naive2_(X_REF.f32, conv2_out, RW, RB, pads, ksize, strides, out_chan, groups,
