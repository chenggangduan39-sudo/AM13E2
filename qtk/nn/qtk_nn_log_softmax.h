#ifndef QBL_NN_QBL_NN_LOG_SOFTMAX_H
#define QBL_NN_QBL_NN_LOG_SOFTMAX_H
#pragma once
...
static void nn_log_softmax_(qtk_numeric_data_t X, qtk_numeric_data_t Y,
    ...
    for (uint32_t i = 0; i < row; i++, x_cursor += col) {
        ...
        for (uint32_t j = 0; j < col; j++, y_cursor++) {
            ...
            *y_cursor = qtk_logf(*y_cursor);
        }
    }
}

qtk_maybe_unused static void vm_LogSoftmax_(qtk_nn_vm_t *nv,
    ...
    *instructions += 2;
    ...
    nn_log_softmax_(X, Y, merge_shape[0], merge_shape[1]);
}

qtk_maybe_unused static int vm_LogSoftmax_infer_shape_(qtk_nn_vm_t *nv,
                                                       uint8_t *instructions) {
    uint16_t x = qtk_littleEndian_uint16_from_bin(instructions),
             y = qtk_littleEndian_uint16_from_bin(instructions + 2);
