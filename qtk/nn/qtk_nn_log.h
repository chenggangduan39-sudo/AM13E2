#ifndef QBL_NN_QBL_NN_LOG_H
#define QBL_NN_QBL_NN_LOG_H
#pragma once
...
static void nn_log_f32_(float *x, float *y, uint32_t nelem) {
    for (uint32_t i = 0; i < nelem; i++) {
        y[i] = logf(x[i]);
    }
}

qtk_maybe_unused static void vm_Log_(qtk_nn_vm_t *nv, uint8_t **instructions) {
...
qtk_maybe_unused static int vm_Log_infer_shape_(qtk_nn_vm_t *nv,
