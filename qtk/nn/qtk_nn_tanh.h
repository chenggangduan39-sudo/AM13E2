#ifndef QBL_NN_QBL_NN_TANH_H
#define QBL_NN_QBL_NN_TANH_H
#pragma once
...
static void nn_tanh_f32_(float *x, float *y, uint32_t nelem)
{
    float inv_expx,expx;
    if (x == y) {
        ...
        for (uint32_t i = 0; i < nelem; i++) {
            ...
            {
                inv_expx = expf(-(*x));
                *y = -1.0 + 2.0/(1.0 + inv_expx*inv_expx);
            }else
            {
                expx = expf(*x);
                *y = 1.0 - 2.0/(1.0 + expx*expx);
            }
            x++;
            y++;
        }
...
qtk_maybe_unused static void vm_Tanh_(qtk_nn_vm_t *nv, uint8_t **instructions) {
    ...
    case QBL_NN_VM_TENSOR_ELEM_F32:
        nn_tanh_f32_(X.f32, Y.f32, nelem);
        break;
    default:
        qtk_assert(0);
    }
...
qtk_maybe_unused static int vm_Tanh_infer_shape_(qtk_nn_vm_t *nv,
