...
static void setup_input_dump_(qtk_nnrt_t *rt) {
    int i;
    wtk_mkdir_p(rt->cfg->input_dump_dir, '/', 1);
    int num_in = qtk_nnrt_get_num_in(rt);
    rt->input_dump_file = wtk_calloc(sizeof(FILE *), num_in);
    for (i = 0; i < num_in; i++) {
        char filename[2048];
        char *input_name = qtk_nnrt_net_get_input_name(rt, i);
        if (input_name) {
            snprintf(filename, sizeof(filename), "%s/%s.bin",
                     rt->cfg->input_dump_dir, input_name);
        } else {
            snprintf(filename, sizeof(filename), "%s/%d.bin",
                     rt->cfg->input_dump_dir, i);
        }
        rt->input_dump_file[i] = fopen(filename, "wb");
    }
}

static void cleanup_input_dump_(qtk_nnrt_t *rt) {
    int i;
    int num_in = qtk_nnrt_get_num_in(rt);
    for (i = 0; i < num_in; i++) {
        if (rt->input_dump_file[i]) {
            fclose(rt->input_dump_file[i]);
        }
    }
    wtk_free(rt->input_dump_file);
}

static void dump_input_(qtk_nnrt_t *rt, qtk_nnrt_value_t value, int idx) {
    void *data = qtk_nnrt_value_get_data(rt, value);
    int nbytes = qtk_nnrt_get_input_nbytes(rt, idx);
    fwrite(data, 1, nbytes, rt->input_dump_file[idx]);
}

qtk_nnrt_t *qtk_nnrt_new(qtk_nnrt_cfg_t *cfg) {
    qtk_nnrt_t *rt = wtk_calloc(sizeof(qtk_nnrt_t), 1);
    rt->cfg = cfg;
    rt->input_dump_file = NULL;
#ifdef QTK_NNRT_NCNN
    if (cfg->use_ncnn) {
    ...
    if(cfg->use_rknpu){
        ...
        for (i = 0; i < rt->num_in; i++) {
            ...
            rt->input[i].pass_through = 0;
        }

        memset(rt->outputs, 0, sizeof(rt->outputs));
        for (i = 0; i < rt->num_out; i++) {
    ...
    if (rt->cfg->input_dump_dir) {
        setup_input_dump_(rt);
    }
    return rt;
err:
    wtk_free(rt);
...
int qtk_nnrt_feed(qtk_nnrt_t *rt, qtk_nnrt_value_t value, int idx) {
    ...
    if (rt->cfg->use_ncnn) {
        ...
        if (!ncnn_extractor_input_index(rt->extractor, ncnn_idx,
                                        (ncnn_mat_t)value)) {
            goto err;
        }
        goto end;
...
#ifdef QTK_NNRT_LIBTORCH
    if (rt->cfg->use_libtorch) {
        if (!qtk_nnrt_libtorch_feed(rt->libtorch, value, idx)) {
            goto end;
        }
        goto err;
    }
...
qtk_nnrt_value_t qtk_nnrt_value_create_external(qtk_nnrt_t *rt,
    ...
    if (rt->cfg->use_onnxruntime) {
        ...
        case QTK_NNRT_VALUE_ELEM_I64:
            ...
            return val;
        case QTK_NNRT_VALUE_ELEM_BOOL:
            QTK_ONNXRUNTIME_CALL(
                rt, CreateTensorWithDataAsOrtValue, return NULL;
                , rt->ort_meminfo, data, nelem * nnrt_value_elem_sz_[t], shape,
                shape_len, ONNX_TENSOR_ELEMENT_DATA_TYPE_BOOL, &val);
            return val;
        default:
            return NULL;
        }
...
int qtk_nnrt_get_output_shape(qtk_nnrt_t *rt, int idx, int64_t *shape,
    ...
    if (rt->cfg->use_rknpu) {
        int i;
        if (shape_cap < rt->output_attrs[idx].n_dims) {
            return -1;
        }
        for (i = 0; i < rt->output_attrs[idx].n_dims; i++) {
            shape[i] = rt->output_attrs[idx].dims[i];
        }
        return rt->output_attrs[idx].n_dims;
    }
#endif
    return -1;
}
...
#ifdef QTK_NNRT_RKNPU
    if (rt->cfg->use_rknpu) {
        return rt->output_attrs[idx].size;
    }
#endif
...
qtk_nnrt_value_elem_type_t qtk_nnrt_get_input_elem_type(qtk_nnrt_t *rt,
    ...
    if (rt->cfg->use_onnxruntime) {
        ...
        QTK_ONNXRUNTIME_CALL(rt, SessionGetInputTypeInfo, goto onnx_err;
                             , rt->ort_session, idx, &info);
        QTK_ONNXRUNTIME_CALL(rt, CastTypeInfoToTensorInfo, goto onnx_err;
                             , info, &tensor_info);
        QTK_ONNXRUNTIME_CALL(rt, GetTensorElementType, goto onnx_err;
