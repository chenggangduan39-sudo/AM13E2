...
wtk_matf_t* qtk_durian_duration_predictor_length_regulator(qtk_durian_duration_predictor_t *dur,wtk_matf_t *encoder_in,wtk_matf_t *alis);
...
{
    qtk_durian_duration_predictor_t *dur = NULL;
    int layer = cfg->conv1d_layer,i  =0;
    int *args = NULL;

    dur = wtk_malloc(sizeof(qtk_durian_duration_predictor_t));
    dur->heap = heap;
    dur->cfg = cfg;
...
wtk_matf_t* qtk_durian_duration_predictor_forward(qtk_durian_duration_predictor_t *dur,wtk_matf_t *encoder_in)
{
    int i = 0,j = 0;
    ...
    for(i = 0; i < fc_out->row * fc_out->col; ++i){
        tmp = roundf((expf(fc_out->p[i])-1)*speed);
        alis->p[i] = tmp < 0 ? 0:tmp;
    }
   
    wtk_matf_t *dur_LR = qtk_durian_duration_predictor_length_regulator(dur,encoder_in,alis);
...
int qtk_durian_duration_predictor_delete(qtk_durian_duration_predictor_t *dur)
{
