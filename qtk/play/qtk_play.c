...
int qtk_play_start(qtk_play_t *play)
{
	if(play->cfg->use_get_soundcard)
	{
		...
		play->alsa = qtk_alsa_player_start(play->cfg->snd_name.data, play->cfg->sample_rate, play->cfg->channel, play->cfg->bytes_per_sample, play->cfg->buf_time, play->cfg->use_uac);
	}
	if(!play->alsa){
		return -1;
	}
