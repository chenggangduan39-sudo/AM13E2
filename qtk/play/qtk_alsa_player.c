...
int qtk_alsa_player_stop(qtk_alsa_player_t *player)
{
	free(player);
	return 0;
}

int qtk_alsa_player_write(qtk_alsa_player_t *player,char *data,int bytes)
{
	double t;
...
int qtk_alsa_player_start2(qtk_alsa_player_t *player)
{
	int ret;
	int err = 0;
	...
	// printf("============ sample_rate = %d channel = %d bytes_per_sample = %d buftime = %d\n",
	// 		sample_rate,
	// 		channel,
	// 		bytes_per_sample,
	// 		buf_time);

	// memset(player->zerobuf, 0, player->rate/1000*2*20);

	buf_size=player->rate*player->channel*player->bytes_per_sample*player->buf_time/1000;
	period_size=player->rate*player->channel*player->bytes_per_sample*player->period_time/1000;
	...
	// printf("============ret=%d mrate=%d\n",ret,mrate);
	ret=snd_pcm_hw_params_set_rate_near(player->pcm,hw_params,(unsigned int*)(&(player->rate)), 0);
	// ret=snd_pcm_hw_params_set_rate_resample(player->pcm,hw_params,sample_rate);
	if(ret<0){err = -6; goto end;}
	ret=snd_pcm_hw_params_set_channels(player->pcm, hw_params, player->channel);
	if(ret<0){err = -7; goto end;}

	frame=period_size/(player->bc);
	// printf("frame = %ld\n", frame);
	ret=snd_pcm_hw_params_set_period_size(player->pcm,hw_params,frame,0);
	// ret=snd_pcm_hw_params_set_period_size_near(player->pcm, hw_params, &frame);
	if(ret<0)
	{
		wtk_debug("set peiriod failed\n");
		err = -8; goto end;
	}

	frame=buf_size/(player->bc);
	// printf("frame = %ld\n", frame);
	ret=snd_pcm_hw_params_set_buffer_size(player->pcm,hw_params,frame);
	// ret=snd_pcm_hw_params_set_buffer_size_near(player->pcm, hw_params, &frame);
	if(ret<0)
	{
		wtk_debug("set buf failed\n");
		err = -8; goto end;
	}
	...
	qtk_alsa_player_set_sw_params(player);
...
int qtk_alsa_player_start3(qtk_alsa_player_t *player)
{
	...
	// printf("============%p card=%s sample_rate = %d channel = %d bytes_per_sample = %d buftime = %d\n",player,player->snd_name,
	// 		rate,
	// 		player->channel,
...
// 获取底层支持的最大buffer大小，换算为时间
	...
	// period_time = buffer_time / 4;
	// period_time = 32000;
	period_time = player->buf_time *1000;
...
qtk_alsa_player_t* qtk_alsa_player_start4(
		char *name,
		int sample_rate,
		int channel,
		int bytes_per_sample,
...
long qtk_alsa_player_write(qtk_alsa_player_t *player,char *data,int bytes)
{
	long ret;
	int step;
	...
	if(player->use_uac)
	{
		...
		// if((sm - tm) >= 1000.0)
		// {
		// 	wtk_debug("==================>>>>>>>>>>>>>send len=%d / %f s\n",(sm-tm)/1000.0, sendlen);
		// 	system("cat /proc/asound/UAC1Gadget/pcm0p/sub0/sw_params");
		// 	printf("==========================================================\n");
		// 	int i;
		// 	for(i=0;i<1;++i)
		// 	{
		// 		system("cat /proc/asound/UAC1Gadget/pcm0p/sub0/status");
		// 	}
		// 	printf("==========================================================\n");
		// 	tm = time_get_ms();
		// 	sendlen =0;
	...
	if (ret == -32)
	{
		...
		{
			ret=qtk_alsa_player_write(player, data, bytes);
		}
		//ret=-32;
		kx=0;
		//ret=-32;
