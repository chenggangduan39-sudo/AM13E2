...
static void qtk_heap_adjust(qtk_heap_t *heap) {
    ...
    while (n >= 0) {
        child = 2 * n + 1;
        if (child < heap->capacity &&
            heap->compare_func(heap->list[n], heap->list[child])) {
            tmp = heap->list[n];
...
void qtk_heap_init(qtk_heap_t *heap, int max_size, int up) {
    ...
    heap->capacity = 0;
    heap->list =
        (qtk_heap_node_t *)wtk_calloc(heap->max_size, sizeof(qtk_heap_node_t));
    if (up) {
...
void qtk_heap_clean(qtk_heap_t *heap) { wtk_free(heap->list); }

int qtk_heap_is_empty(qtk_heap_t *heap) { return heap->capacity == 0 ? 1 : 0; }
