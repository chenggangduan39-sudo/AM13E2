...
void _seammasks_modify(cv::UMat &left_img,cv::UMat &right_img,std::vector<cv::Point> &corners);
...
cv::Mat _stitch_seamfinder_create_fullmask(qtk_stitch_seamfinder_t *finder,cv::Mat *mask,int *seammask_points,int point_num, int site)
{
    ...
    // wtk_debug("left_line:%d right_line:%d max %d\n",left_line,right_line,mask->cols);
    //按照这个重新画一个mask
                        ...
                        void* corners,void* imgs, void* masks,int *point,int point_num,int idx)
{
    ...
    //在这里对缝作修饰   不能超过图的一半
    // _seammasks_modify(crop_masks_umat[0],crop_masks_umat[1],crop_corners_new);
    ...
    _seammasks_update(new_seammask,old_seammask,point+((idx*2)*point_num),1);
                            ...
                            void* corners,void* imgs, void* masks,int *point,int point_num,int idx,int split_num,int start,int end) //开始块和结束块
{
    ...
    cv::UMat new_seammask = crop_masks_umat[0];
    cv::UMat old_seammask = the_low_seammasks->at(idx);
    _seammasks_update_block(new_seammask,old_seammask,point+((idx*2)*point_num),1,split_num,start);
    new_seammask = crop_masks_umat[1];
    old_seammask = the_low_seammasks->at(idx+1);
    // char path[128] = {0};
    // sprintf(path,"update_old_seam_%d.png",1);
    // qtk_stitch_image_save_data(path,old_seammask.getMat(cv::ACCESS_RW).data,old_seammask.cols,old_seammask.rows,1);
    _seammasks_update_block(new_seammask,old_seammask,point+(((idx+1)*2+1)*point_num),0,split_num,start);
    // char path[128] = {0};
    // sprintf(path,"update_seam_%d.png",1);
    // qtk_stitch_image_save_data(path,old_seammask.getMat(cv::ACCESS_RW).data,old_seammask.cols,old_seammask.rows,1);
    return;
}
...
void _seammasks_modify(cv::UMat &left_img,cv::UMat &right_img,std::vector<cv::Point> &corners)
{
    int left_rows = left_img.rows;
    int left_cols = left_img.cols;
    int right_rows = right_img.rows;
    int right_cols = right_img.cols;
    int point_x = corners[1].x;
    //算下left上right_img的中间位置
    int point = point_x+right_cols/2+3;
    if(point < left_cols){
        char *data = (char*)left_img.getMat(cv::ACCESS_RW).data;
        for(int i = 0; i < left_rows; ++i){
            memset(data+i*left_cols+point,0,left_cols-point);
        }
    }

    //算下right上的left的中间位置
    point = left_cols/2-point_x-3;
    if(point > 0){
        char *data = (char*)right_img.getMat(cv::ACCESS_RW).data;
        for(int i = 0; i < right_rows; ++i){
            memset(data+i*right_cols,0,point);
        }
    }
...
void _seammasks_update_block(cv::UMat &new_seamask_block,cv::UMat &old_seammask,int *point,int is_right,int split_num,int start)
{
    ...
    // char path[128] = {0};
    // sprintf(path,"new_seammask.png");
    // qtk_stitch_image_save_data(path,new_seammask.data,new_seammask.cols,new_seammask.rows,1);
    // sprintf(path,"old_seammask.png");
    // qtk_stitch_image_save_data(path,old_seammask.getMat(cv::ACCESS_RW).data,old_seammask.cols,old_seammask.rows,1);
...
void qtk_stitch_seamfinder_seam_masks_vector_push(void *seam_masks,unsigned char *data,int rows,int cols)
{
    ...
    new_data = (unsigned char*)m.data;
    if(nrows == rows && ncols == cols){
        memcpy(new_data,data,nrows*ncols);
    }else{
        for(int i = 0; i < nrows; ++i){
            memcpy(new_data+i*ncols,data+i*cols,ncols);
        }
    }
    cv::UMat u = cv::UMat(m.getUMat(cv::ACCESS_RW));
...
void qtk_stitch_seamfinder_seammasks_low2final(void *masks,void* seam_masks,void *seam_masks_low)
{
    ...
    for(int i = 0; i < n; i++){
        ...
        // the_seam_masks->at(i).setTo(0,the_seam_masks->at(i).getMat(cv::ACCESS_READ) < 128);
    }
    // for(int i = 0; i < n; i++){
    //     char pp[128] = {0};
...
void qtk_stitch_seamfinder_seammask_find_point(void *seam_masks,int point_num,int *points, int channels,int idx)
{
    std::vector<cv::UMat> *the_masks = (std::vector<cv::UMat>*)seam_masks;
    _seammask_find_point(the_masks->at(idx), point_num, points, channels);
    return;
}
