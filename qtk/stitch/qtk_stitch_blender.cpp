...
#include "blenders/qtk_blender_multiband.h"
...
class blender_create_weight_table_parallel: public cv::ParallelLoopBody
{
public:
    blender_create_weight_table_parallel(qtk_stitch_blender_t *_blender,std::vector<cv::UMat> *_weight,std::vector<cv::Mat> *_y_offset_map, wtk_strbuf_t **_buf, int _step)
    {
        weights = _weight;
        blender = _blender;
        y_offset_maps = _y_offset_map;
        weight_tabs = _buf;
        step = _step;
    }
    ~blender_create_weight_table_parallel(){};
    void operator()(const cv::Range& range) const;
private:
    std::vector<cv::UMat> *weights;
...
struct qtk_stitch_blender{
    ...
    qtk_blender_multiband_t *multiband;
    ...
    std::vector<cv::UMat>* multiband_weight_maps;
    std::vector<cv::Rect> multiband_sizes; //multiband 新的size
    std::vector<cv::Mat> *multiband_y_offset_maps;
    std::vector<cv::Mat> *multiband_uv_y_offset_maps;
    float blend_width;
    // char *blend_alpha;
    // float *blend_x;
    int use_alpha;
...
typedef struct qtk_stitch_blender_package{
    std::vector<cv::UMat> *weight_maps;
    std::vector<cv::UMat> *uv_weight_maps;
    wtk_strbuf_t **weight_tabs;
    wtk_strbuf_t **uv_weight_tabs;
}qtk_stitch_blender_package_t;
                                        ...
                                        int use_alpha, float smooth, int img_type)
{
    ...
    blender->multiband_weight_maps = new std::vector<cv::UMat>[8];
    blender->multiband_y_offset_maps = new std::vector<cv::Mat>[8];
    blender->multiband_uv_y_offset_maps = new std::vector<cv::Mat>[8];
    return blender;
}
...
void qtk_stitch_blender_delete(qtk_stitch_blender_t* blender)
{
    ...
    if(blender->multiband){
        blender->multiband = NULL;
    }
    if(blender->multiband_weight_maps){
        delete[] blender->multiband_weight_maps;
        blender->multiband_weight_maps = NULL;
    }
    if(blender->multiband_y_offset_maps){
        delete[] blender->multiband_y_offset_maps;
        blender->multiband_y_offset_maps = NULL;
    }
    if(blender->multiband_uv_y_offset_maps){
        delete[] blender->multiband_uv_y_offset_maps;
        blender->multiband_uv_y_offset_maps = NULL;
    }
...
void _feed(qtk_stitch_blender_t* blender, cv::Mat &img, cv::Mat &mask, cv::Point &corner,int idx)
{
    ...
    if(blender->multiband){
        // qtk_blender_multiband_feed(blender->multiband,img,mask,corner);
        qtk_blender_multiband_feed2(blender->multiband,img,corner,blender->multiband_sizes[idx],
                                blender->multiband_weight_maps[idx],blender->multiband_y_offset_maps[idx]);
...
void _feed_nv12(qtk_stitch_blender_t* blender, cv::Mat &img, cv::Mat &mask, cv::Point &corner,int idx)
{
    ...
    // wtk_debug("%d -> %d\n",img.type(),uM.type());
    // double t = time_get_ms();
    if(blender->multiband != NULL){
        qtk_blender_multiband_nv12_feed2(blender->multiband,img,corner,blender->multiband_sizes[idx],
            blender->multiband_weight_maps[idx],blender->multiband_y_offset_maps[idx],blender->multiband_uv_y_offset_maps[idx]);
...
cv::Mat _blend2(qtk_stitch_blender_t* blender)
{
    ...
    if(blender->multiband){
        qtk_blender_multiband_blend(blender->multiband,result,dst_mask);
...
void qtk_stitch_blender_prepare2(qtk_stitch_blender_t* blender,void *corners, void *sizes, void *ptr)
{
    ...
    if(blender->multiband != NULL){
        if(blender->img_type == QTK_STITCH_USE_IMG_RGB){
            qtk_blender_multiband_prepare2(blender->multiband,ptr);
            // qtk_blender_multiband_prepare(blender->multiband);
        }else if(blender->img_type == QTK_STITCH_USE_IMG_NV12){
            qtk_blender_multiband_nv12_prepare2(blender->multiband,ptr);
        }
    }else if(blender->feather != NULL){
        if(blender->img_type == QTK_STITCH_USE_IMG_RGB){
...
//作为拆分 _prepare 的函数
void qtk_stitch_blender_start(qtk_stitch_blender_t* blender,void *seam_masks, void *corners, void *sizes)
{
    std::vector<cv::Point> *roi_corners = (std::vector<cv::Point>*) corners;
    std::vector<cv::Size> *roi_sizes = (std::vector<cv::Size>*) sizes;
    ...
    if(blender->type == QTK_STITCH_BLENDER_NO || blender->blend_width < 1){
        ...
        //设置宽度
        // int num_bands_ = blender->blend_strength;
        // dst_sz.width += ((1 << num_bands_) - dst_sz.width % (1 << num_bands_)) % (1 << num_bands_);
        // dst_sz.height += ((1 << num_bands_) - dst_sz.height % (1 << num_bands_)) % (1 << num_bands_);
        // int num = log(blender->blend_width)/log(2.0f) - 1;
        blender->multiband = qtk_blender_multiband_new(blender->blend_strength,dst_sz);
        qtk_blender_multiband_corners2newtl(blender->multiband,*roi_corners,*roi_sizes, blender->multiband_sizes);
        qtk_blender_multiband_weightmaps(blender->multiband,*masks,*roi_corners,blender->multiband_sizes,blender->multiband_weight_maps);
        if(blender->img_type == QTK_STITCH_USE_IMG_RGB){
            qtk_blender_multiband_yoffsetmaps(blender->multiband,dst_sz,blender->multiband_sizes,
                                    blender->multiband_y_offset_maps,3);
        }else if(blender->img_type == QTK_STITCH_USE_IMG_NV12){
            qtk_blender_multiband_yoffsetmaps(blender->multiband,dst_sz,blender->multiband_sizes,
                blender->multiband_y_offset_maps,1);
            qtk_blender_multiband_uv_yoffsetmaps(blender->multiband,dst_sz,blender->multiband_sizes,
                blender->multiband_uv_y_offset_maps);
        }
    }else if(blender->type == QTK_STITCH_BLENDER_FEATHER){
...
void qtk_stitch_blender_stop(qtk_stitch_blender_t* blender)
{
    ...
    if(blender->feather){
        ...
        wtk_strbufs_reset(blender->uv_weight_tabs,8);
        wtk_strbufs_reset(blender->weight_tabs,8);
        qtk_blender_feather_delete(blender->feather);
        blender->feather = NULL;
    }
    if(blender->blender_no){
...
void qtk_stitch_blender_blend2(qtk_stitch_blender_t* blender)
{
    if(blender->type == QTK_STITCH_BLENDER_MULTIBAND){
        if(blender->img_type == QTK_STITCH_USE_IMG_RGB){
            qtk_blender_multiband_blend2(blender->multiband);
        }else if(blender->img_type ==QTK_STITCH_USE_IMG_NV12){
            qtk_blender_multiband_nv12_blend2(blender->multiband);
        }
    }
    return;
}
...
void qtk_stitch_blender_get_rect(qtk_stitch_blender_t* blender,void *corners, void *sizes,int *rows, int *cols)
{
    ...
    dst_sz = cv::detail::resultRoi(*roi_corners,*roi_sizes);
    if(blender->type == QTK_STITCH_BLENDER_MULTIBAND){
        qtk_blender_multiband_to_rect(blender->blend_strength,dst_sz);
    }
    *rows = dst_sz.height;
    *cols = dst_sz.width;
    return;
...
void blender_create_weight_table_parallel::operator()(const cv::Range& range) const
{
    int i = range.start;
    cv::Mat weight = (*weights)[i].getMat(cv::ACCESS_READ);
...
void _blender_create_weight_table(cv::Mat &weight,cv::Mat &y_offset, wtk_strbuf_t *buf,int step)
{
    ...
    for(int i = 0; i < rows; ++i){
        if(t == -1){
            if(ww[0] == 1<<QTK_STITCH_FIX_POINT){
                t = 1; //这个权重就是1
            }else if(ww[0] == 0){
                t = 0; //这个权重是0
            }else{
                                        ...
                                        std::vector<cv::UMat> &weight_maps,wtk_strbuf_t **weight_tabs,int step)
{
    ...
    // int n = blender->weight_maps.size();
    // blender_create_weight_table_parallel weight_table = blender_create_weight_table_parallel(blender,&weight_maps,&y_offset_maps,weight_tabs,step);
    // cv::parallel_for_(cv::Range(0,n),weight_table,-1.0);
    return;
}

void qtk_stitch_blender_draw_seam_lines(qtk_stitch_blender_t* blender,void *seam_masks, void *corners)
{
    std::vector<cv::UMat> *masks = (std::vector<cv::UMat>*)seam_masks;
    std::vector<cv::Point> *roi_corners = (std::vector<cv::Point>*)corners;
    int n = masks->size();

    int rows = masks->at(0).rows;
    int cols = roi_corners->at(n-1).x+masks->at(n-1).cols;
    cv::Mat mm(cv::Size(cols,rows),CV_8UC1);
    mm.setTo(0);
    for(int i = 1; i < n; i+=2){
        cv::Rect roi(roi_corners->at(i).x, roi_corners->at(i).y,
            masks->at(i).cols, masks->at(i).rows);
        add(mm(roi), masks->at(i), mm(roi));
    }
    cv::Mat edges;
    cv::Canny(mm,edges,100,200);
    cv::Mat kernel = cv::Mat::ones(cv::Size(3,3),CV_8UC1);
    cv::Mat seam_lines; 
    cv::dilate(edges,seam_lines,kernel);
    if(blender->feather){
        qtk_blender_feather_draw_seam_lines(blender->feather,seam_lines);
    }else if(blender->blender_no){
        qtk_blender_no_draw_seam_lines(blender->blender_no,seam_lines);
    }
    return;
}

//使用后会清空原始数据
void* qtk_stitch_blender_get_blender_package(qtk_stitch_blender_t* blender)
{
    qtk_stitch_blender_package_t *package = NULL;
    package = (qtk_stitch_blender_package_t*)wtk_malloc(sizeof(qtk_stitch_blender_package_t));
    package->weight_maps = new std::vector<cv::UMat>;
    package->uv_weight_maps = new std::vector<cv::UMat>;
    
    package->weight_tabs = blender->weight_tabs;
    blender->weight_tabs = NULL;
    blender->weight_tabs = wtk_strbufs_new(8);
    package->uv_weight_tabs = blender->uv_weight_tabs;
    blender->uv_weight_tabs = NULL;
    blender->uv_weight_tabs = wtk_strbufs_new(8);

    package->weight_maps->resize(blender->weight_maps.size());
    for(unsigned int i = 0; i < blender->weight_maps.size();++i){
        package->weight_maps->at(i) = blender->weight_maps[i].clone();
    }
    
    package->uv_weight_maps->resize(blender->uv_weight_maps.size());
    for(unsigned int i = 0; i < blender->uv_weight_maps.size();++i){
        package->uv_weight_maps->at(i) = blender->uv_weight_maps[i].clone();
    }
    // blender->weight_maps.clear();
    // blender->uv_weight_maps.clear();
    return package;
...
void qtk_stitch_blender_set_blender_package(qtk_stitch_blender_t* blender, void *_package)
{
    qtk_stitch_blender_package_t *package = (qtk_stitch_blender_package_t *)_package;
    ...
    blender->weight_tabs = package->weight_tabs;
    blender->uv_weight_tabs = package->uv_weight_tabs;

    blender->weight_maps.resize(package->weight_maps->size());
    for(unsigned int i = 0; i < package->weight_maps->size(); ++i){
        blender->weight_maps[i] = package->weight_maps->at(i);
    }

    blender->uv_weight_maps.resize(package->uv_weight_maps->size());
    for(unsigned int i = 0; i < package->uv_weight_maps->size(); ++i){
        blender->uv_weight_maps[i] = package->uv_weight_maps->at(i);
    }

    delete package->weight_maps;
    delete package->uv_weight_maps;
    wtk_free(package);
    return;
}


void qtk_stitch_blender_update_seammasks_weightmaps(qtk_stitch_blender_t* blender,void *seam_masks, void *corners, void *sizes, int use_table)
{
    std::vector<cv::Point> *roi_corners = (std::vector<cv::Point>*) corners;
    std::vector<cv::Size> *roi_sizes = (std::vector<cv::Size>*) sizes;
    std::vector<cv::UMat> *masks = (std::vector<cv::UMat>*)seam_masks;
...
void qtk_stitch_blender_save_seammasks(qtk_stitch_blender_t* blender,void *seam_masks)
{
    std::vector<cv::UMat> *masks = (std::vector<cv::UMat>*)seam_masks;
    for(unsigned int i = 0; i < masks->size(); ++i){
        cv::Mat m = masks->at(i).getMat(cv::ACCESS_READ);
        int w = m.cols;
        int h = m.rows;
        char path[128] = {0};
        sprintf(path,"seam_mask_%d_%d_%d.bin",i,h,w);
        FILE *f = fopen(path,"wb");
        fwrite(m.data,w*h,1,f);
        fclose(f);
    }
    return;
}

void* qtk_stitch_blender_get_weight_table(qtk_stitch_blender_t *blender)
{
    return blender->weight_tabs;
}

void* qtk_stitch_blender_get_uv_weight_table(qtk_stitch_blender_t *blender)
{
    return blender->uv_weight_tabs;
}