...
void _crop_remaps_imgs_single(qtk_stitch_run_phase_t* phase);
...
void _draw_seam_lines(qtk_stitch_run_phase_t *phase);

typedef struct{
    void *blender_pack;
    void *reseam_mask;
}qtk_stitch_run_phase_package_t;
...
{
    int seam_size = row * col;
    unsigned char *seam = wtk_malloc(sizeof(unsigned char)*seam_size);
    FILE *fp = fopen(path,"rb");
    unsigned char *p = seam;
    int ret = 0;
    do{
        ret = fread(p,seam_size,1,fp);
        if(ret <= 0){
            break;
        }
        seam_size -= ret;
...
void qtk_stitch_run_init(qtk_stitch_run_phase_t *phase)
{
    ...
    if(phase->cfg->use_compensator || phase->cfg->use_reseammask){
        ...
        if(phase->cfg->use_limit_region){
            qtk_stitch_cropper_limit_region_masks(phase->low_cropper);
        }
        ...
        if(phase->low_cropper->remap_type == 0){ //曝光不用nearest remap
    ...
    if(phase->cfg->remap_type == 0){
        ...
        if(phase->cfg->image_direction==0){
        ...
        qtk_stitch_corpper_clear_crop_maps(phase->cropper);
        if(phase->cfg->image_direction==0){
            qtk_stitch_corpper_nearest_uv_2tables(phase->cropper);
        }
    }
...
qtk_stitch_run_phase_t* qtk_stitch_run_phase_new(qtk_stitch_run_cfg_t *cfg)
{
    ...
    phase->cropper = qtk_stitch_cropper_new(cfg->crop,cfg->camera_num,cfg->remap_type,cfg->src_type,cfg->images_arrangement,cfg->image_direction);
    if(cfg->use_compensator || cfg->use_reseammask){
        phase->low_warper = qtk_stitch_warper_new(cfg->warper_type,cfg->camera_num);
        phase->low_cropper = qtk_stitch_cropper_new(cfg->crop,cfg->camera_num,1,0,cfg->images_arrangement,cfg->image_direction);
    }
    ...
    phase->blender = qtk_stitch_blender_new(cfg->blender_type,cfg->blend_strength,
                        cfg->use_gpu,0,cfg->smooth,cfg->src_type);
...
void qtk_stitch_run_phase_estimate_esposure(qtk_stitch_run_phase_t *phase)
{
    if(phase->cfg->use_compensator){
        ...
        if(phase->cfg->use_compensator_on_single && phase->cfg->use_reseammask == 0){
...
int qtk_stitch_run_phase_start(qtk_stitch_run_phase_t *phase)
{
    ...
    qtk_stitch_blender_start(phase->blender,phase->reseam_masks,
                            phase->cropper->corners,phase->cropper->size);
...
void qtk_stitch_run_phase_run(qtk_stitch_run_phase_t *phase)
{
    ...
    _crop_remaps_imgs_single(phase);
    wtk_debug("%f\n",time_get_ms()-t);
    ...
    wtk_debug("%f\n",time_get_ms()-t);
    if(phase->cfg->use_draw_seam_lines){
        _draw_seam_lines(phase);
    }
    if(phase->cfg->blender_type == QTK_STITCH_BLENDER_MULTIBAND){
...
void qtk_stitch_run_phase_push_data(qtk_stitch_run_phase_t *phase, float channel, void *img)
{
    ...
    if(phase->cfg->use_all_img == 0){
        ...
        if(phase->cfg->image_direction==0){
            ...
            data->img_data = qtk_stitch_image_new(phase->cfg->w,phase->cfg->h*phase->cfg->camera_num,phase->cfg->medium_megapix,phase->cfg->low_megapix,-1);
        }
    }
...
void _crop_remaps_imgs_single(qtk_stitch_run_phase_t* phase)
{
    if(phase->cfg->use_all_img == 0){
        ...
        if(phase->cfg->blender_type == QTK_STITCH_BLENDER_MULTIBAND){
            ...
            // qtk_stitch_corpper_crop_remaps2_all(phase->cropper,&phase->img_queue);
            void *weight_table = qtk_stitch_blender_get_weight_table(phase->blender);
            void *uv_weight_table = qtk_stitch_blender_get_uv_weight_table(phase->blender);
            qtk_stitch_corpper_crop_remaps3_all(phase->cropper,&phase->img_queue,weight_table,uv_weight_table);
        }
    }
    return;
}
...
void __run_phase_create_final_panorama(qtk_stitch_run_phase_t* phase)
{
    ...
    qtk_stitch_blender_blend2(phase->blender);
    return;
}
...
int _run_phase_re_charge_seamask_scores(qtk_stitch_run_phase_t *phase, u_char *update_list)
{
    ...
    if(update_src == 1){
        ...
        if(phase->update_count == phase->cfg->seammask_to_standard){
            update = 3;
...
int _run_phase_re_charge_seammask(qtk_stitch_run_phase_t *phase)
{
    ...
    if(update == 3){
        ...
        if(phase->cfg->use_compensator && phase->cfg->use_compensator_on_single){
            qtk_stitch_blender_update_seammasks_weightmaps(phase->blender,phase->reseam_masks,phase->cropper->corners,phase->cropper->size,0);
        }else{
            qtk_stitch_blender_update_seammasks_weightmaps(phase->blender,phase->reseam_masks,phase->cropper->corners,phase->cropper->size,1);
        }
        ret = 1;
    }else if(update == 1){
        qtk_stitch_seamfinder_seam_masks_delete(phase->reseam_masks_low);
...
void _draw_seam_lines(qtk_stitch_run_phase_t *phase)
{
    qtk_stitch_blender_draw_seam_lines(phase->blender,phase->reseam_masks,phase->cropper->corners);
    return;
}

void* qtk_stitch_run_phase_get_package(qtk_stitch_run_phase_t *phase)
{
    qtk_stitch_run_phase_package_t *package = NULL;
    package = (qtk_stitch_run_phase_package_t*)wtk_malloc(sizeof(qtk_stitch_run_phase_package_t));
    package->blender_pack = qtk_stitch_blender_get_blender_package(phase->blender);
    if(phase->cfg->use_draw_seam_lines){
        package->reseam_mask = qtk_stitch_seamfinder_seammasks_copy(phase->reseam_masks);
    }
    return (void*)package;
}

void qtk_stitch_run_phase_set_package(qtk_stitch_run_phase_t *phase, void *_package)
{
    qtk_stitch_run_phase_package_t *package = (qtk_stitch_run_phase_package_t*)_package;
    qtk_stitch_blender_set_blender_package(phase->blender,package->blender_pack);
    if(phase->cfg->use_draw_seam_lines){
        qtk_stitch_seamfinder_seam_masks_delete(phase->reseam_masks);
        phase->reseam_masks = package->reseam_mask;
    }
    wtk_free(package);
    return;
}

void qtk_stitch_run_phase_save_seammasks(qtk_stitch_run_phase_t *phase)
{
    qtk_stitch_blender_save_seammasks(phase->blender,phase->reseam_masks);
    return;
}