...
void _stitch_remap_linear2_1channel(cv::Mat &img,cv::Mat &dst,cv::Mat &offmap,cv::Mat &uymap,cv::Mat &weight_table);
void _initInterTab2D(void);
static void _initInterTab2D2(void);
void _create_nv12_uv_offset_map_vertical(qtk_stitch_cropper_t* cropper,cv::UMat &uxmap,cv::UMat &uymap,
    cv::Mat &nv_offset_map,int x_offset,int step);
void _create_nv12_offset_map_vertical(qtk_stitch_cropper_t* cropper,
    cv::UMat &uxmap,cv::UMat &uymap,cv::Mat &offset_map,int x_offset,int step);
void _create_offset_map_vertical(qtk_stitch_cropper_t* cropper,
    cv::UMat &uxmap,cv::UMat &uymap,cv::Mat &offset_map,int x_offset,int step);
void _stitch_remap_linear_1channel_vertical_rect(cv::Mat &img,cv::Mat &dst,cv::Mat &offset_map,cv::Mat &uymap, int s, int e);
void _stitch_remap_linear_vertical_rect(cv::Mat &img,cv::Mat &dst,cv::Mat &offset_map,cv::Mat &uymap, int s, int e);
void _stitch_remap_linear2_1channel_rect(cv::Mat &img,cv::Mat &dst,cv::Mat &offmap,cv::Mat &uymap,
    cv::Mat &weight_table,int s, int e);
...
qtk_stitch_cropper_t* qtk_stitch_cropper_new(int do_crop,int ncamera,int remap_type, int img_type,int images_arrangement,int image_direction)
{
    ...
    cropper->images_arrangement = images_arrangement;
    cropper->image_direction = image_direction;
...
void _crop_images(qtk_stitch_cropper_t* cropper,cv::Mat* imgs, float aspect,cv::Mat* output)
{
    ...
    if(cropper->do_crop){
        for(int i = 0,idx=0; i < n; ++i,++idx){
            ...
            output[i] = imgs[i](cv::Range(rr.y,min_y),cv::Range(rr.x,min_x));
        }
...
void _crop_image(qtk_stitch_cropper_t* cropper,cv::Mat *img, float aspect,cv::Mat *output,int idx)
{
    ...
    if(cropper->do_crop){
        ...
        *output = img[0](cv::Range(rr.y,min_y),cv::Range(rr.x,min_x));
        return;
    }else{
        *output = *img;
    }
...
cv::Mat get_xymap_grade(cv::Mat &xymap,int offset,int gaid,int min, int max)
{
    int rows = xymap.rows-(xymap.rows%16);
    int cols = xymap.cols-(xymap.cols%16);
    int grade_cols = cols/gaid + 1 + (cols%gaid > 0 ? 1 : 0);
    int grade_rows = rows/gaid + 1 + (rows%gaid > 0 ? 1 : 0);
    printf("%d %d %d %d %d %d %d\n",grade_cols,grade_rows,gaid,xymap.rows,xymap.cols,rows,cols);
    cv::Mat grade_xymap = cv::Mat(grade_rows,grade_cols,CV_32FC1);

    for(int i = 0; i < grade_rows; i++){
        int x = i * gaid;
        if(x >= rows){
            x = rows - 1;
        }
        for(int j = 0; j < grade_cols; j++){
            int y = j * gaid;
            if(y > cols){
                y = cols - 1;
            }
            grade_xymap.at<float>(i,j) = xymap.at<float>(x,y)+offset;
            if(grade_xymap.at<float>(i,j) < min){
                grade_xymap.at<float>(i,j) = min;
            }else if(grade_xymap.at<float>(i,j) > max){
                grade_xymap.at<float>(i,j) = max;
            }
        }
    }
...
cv::Mat get_xymap_grade_vertical(cv::Mat &xymap,int w, int offset,int gaid,int min, int max)
{
    int rows = xymap.rows-(xymap.rows%16);
    int cols = xymap.cols-(xymap.cols%16);
    int grade_cols = cols/gaid + 1 + (cols%gaid > 0 ? 1 : 0);
    ...
    for(int i = 0; i < grade_rows; i++){
        ...
        for(int j = 0; j < grade_cols; j++){
            ...
            if(w > 0){
                grade_xymap.at<float>(i,j) = w-xymap.at<float>(x,y);
            }else{
                grade_xymap.at<float>(i,j) = xymap.at<float>(x,y);
            }
            grade_xymap.at<float>(i,j) = grade_xymap.at<float>(i,j)+offset;
            if(grade_xymap.at<float>(i,j) < min){
                grade_xymap.at<float>(i,j) = min;
            }else if(grade_xymap.at<float>(i,j) > max){
                grade_xymap.at<float>(i,j) = max;
...
cv::Mat get_xymap_grade_vertical_v(cv::Mat &xymap,int w, int offset,int gaid,int min, int max)
{
    int rows = xymap.rows-(xymap.rows%16);
    int cols = xymap.cols-(xymap.cols%16);
    int grade_rows = cols/gaid + 1 + (cols%gaid > 0 ? 1 : 0);
    int grade_cols = rows/gaid + 1 + (rows%gaid > 0 ? 1 : 0);
    printf("%d %d %d %d %d %d %d\n",grade_cols,grade_rows,gaid,xymap.rows,xymap.cols,rows,cols);
    cv::Mat grade_xymap = cv::Mat(grade_rows,grade_cols,CV_32FC1);
...
cv::Mat get_xymap_grade_big_map(std::vector<cv::UMat>* maps,int gaid,int is_x,int offset,int max,int min)
{
    cv::Mat xymap;

    for(unsigned int i = 0; i < maps->size(); ++i){
        cv::Mat mm = maps->at(i).getMat(cv::ACCESS_RW);
        mm+=offset*i;
    }

    cv::hconcat(*maps,xymap);
...
void qtk_stitch_corpper_crop_maps(qtk_stitch_cropper_t* cropper,void *imgs1,void *imgs2, float aspect)
{
    ...
    for(int k = 0; k < n; k++){
        ...
        tmp1.push_back((*uxmaps)[k](cv::Range(rr.y,min_y),cv::Range(rr.x,min_x)));
        tmp2.push_back((*uymaps)[k](cv::Range(rr.y,min_y),cv::Range(rr.x,min_x)));
    ...
    // for(int k = 0; k < n; k++){
    //     char path[256] = {};
    //     cv::Mat uxmap = ((std::vector<cv::UMat>*)cropper->crop_uxmaps)->at(k).getMat(cv::ACCESS_RW);
    //     cv::Mat uxmap_g  = get_xymap_grade(uxmap,k*cropper->width,16,k*cropper->width,(k+1)*cropper->width);
    //     sprintf(path,"%d_%d_%d_xmap.bin",uxmap_g.cols,uxmap_g.rows,k);
    //     FILE *fp = fopen(path,"wb");
    //     fwrite(uxmap_g.ptr(),uxmap_g.cols*uxmap_g.rows*sizeof(float),1,fp);
    //     fclose(fp);
    //     cv::Mat uymap = ((std::vector<cv::UMat>*)cropper->crop_uymaps)->at(k).getMat(cv::ACCESS_RW);
    //     cv::Mat uymap_g  = get_xymap_grade(uymap,0,16,0,cropper->hight);
    //     sprintf(path,"%d_%d_%d_ymap.bin",uymap_g.cols,uymap_g.rows,k);
    //     fp = fopen(path,"wb");
    //     fwrite(uymap_g.ptr(),uymap_g.cols*uymap_g.rows*sizeof(float),1,fp);
    //     fclose(fp);
    // }
    // for(int k = 0; k < n; k++){
    //     char path[256] = {};
    //     cv::Mat uxmap = ((std::vector<cv::UMat>*)cropper->crop_uxmaps)->at(k).getMat(cv::ACCESS_RW);
    //     cv::Mat uxmap_g  = get_xymap_grade_vertical(uxmap,0,0,16,0,cropper->width);
    //     sprintf(path,"%d_%d_%d_ymap.bin",uxmap_g.cols,uxmap_g.rows,k);
    //     FILE *fp = fopen(path,"wb");
    //     fwrite(uxmap_g.ptr(),uxmap_g.cols*uxmap_g.rows*sizeof(float),1,fp);
    //     fclose(fp);
    //     cv::Mat uymap = ((std::vector<cv::UMat>*)cropper->crop_uymaps)->at(k).getMat(cv::ACCESS_RW);
    //     cv::Mat uymap_g  = get_xymap_grade_vertical(uymap,cropper->hight,k*cropper->hight,16,k*cropper->hight,(k+1)*cropper->hight);
    //     sprintf(path,"%d_%d_%d_xmap.bin",uymap_g.cols,uymap_g.rows,k);
    //     fp = fopen(path,"wb");
    //     fwrite(uymap_g.ptr(),uymap_g.cols*uymap_g.rows*sizeof(float),1,fp);
    //     fclose(fp);
    // }
    ...
    //     char path[256] = {};
    //     cv::Mat uxmap = ((std::vector<cv::UMat>*)cropper->crop_uxmaps)->at(k).getMat(cv::ACCESS_RW);
    //     cv::Mat uxmap_g  = get_xymap_grade_vertical_v(uxmap,0,0,16,0,cropper->width);
    //     sprintf(path,"%d_%d_%d_ymap.bin",uxmap_g.cols,uxmap_g.rows,k);
    //     FILE *fp = fopen(path,"wb");
    //     fwrite(uxmap_g.ptr(),uxmap_g.cols*uxmap_g.rows*sizeof(float),1,fp);
    //     fclose(fp);
    //     cv::Mat uymap = ((std::vector<cv::UMat>*)cropper->crop_uymaps)->at(k).getMat(cv::ACCESS_RW);
    //     cv::Mat uymap_g  = get_xymap_grade_vertical_v(uymap,0,k*cropper->hight,16,k*cropper->hight,(k+1)*cropper->hight);
    //     sprintf(path,"%d_%d_%d_xmap.bin",uymap_g.cols,uymap_g.rows,k);
    //     fp = fopen(path,"wb");
    //     fwrite(uymap_g.ptr(),uymap_g.cols*uymap_g.rows*sizeof(float),1,fp);
    //     fclose(fp);
    ...
    // char path[256] = {};
    // FILE *fp = NULL;
    // cv::Mat uxmap_g = get_xymap_grade_big_map((std::vector<cv::UMat>*)cropper->crop_uxmaps,16,1,cropper->width,cropper->width*3,0);
    // sprintf(path,"%d_%d_big_xmap.bin",uxmap_g.cols,uxmap_g.rows);
    // fp = fopen(path,"wb");
    // fwrite(uxmap_g.ptr(),uxmap_g.cols*uxmap_g.rows*sizeof(float),1,fp);
    // fclose(fp);
    // cv::Mat uymap_g = get_xymap_grade_big_map((std::vector<cv::UMat>*)cropper->crop_uymaps,16,0,0,cropper->hight,0);
    // sprintf(path,"%d_%d_big_ymap.bin",uymap_g.cols,uymap_g.rows);
    // fp = fopen(path,"wb");
    // fwrite(uymap_g.ptr(),uymap_g.cols*uymap_g.rows*sizeof(float),1,fp);
    // fclose(fp);
    // exit(1);
                        ...
                        int x_offset,int step)
{
    ...
    int w = 0;
    int h = 0;
    int rowsh = cropper->hight;
    if(cropper->remap_type == cv::INTER_LINEAR){
        w = cropper->width-1;
        h = cropper->hight+x_offset-1;
        puxs = uxmap.getMat(cv::ACCESS_RW).ptr<short>(0,0);
    }else{
        w = cropper->width;
        h = cropper->hight+x_offset;
        pux = uxmap.getMat(cv::ACCESS_RW).ptr<float>(0,0);
        puy = uymap.getMat(cv::ACCESS_RW).ptr<float>(0,0);
    }
    ...
    if(uxmap.channels() == 1){
        for(int i = 0; i < rows; i++){
            for(int j = 0; j < cols; j++){
                int puxi = pux[i*cols+j]; //是x轴的偏移
                int puyi = rowsh-puy[i*cols+j]+x_offset; //是y轴的偏移
                if(puxi >= w || puyi >= h || puxi < 0 || puyi < 0){
                    dmp[i*cols+j] = 0;
                }else{
    #ifdef USE_9391
                    dmp[i*cols+j] = puyi*4+puxi*4*step;
    #else
                    dmp[i*cols+j] = puyi*3+puxi*3*step;
    #endif
                    // printf("%d\n",puxi*3+puyi*3*w);
                }
            }
        ...
        for(int i = 0; i < rows; i++){
            for(int j = 0; j < cols; j++){
                int k = (i*cols+j)*2;
                int puxi = puxs[k]; //是x轴的偏移
                int puyi = rowsh-puxs[k+1]+x_offset; //是y轴的偏移
                if(puxi >= w || puyi >= h || puxi < 0 || puyi < 0){
                    dmp[i*cols+j] = 0;
                }else{
    #ifdef USE_9391
                    dmp[i*cols+j] = (puyi-1)*4+puxi*4*step;
    #else
                    dmp[i*cols+j] = (puyi-1)*3+puxi*3*step;
    #endif
                    // printf("%d\n",puxi*3+puyi*3*w);
                }
            }
        ...
        for(int i = 0; i < rows; i++){
            for(int j = 0; j < cols; j++){
                ...
                if(puxi >= w || puyi >= h || puxi < 0 || puyi < 0){
                    ...
                    dmp[i*cols+j] = puyi+puxi*step;
                    // printf("(%d,%d) %d %d %d\n",i,j,dmp[i*cols+j],puxi,puyi);
                }
            }
        }
    }else if(uxmap.channels() == 2){
                        ...
                        cv::Mat &nv_offset_map,int x_offset,int step)
{
    ...
    if(cropper->remap_type == cv::INTER_LINEAR){
        w = cropper->width/2-1;
        h = (cropper->hight+x_offset)/2-1;
        puxs = uxmap.getMat(cv::ACCESS_RW).ptr<short>(0,0);
    }else{
        w = cropper->width/2;
        h = (cropper->hight+x_offset)/2;
        pux = uxmap.getMat(cv::ACCESS_RW).ptr<float>(0,0);
        puy = uymap.getMat(cv::ACCESS_RW).ptr<float>(0,0);
    }
    
    base = cropper->width*step;
    // printf("%d \n",base);
    if(uxmap.channels() == 1){
        for(int i = 0; i < rows; ++i){
            for(int j = 0; j < cols; j++){
                int puxi = pux[(i*xcols+j)*2]/2; //是x轴的偏移
                int puyi = (rowsh-puy[(i*xcols+j)*2]+x_offset)/2; //是y轴的偏移
                if(puxi >= w || puyi >= h || puxi < 0 || puyi < 0){
                    dmp[i*cols+j] = base;
                }else{
                    dmp[i*cols+j] = base+puyi*2+puxi*step;//puyi*step/2*2;
                    // printf("%d\n",puxi*3+puyi*3*w);
                }
            }
        }
        ...
        for(int i = 0; i < rows; ++i){
            for(int j = 0; j < cols; ++j){
                int k = (i*xcols+j)*2*2;
                int puxi = (puxs[k])/2; //是x轴的偏移
                int puyi = (rowsh-puxs[k+1]+x_offset)/2; //是y轴的偏移
                if(puxi >= w || puyi >= h || puxi < 0 || puyi < 0){
                    dmp[i*cols+j] = base;
                }else{
                    dmp[i*cols+j] = base+puyi*2+puxi*step;//puyi*step/2*2;
...
void qtk_stitch_corpper_create_offset_maps(qtk_stitch_cropper_t* cropper)
{
    ...
    for(int idx = 0; idx < size; idx++){
        ...
        if(cropper->images_arrangement == 0){
            if(cropper->image_direction == 0){
                ...
                if(cropper->img_type == QTK_STITCH_USE_IMG_NV12){
                    _create_nv12_offset_map_vertical(cropper,uxmap,uymap,offset_maps[idx],0,cropper->hight);
                    uv_offset_maps[idx].create(uxmap.rows/2,uxmap.cols/2,CV_32S);
                    _create_nv12_uv_offset_map_vertical(cropper,uxmap,uymap,uv_offset_maps[idx],0,cropper->hight);
                }else if(cropper->img_type == QTK_STITCH_USE_IMG_RGB){
                    _create_offset_map_vertical(cropper,uxmap,uymap,offset_maps[idx],0,cropper->hight);
                }
            }
        }
    }
    ...
    for(int idx = 0; idx < size; idx++){
        ...
        if(cropper->images_arrangement == 0){
            if(cropper->image_direction == 0){
                ...
                int step = cropper->hight*cropper->ncamera;
                if(cropper->img_type == QTK_STITCH_USE_IMG_NV12){
                    _create_nv12_offset_map_vertical(cropper,uxmap,uymap,offset_maps[idx],idx*cropper->hight,step);
                    uv_offset_maps[idx].create(uxmap.rows/2,uxmap.cols/2,CV_32S);
                    _create_nv12_uv_offset_map_vertical(cropper,uxmap,uymap,uv_offset_maps[idx],idx*cropper->hight,step);
                }else if(cropper->img_type == QTK_STITCH_USE_IMG_RGB){
                    _create_offset_map_vertical(cropper,uxmap,uymap,offset_maps[idx],idx*cropper->hight,step);
                }
            }
        }
    }
...
void qtk_stitch_corpper_nearest_2tables(qtk_stitch_cropper_t* cropper)
{
    ...
    if(cropper->img_type == QTK_STITCH_USE_IMG_NV12){
        ...
        //显光反馈内存占用  清理下
        delete[] crop_offset_maps;
        cropper->crop_offset_maps = NULL;
        delete[] crop_offset_uv_maps;
        cropper->crop_uv_offset_maps = NULL;
    }
    return;
}

//把邻近插值算法的单独 uv offset转换为批量处理的做法
void qtk_stitch_corpper_nearest_uv_2tables(qtk_stitch_cropper_t* cropper)
{
    int n = cropper->ncamera;
    cv::Mat *crop_offset_uv_maps = (cv::Mat*)cropper->crop_uv_offset_maps;
    if(cropper->img_type == QTK_STITCH_USE_IMG_NV12){
...
public:
    ...
    cromper_remape_parallel(int _type, cv::Mat *_img,cv::Mat *_dst,cv::Mat *_offset_map,cv::Mat *_uymap,int _image_direction)
    {
        img = _img;
        dst = _dst;
        offset_map = _offset_map;
        type = _type;
        uymap = _uymap;
        image_direction = _image_direction;
    }
...
void _stitch_remap_linear_vertical_rect(cv::Mat &img,cv::Mat &dst,cv::Mat &offset_map,cv::Mat &uymap, int s, int e)
{
    ...
    for(; n < cross_n; ++n,++XY,++FXY){
        ...
        D[0] = (S[0]*w[1] + S[sstep]*w[0])>>INTER_REMAP_COEF_BITS;
        D[1] = (S[1]*w[1] + S[sstep1]*w[0])>>INTER_REMAP_COEF_BITS;
        D[2] = (S[2]*w[1] + S[sstep2]*w[0])>>INTER_REMAP_COEF_BITS;
        D += 3;
    }
    return;
}
    ...
    if(type==cv::INTER_NEAREST){
        ...
        if(dst->channels() == 1){
            if(image_direction == 0){
                ...
                _stitch_remap_linear_1channel_vertical_rect(*img,*dst,*offset_map,*uymap,range.start,range.end);
            }
        }else if(dst->channels() == 3){
            if(image_direction==0){
                _stitch_remap_linear_rect(*img,*dst,*offset_map,*uymap,range.start,range.end);
            }else{
                _stitch_remap_linear_vertical_rect(*img,*dst,*offset_map,*uymap,range.start,range.end);
            }
        }
    }
    return;
...
public:
    ...
    cromper_table_remape_parallel(int _type, cv::Mat *_img,cv::Mat *_dst,cv::Mat *_offset_map,cv::Mat *_uymap,cv::Mat *_table)
    {
        img = _img;
        dst = _dst;
        table = _table;
...
void cromper_table_remape_parallel::operator()(const cv::Range& range) const
{
    if(type == cv::INTER_NEAREST){
        ...
        _stitch_remap_linear2_1channel_rect(*img,*dst,*offset_map,*uymap,*table,range.start,range.end);
    }
    return;
}
                                ...
                                cv::UMat *uxmap,cv::UMat *uymap, cv::Mat *offset_map,cv::Mat *dst)
{
    ...
    if(warpe->remap_type == cv::INTER_NEAREST){
        ...
        class cromper_remape_parallel cp = cromper_remape_parallel(cv::INTER_LINEAR,img,dst,offset_map,&uymap_,warpe->image_direction);
        cv::parallel_for_(cv::Range(0,dst->rows),cp);
#endif
    }
    else{
...
void _threads_cb(void *userdate,int start,int end)
{
    _the_threads_date_t *thread_date = (_the_threads_date_t *)userdate;
    if(thread_date->type==cv::INTER_NEAREST){
        if(thread_date->dst->channels() == 1){
            _stitch_remap_nearest_1channel_rect(*thread_date->img,*thread_date->dst,*thread_date->offset_map,start,end);
        }else if(thread_date->dst->channels() == 2){
            _stitch_remap_nearest_2channel_rect(*thread_date->img,*thread_date->dst,*thread_date->offset_map,start,end);
        }else if(thread_date->dst->channels() == 3){
            _stitch_remap_nearest_rect(*thread_date->img,*thread_date->dst,*thread_date->offset_map,start,end);
        }
    }else if(thread_date->type==cv::INTER_LINEAR){
        if(thread_date->dst->channels() == 1){
            if(thread_date->image_direction == 0){
                _stitch_remap_linear_1channel_rect(*thread_date->img,*thread_date->dst,*thread_date->offset_map,*thread_date->uymap,start,end);
            }else{
                _stitch_remap_linear_1channel_vertical_rect(*thread_date->img,*thread_date->dst,*thread_date->offset_map,*thread_date->uymap,start,end);
            }
        }else if(thread_date->dst->channels() == 3){
            if(thread_date->image_direction==0){
                _stitch_remap_linear_rect(*thread_date->img,*thread_date->dst,*thread_date->offset_map,*thread_date->uymap,start,end);
            }else{
                _stitch_remap_linear_vertical_rect(*thread_date->img,*thread_date->dst,*thread_date->offset_map,*thread_date->uymap,start,end);
            }
        }
    }
    return;
...
void _threads_nearest_table_cb(void *userdate,int start,int end)
{
    _the_threads_date_t *thread_date = (_the_threads_date_t *)userdate;
    if(thread_date->dst->channels() == 1){
        _stitch_remap_nearest_table_1channel_rect(*thread_date->img,*thread_date->dst,*thread_date->table,start,end);
    }else if(thread_date->dst->channels() == 2){
        _stitch_remap_nearest_table_2channel_rect(*thread_date->img,*thread_date->dst,*thread_date->table,start,end);
    }
    return;
}
#endif
                            ...
                            cv::UMat *uymap,cv::Mat *offset_map,cv::Mat *uv_offset_map,cv::Mat *dst)
{
...
#ifdef QTK_THREAD
        _the_threads_date_t user_date;
        user_date.type = cv::INTER_NEAREST;
        user_date.img = img;
        user_date.dst = dst;
        user_date.offset_map = offset_map;
        qtk_stitch_threads_process(&user_date,_threads_cb,0,offset_map->rows,-1);
#else
        class cromper_remape_parallel cp = cromper_remape_parallel(cv::INTER_NEAREST,img,dst,offset_map);
...
#ifdef QTK_THREAD
        _the_threads_date_t user_date;
        user_date.type = cv::INTER_LINEAR;
        user_date.img = img;
        user_date.dst = dst;
        user_date.offset_map = offset_map;
        user_date.uymap = &uymap_;
        user_date.image_direction = warpe->image_direction;
        qtk_stitch_threads_process(&user_date,_threads_cb,0,offset_map->rows,-1);
#else
        class cromper_remape_parallel cp = cromper_remape_parallel(cv::INTER_LINEAR,img,dst,offset_map,&uymap_,warpe->image_direction);
        cv::parallel_for_(cv::Range(0,offset_map->rows),cp);
    ...
    user_date.img = img;
    user_date.dst = &dp;
    user_date.offset_map = uv_offset_map;
    qtk_stitch_threads_process(&user_date,_threads_cb,0,uv_offset_map->rows,-1);
#else
...
cv::Mat _corpper_crop_image_nv12_liner_remap2(qtk_stitch_cropper_t *warpe,qtk_stitch_image_t *image,
                            cv::UMat *uymap,cv::Mat *offset_map,wtk_strbuf_t *_uv_table,cv::Mat *dst)
{
    // cv::Mat dst;
    cv::Mat *img = NULL;
    ...
    // user_date.img = img;
    // user_date.dst = &dp;
    // user_date.table = &uv_table;
    // qtk_stitch_threads_process(&user_date,_threads_nearest_table_cb,0,uv_table.rows,-1);
#else
    class cromper_table_remape_parallel cp_uv = cromper_table_remape_parallel(cv::INTER_NEAREST,img,&dp,&uv_table);
    cv::parallel_for_(cv::Range(0,uv_table.rows),cp_uv);
#endif
...
//尝试把blender的table数据引入 部分不作试试
cv::Mat _corpper_crop_image_nv12_liner_remap3(qtk_stitch_cropper_t *warpe,qtk_stitch_image_t *image,
                            cv::UMat *uymap,cv::Mat *offset_map,wtk_strbuf_t *_weight_table,
                            wtk_strbuf_t *_uv_table,cv::Mat *dst)
{
    // cv::Mat dst;
    cv::Mat *img = NULL;
    ...
    // cv::Mat uymap_(uymap->getMat(cv::ACCESS_RW));
    cv::Mat uymap_(uymap->getMat(cv::ACCESS_RW));
    cv::Mat weight_table = cv::Mat(cv::Size(1,_weight_table->pos/(4*sizeof(int))), CV_32SC4, _weight_table->data);
    // _stitch_remap_linear2_1channel(*img,*dst,*offset_map,uymap_,weight_table);
#ifdef QTK_THREAD
    // _the_threads_date_t user_date;
    // user_date.type = cv::INTER_LINEAR;
    // user_date.img = img;
    ...
    // qtk_stitch_threads_process(&user_date,_threads_cb,0,offset_map->rows,-1);
#else
    class cromper_table_remape_parallel cp = cromper_table_remape_parallel(cv::INTER_LINEAR,img,dst,offset_map,&uymap_,&weight_table);
    cv::parallel_for_(cv::Range(0,weight_table.rows),cp);
#endif
                ...
                wtk_strbuf_t *_table, wtk_strbuf_t *_uv_table, cv::Mat *dst)
{
    ...
    user_date.img = img;
    user_date.dst = dst;
    user_date.table = &table;
    qtk_stitch_threads_process(&user_date,_threads_nearest_table_cb,0,table.rows,-1);
#else
    ...
    for(int i = 0; i < n; ++i){
        ...
        if(cropper->img_type == QTK_STITCH_USE_IMG_RGB){
            ...
            if(cropper->remap_type == cv::INTER_NEAREST){
                ...
                cv::UMat map2 = concert_maps2[i].getUMat(cv::ACCESS_RW);
                if(cropper->image_direction == 0){
                    _corpper_crop_image_nv12_liner_remap2(cropper,(qtk_stitch_image_t*)image->img_data,
                                            &map2,offset_maps+i,cropper->crop_offset_uv_table[i],images+i);
                }else{
                    _corpper_crop_image_nv12_remap2(cropper,(qtk_stitch_image_t*)image->img_data,
                                            &map2,offset_maps+i,uv_offset_maps+i,images+i);
                }
...
void qtk_stitch_corpper_crop_remaps3_all(qtk_stitch_cropper_t* cropper,wtk_queue_t* queue, 
                                            void *_weight_table, void *_uv_weight_table)
{
    int n = cropper->ncamera;
    wtk_queue_node_t *node = NULL;
    qtk_stitch_queue_data_t *image = NULL;
    cv::Mat *images = (cv::Mat*)cropper->imgs;
    wtk_strbuf_t **weight_table = (wtk_strbuf_t**)_weight_table;
    // wtk_strbuf_t **uv_weight_table = (wtk_strbuf_t**)_uv_weight_table;
    ...
    for(int i = 0; i < n; ++i){
        ...
        if(cropper->img_type == QTK_STITCH_USE_IMG_RGB){
            ...
            if(cropper->remap_type == cv::INTER_NEAREST){
                ...
                cv::UMat map2 = concert_maps2[i].getUMat(cv::ACCESS_RW);
                if(cropper->image_direction == 0){
                    _corpper_crop_image_nv12_liner_remap3(cropper,(qtk_stitch_image_t*)image->img_data,
                                            &map2,offset_maps+i,weight_table[i],cropper->crop_offset_uv_table[i],images+i);
                }else{
                    _corpper_crop_image_nv12_remap2(cropper,(qtk_stitch_image_t*)image->img_data,
                                            &map2,offset_maps+i,uv_offset_maps+i,images+i);
                }
...
void _stitch_remap_linear2_1channel(cv::Mat &img,cv::Mat &dst,cv::Mat &offmap,cv::Mat &uymap,cv::Mat &weight_table)
{
    ...
    // int height = offmap.rows;
    // int width = offmap.cols;
    int* SXY = offmap.ptr<int>(0,0);
    uchar* SD = dst.ptr<uchar>(0,0);
    ushort* SFXY = uymap.ptr<ushort>(0,0);
    uchar* w = NULL;
    uchar* S = NULL;
    int cross_n = weight_table.rows;
    int *table = (int*)weight_table.data;

    for(int n = 0; n < cross_n; ++n){
        int type = table[0];
        int tl = table[1];
        if(type > 0){
            int* XY = SXY+table[2];
            ushort* FXY=SFXY + table[2];
            uchar* D = SD + table[2];
            for(int i = 0; i < tl; ++i){
                w = (uchar*)FXY;
                S = S0 + *XY;
                                        ...
                                        cv::Mat &weight_table,int s, int e)
{
    ...
    uchar* w = NULL;
    uchar* S = NULL;
    int *table = weight_table.ptr<int>(s,0);
...
void qtk_stitch_cropper_limit_region_masks(qtk_stitch_cropper_t* cropper)
{
    cv::Mat *masks = (cv::Mat*)cropper->masks;
    ...
    for(int i = 2; i < n; ++i){
        ...
        int k_pre = i-2;
        cv::Point corner = corners->at(i);
        cv::Point corner_pre = corners->at(k_pre);
        int lop = corner_pre.x+masks[k_pre].cols - corner.x;
        // char path[256]  = {0,};
        // sprintf(path,"low_mask%d.png",i);
        // qtk_stitch_image_save_data(path,masks[i].data,masks[i].cols,masks[i].rows,1);
        // sprintf(path,"low_mask%d.png",k_pre);
        // qtk_stitch_image_save_data(path,masks[k_pre].data,masks[k_pre].cols,masks[k_pre].rows,1);
        if(lop > 0){ //有重叠了要去掉 一边减掉一半
            // std::cout << corner_pre << std::endl;
            // std::cout << corner << std::endl;
            int offset = (lop/2) + 1;
            cv::rectangle(masks[i],cv::Point(0,0),cv::Point(offset,masks[i].rows-1),0,-1);
            int right_len = masks[k_pre].cols - offset;
            cv::rectangle(masks[k_pre],cv::Point(right_len,0),cv::Point(masks[k_pre].cols-1,masks[k_pre].rows-1),0,-1);
        }
        // sprintf(path,"low_mask_limit_%d.png",i);
        // qtk_stitch_image_save_data(path,masks[i].data,masks[i].cols,masks[i].rows,1);
        // sprintf(path,"low_mask_limit_%d.png",k_pre);
        // qtk_stitch_image_save_data(path,masks[k_pre].data,masks[k_pre].cols,masks[k_pre].rows,1);
    }
    return;
}
